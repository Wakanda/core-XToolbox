...
Boolean XWinStyledTextBox::DoDraw(const XBOX::VRect &inBounds)
{
	...
	hr = fTextServices->TxDraw(
		...
		fInplaceActive ? TXTVIEW_ACTIVE : TXTVIEW_INACTIVE);		// What view of the object could be TXTVIEW_ACTIVE
...
void XWinStyledTextBox::DoApplyStyle(const VTextStyle* inStyle, VTextStyle * inStyleInherit)
{
	...
	{
		AlignStyle alignment = VDocBaseLayout::JustToAlignStyle(inStyle->GetJustification());
...
void XWinStyledTextBox::GetSizeForPrinting(GReal &ioWidth, GReal &outHeight)
{
	...
	if (fTextServices)
	{
		...
		width = MulDiv( ioWidth+fabs(offsetX), outputDCDPI ,72);
		width =  MulDiv(width,72,refDCDPI);
		szExtent.cx = width;

		fTextServices->TxSendMessage(EM_GETZOOM, (WPARAM)&ratio, (LPARAM)&denom, &r);
		fTextServices->TxSendMessage(EM_SETZOOM, (WPARAM)refDCDPI, (LPARAM)72, &r);
		...
		fTextServices->TxGetNaturalSize(DVASPECT_CONTENT, 
			...
			&width,
			&height);
		fTextServices->TxSendMessage(EM_SETZOOM, (WPARAM)ratio, (LPARAM)denom ,&r);
	
		ioWidth = ((width-fabs(offsetX)) *72) / (outputDCDPI*1.0);

		if (ioWidth < 0.0f)
			ioWidth = 0.0f;
		outHeight = (height * 72) / (outputDCDPI*1.0);
	}
}
...
sLONG XWinStyledTextBox::_GetLineHeight( VFont *inFont)
{
	...
	sLONG lineHeight = ICSSUtil::PointToTWIPS( metrics->GetLineHeight()*fFontSizeToDocumentFontSize); 
...
bool XWinStyledTextBox::_VTextStyleFromCharFormat(VTextStyle* ioStyle, CHARFORMAT2W* inCharFormat)
{
	...
	ioStyle->SetJustification(VDocBaseLayout::JustFromAlignStyle(fHAlign));
