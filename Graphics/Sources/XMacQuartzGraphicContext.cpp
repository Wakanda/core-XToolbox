...
{	
	...
	{
		assert(fAxisInvertedStack->IsEmpty());
...
void VMacQuartzGraphicContext::SetLineWidth(GReal inWidth, VBrushFlags* /*ioFlags*/)
{
	if(fHairline && fPrinterScale)
		::CGContextSetLineWidth(fContext, 0.01); // pas trouv� d'autre moyen pour faire du hairline
	else
...
void VMacQuartzGraphicContext::SetTransparency(GReal inAlpha)
{
...
GReal VMacQuartzGraphicContext::GetTextWidth( const VString& inString) const
{
	...
	fTextLayoutMode = TLM_NORMAL;
	return GetFontMetrics()->GetTextWidth( inString);	
}
...
void VMacQuartzGraphicContext::SaveContext()
{
	if (fAxisInvertedStack == NULL)
		fAxisInvertedStack = new VArrayOf<Boolean>;
	
	fAxisInvertedStack->Push(fAxisInverted);
	fLineWidthStack.Push(fLineWidth);
	fLineDashStack.Push(fUseLineDash);
	
	fFillPatternStack.Push(fFillPattern);
	RetainRefCountable(fFillPattern);
	
	
	fLinePatternStack.Push(fLinePattern);
...
void VMacQuartzGraphicContext::RestoreContext()
{
	...
	fLineWidth=fLineWidthStack.Pop();
	
	
	VPattern* pat=fFillPatternStack.Pop();
	CopyRefCountable(&fFillPattern,pat);
	ReleaseRefCountable(&pat);
	
	
	pat=fLinePatternStack.Pop();
	CopyRefCountable(&fLinePattern,pat);
	ReleaseRefCountable(&pat);
	
	fAxisInverted = fAxisInvertedStack->Pop();
	fUseLineDash = fLineDashStack.Pop();
}
...
void VMacQuartzGraphicContext::DrawText( const VString& inString, TextLayoutMode inLayoutMode)
{
	DrawText( inString, fTextPos, inLayoutMode);
}
...
void	VMacQuartzGraphicContext::DrawText (const VString& inString, const VPoint& inPos, TextLayoutMode inLayoutMode)
{
	SaveContext();
	_UseQuartzAxis();
	...
	{
		CGFloat primaryOffset = (inLayoutMode & TLM_RIGHT_TO_LEFT) ? 0 : CTLineGetOffsetForStringIndex(line, inString.GetLength(), NULL);
...
// if input bounds width is not null, use it as max line wrapping width (or max text height in vertical layout mode)
// if input bounds height is not null, use it as max text height (or max line wrapping width in vertical layout mode)
void VMacQuartzGraphicContext::GetStyledTextBoxBounds( const VString& inText, VTreeTextStyle *inStyles, VRect& ioBounds, const GReal inRefDocDPI)
{
	if (inText.IsEmpty())
	{
...
*/
bool VMacQuartzGraphicContext::DrawLayerOffScreen(const VRect& inBounds, VImageOffScreenRef inBackBuffer)
{
	if (!inBackBuffer)
		return false;
	CGLayerRef layer = inBackBuffer ? (CGLayerRef)(*inBackBuffer) : NULL;
...
VMacQuartzBitmapContext::VMacQuartzBitmapContext(const VRect& inBounds, Boolean inReversed):VMacQuartzGraphicContext()
{
	_Init();
...
Boolean VMacQuartzBitmapContext::CreateBitmap(const VRect& inHwndBounds)
{
	...
	if (fBitmapData != NULL)
		//VMemory::SetPtrSize((VPtr)fBitmapData, totalBytes);
		VMemory::DisposePtr((VPtr)fBitmapData);
	//else
	...
	{
		CGColorSpaceRef	colorSpace = ::CGColorSpaceCreateWithName(kCGColorSpaceGenericRGB);
		fContext = ::CGBitmapContextCreate(fBitmapData, width, height, 8, bytesPerRow, colorSpace, kCGImageAlphaPremultipliedFirst);
		::CGColorSpaceRelease(colorSpace);
		if(fContext!=NULL)
			SaveContext();
	}
