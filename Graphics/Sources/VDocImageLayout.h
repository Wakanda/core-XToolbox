...
*/
class XTOOLBOX_API VDocImageLayout: public VDocBaseLayout
{
public:
							VDocImageLayout(const VString& inURL, const VString& inAltText = CVSTR(""));

		/** create from the passed image node */
							VDocImageLayout(const VDocImage *inNode, VDocClassStyleManager *ioCSMgr = NULL);
...
/////////////////
		...
		*/
		...
		const VPictureData*	RetainPictureData() const;
...
*/
class XTOOLBOX_API VDocBackgroundImageLayout: public VObject
{
public:
							/** background image layout 
...
virtual									~VDocImageLayoutPictureDataManager();
		...
		*/
		bool							NormalizeURLBasePath( VString& ioURL);		
		
		/** encode picture to data URI string (using base64 encoding) */
		bool							EncodeToDataURI( const VPictureData *inPictureData, VString& outDataURI);				
protected:
		VPictureData*					_DecodePictureDataURI( const VString& inDataURI);


		typedef std::vector<VPictureProxyCache_Folder *> VectorOfFolderCache;
		typedef unordered_map_VString<VRefPtr<VPictureData> > MapOfPictureDataPerDataURI;
		...
		MapOfPictureDataPerDataURI			fMapOfPictureDataPerDataURI;
