			...
			else
			{
				...
				else
				{
					...
					if(!data)
					{
						...
						{
							// on a rien trouvé, peut etre une image quicktime
...
const VPictureData* VPicture::_GetBestForPrinting()
{
	...
	if(!data)
	{
		...
		if(!data)
		{
			data=_GetPictDataByExtension(sCodec_tiff); //priorité au tiff qui est utilisé pour des doc haute def 
			if(!data)
...
void VPicture::_SelectDefault()
{
	...
	if(data)
	{
		...
		{
			// c'est une annimation on prend le gif en priorité
			fBestForDisplay=data;
		}
		else
		{
...
void VPicture::FromVPicture_Retain(const VPicture& inData,bool inKeepSettings,bool inCopyOutsidePath)
{
	...
	{
		// image lié a la base de donnée
		if(!inData._IsValueSourceDirty())
		{
			if(!inData.fVValueSourceLoaded)
			{
				// la pict n'est pas encore chargé.
			...
			//else
			//{
			//	// L'image est deja chargée, on utilise les pictdata en memoire afin de beneficier du refconting.
			//}
		}
		//else
		//{
...
void* VPicture::GetPicHandle(bool inAppendPicEnd) const
{
	...
	if(data)
	{
		...
		if(result && (inAppendPicEnd && outCanAddPicEnd))
		{
			...
			{
				// mem error, on peut pas ajouté le picend
...
VError	VPicture::GetPictureForRTF(VString& outRTFKind,VBlob& outBlob)
{
	...
	#if VERSIONMAC
	// force le format pict car le pdf n'est pas supporté dans le rtf
