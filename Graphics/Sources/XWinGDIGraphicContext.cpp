...
void VWinGDIGraphicContext::SetTransparency(GReal inAlpha)
{
	// Not supported
}
...
GReal VWinGDIGraphicContext::GetTextWidth(const VString& inString) const 
{
	SIZE	size;
...
void VWinGDIGraphicContext::DrawText(const VString& inString, TextLayoutMode inOptions)
{
	...
	uLONG	savedAlign = ::SetTextAlign(fContext, TA_BASELINE | TA_UPDATECP);
...
bool VWinGDIGraphicContext::DrawLayerOffScreen(const VRect& inBounds, VImageOffScreenRef inBackBuffer)
{
	if (!inBackBuffer)
		return false;
	if (fPrinterScale) //skip if printing
...
void VWinGDIGraphicContext::BeginUsingContext(bool /*inNoDraw*/)
{
	...
	{		
		...
		if(logx!=reflogx)
		{
			...
			//fSaveMapMode = SetMapMode(fContext,MM_ANISOTROPIC);
			
			//SetWindowExtEx(fContext,MulDiv(nPrnWidth, 72,logx) ,MulDiv(nPrnHeight, 72,logy),NULL);
			//SetViewportExtEx(fContext,nPrnWidth,nPrnHeight,NULL);
		
			//GetWindowExtEx(fContext,&wExt);
			//GetViewportExtEx(fContext,&vExt);
