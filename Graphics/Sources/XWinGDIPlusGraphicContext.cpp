...
@remarks
	use method GetGdiplusBitmap() to get bitmap ref
*/
VWinGDIPlusGraphicContext::VWinGDIPlusGraphicContext( const sLONG inWidth, const sLONG inHeight, const Gdiplus::PixelFormat inPixelFormat)
{
	_Init();
...
void VWinGDIPlusGraphicContext::_Init()
{
	...
	fLineColor = Gdiplus::Color(0,0,0);
	fStrokePen = NULL;
	fFillBrush = NULL;
	fLineBrush = NULL;
	fTextBrush = NULL;
...
void VWinGDIPlusGraphicContext::_InitGDIPlusObjects(bool inBeginUsingContext)
{
	...
	if (inBeginUsingContext && fGDIPlusGraphics == NULL)
	{
		if(fRefContext)
		{
			fOwnGraphics=true;
...
void VWinGDIPlusGraphicContext::BeginUsingContext(bool inNoDraw)
{
	...
	if (fUseCount==1)
	{
		...
		fTextRenderingMode = ~fTextRenderingMode;
		SetTextRenderingMode(~fTextRenderingMode);
	}
}
...
void VWinGDIPlusGraphicContext::SetTransparency(GReal inAlpha)
{
...
GReal VWinGDIPlusGraphicContext::GetTextWidth(const VString& inString) const 
{
	...
	{
		VRect bounds;
		_GetLegacyTextBoxSize(inString, bounds, TLM_NORMAL);
...
void VWinGDIPlusGraphicContext::DrawText(const VString& inString, TextLayoutMode inLayoutMode)
{
	if ((fTextRenderingMode & TRM_LEGACY_ON) || ((!(fTextRenderingMode & TRM_LEGACY_OFF)) && (!fTextFont->IsTrueTypeFont())))
	{
		//legacy GDI drawing 

		DrawText( inString, VPoint(fTextRect->X, fTextRect->Y), inLayoutMode);
		return;
	}
...
void VWinGDIPlusGraphicContext::DrawText(const VString& inString, const VPoint& inPos, TextLayoutMode inLayoutMode)
{
	GReal	x = inPos.GetX();
	GReal	y = inPos.GetY();
	...
	if ((fTextRenderingMode & TRM_LEGACY_ON) || ((!(fTextRenderingMode & TRM_LEGACY_OFF)) && (!fTextFont->IsTrueTypeFont())))
	{
		...
		if (hdc)
		{
			...
			if (!(inLayoutMode & TLM_TEXT_POS_UPDATE_NOT_NEEDED))
			{
				...
				fTextRect->X = pos.GetX();
				fTextRect->Y = pos.GetY();
			}
			gcGDI->Release();
		}
		...
		while (*c)
		{
			if (*c != 0x20)
				fGDIPlusGraphics->DrawString(
						...
						fontNative, 
						pos,
						((inLayoutMode & (TLM_VERTICAL|TLM_RIGHT_TO_LEFT)) != 0) && format ? format : Gdiplus::StringFormat::GenericTypographic(),
						_GetTextBrush());
			if (inLayoutMode & TLM_VERTICAL)
 ...
 */
bool VWinGDIPlusGraphicContext::DrawLayerOffScreen(const VRect& inBounds, VImageOffScreenRef inBackBuffer)
{
	if (!inBackBuffer)
		return false;
	Gdiplus::Bitmap *bmpLayer = (Gdiplus::Bitmap *)(*inBackBuffer);
