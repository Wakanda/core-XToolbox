...
const VPictureCodec* VPictureCodecFactory::RetainDecoder(VFile& inFile,bool inCheckData)
{
	...
	if(inFile.Exists())
	{
		...
		if(!codec && inCheckData)
		{
			for(VSize i=0;i<fBuiltInCodec.size();i++)
			{
				if(fBuiltInCodec[i]->ValidateData(inFile))
				{
					codec=fBuiltInCodec[i];
					codec->Retain();
					break;
			...
			if(!codec)
			{
				...
				{
					if(fPlatformCodec[i]->ValidateData(inFile))
					{
						codec=fPlatformCodec[i];
						codec->Retain();
						break;
				...
				if(!codec)
				{
					...
					{
						if(fQTCodec[i]->ValidateData(inFile))
						{
							codec=fQTCodec[i];
							codec->Retain();
							break;
					...
					if(!codec)
					{
						...
						for(VSize i=0;i<fDynamicCodec.size();i++)
						{
							if(fDynamicCodec[i]->ValidateData(inFile))
							{
								codec=fDynamicCodec[i];
								codec->Retain();
								break;
...
VPictureData* VPictureCodec::CreatePictDataFromVFile(VFile& inFile) const
{
	VPictureData* result;
