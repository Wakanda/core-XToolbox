...
*/
VGraphicContext *VWinD2DGraphicContext::_BeginUsingGdiplusContext( const VRect* inBounds) const
{
	xbox_assert(fChildGC == NULL);

	//constraint clipping to the passed bounds
	if (inBounds)
	{
		fChildGCNeedRestoreClip = true;
		_SaveClip();
		_ClipRect( *inBounds);
	}
	else
		fChildGCNeedRestoreClip = false;

	//get DC from render target or parent dc
	fChildHDC = BeginUsingParentContext();
	if (!fChildHDC)
		return NULL;

	//create Gdiplus context from GDI context
	fChildGC = static_cast<VGraphicContext *>(new VWinGDIPlusGraphicContext( fChildHDC));
	if (!testAssert(fChildGC))
	{
		EndUsingParentContext( fChildHDC);
		fChildHDC = NULL;
		return NULL;
	}

	fChildGC->BeginUsingContext();

	//inherit settings from this context
	Gdiplus::Status status;

	fChildGC->SetMaxPerfFlag( GetMaxPerfFlag());

	fChildGC->SetDesiredAntiAliasing( GetDesiredAntiAliasing());

	fChildGC->EnableShapeCrispEdges( IsShapeCrispEdgesEnabled());
	fChildGC->AllowCrispEdgesOnPath( IsAllowedCrispEdgesOnPath());
	fChildGC->AllowCrispEdgesOnBezier( IsAllowedCrispEdgesOnBezier());

	if (fTextRenderingMode & TRM_WITHOUT_ANTIALIASING)
		fChildGC->SetTextRenderingMode( fTextRenderingMode);
	else
		fChildGC->SetTextRenderingMode( (fTextRenderingMode & (TRM_CONSTANT_SPACING|TRM_LEGACY_ON|TRM_LEGACY_OFF)) | TRM_WITH_ANTIALIASING_NORMAL);

	fChildGC->SetSpaceKerning( GetSpaceKerning());
	fChildGC->SetCharKerning( GetCharKerning());

	fChildGC->UseShapeBrushForText( UseShapeBrushForText());
	if (fTextFont)
		fChildGC->SetFont( fTextFont);
	VColor color;
	GetTextColor( color);
	fChildGC->SetTextColor( color);

	VPoint pos;
	GetTextPos( pos);
	fChildGC->SetTextPosTo( pos);

	return fChildGC;
}

/** end using derived Gdiplus context over render target */
void VWinD2DGraphicContext::_EndUsingGdiplusContext() const
{
	if (!fChildGC)
		return;

	VPoint pos;
	fChildGC->GetTextPos( pos);

	fChildGC->EndUsingContext();
	delete fChildGC;
	fChildGC = NULL;
	...
	if (fChildGCNeedRestoreClip)
		_RestoreClip();

	fTextRect.left = pos.GetX();
	fTextRect.top = pos.GetY();
}
...
void VWinD2DGraphicContext::_SetLinePattern(const VPattern* inPattern, VBrushFlags* ioFlags) const
{
	...
	D2D1_CAP_STYLE dashCap = fStrokeStyle->GetDashCap();
	D2D1_CAP_STYLE newDashCap = dashCap;
	
	if (stdp == NULL)
	{
		newDashStyle = D2D1_DASH_STYLE_SOLID;
		newDashCap = fStrokeStyleProps.startCap;
	...
	if (newDashStyle != dashStyle || newDashCap!=dashCap)
	{
		...
		fStrokeStyleProps.dashCap = newDashCap;
		if (fStrokeDashPattern)
			delete [] fStrokeDashPattern;
		fStrokeDashPattern = NULL;
		fStrokeDashCount = 0;
		...
		if (isDashDot)
		{
			...
			fStrokeDashPattern[0] = fStrokeDashPattern[1] = 1 / (fStrokeWidth == 0.0f ? 0.0001f : fStrokeWidth);
		}
		fMutexFactory.Lock();
...
void VWinD2DGraphicContext::_SetLineDashPattern(GReal inDashOffset, const VLineDashPattern& inDashPattern, VBrushFlags* ioFlags) const
{
	...
	if (inDashPattern.size() < 2)
	{
		...
		if (dashStyle != D2D1_DASH_STYLE_SOLID)
		{
			...
			fStrokeStyleProps.dashCap = fStrokeStyleProps.startCap;
			if (fStrokeDashPattern)
				delete [] fStrokeDashPattern;
			fStrokeDashPattern = NULL;
			fStrokeDashCount = 0;
...
GReal VWinD2DGraphicContext::GetTextWidth(const VString& inString) const
{
	if (inString.IsEmpty())
		return 0.0f;
	if (fRenderTarget == NULL)
	...
	return fTextFontMetrics->GetTextWidth( inString);
}
...
void VWinD2DGraphicContext::GetTextBoundsTypographic( const VString& inString, VRect& oRect, TextLayoutMode inLayoutMode) const
{
	...
	{
		VGraphicContext	*gc = _BeginUsingGdiplusContext( NULL);
		if (gc)
		{
			gc->GetTextBoundsTypographic( inString, oRect, inLayoutMode);
			_EndUsingGdiplusContext();
			return;
		}
	}
...
void VWinD2DGraphicContext::DrawTextBox(const VString& inText, AlignStyle inHAlign, AlignStyle inVAlign, const VRect& inHwndBounds, TextLayoutMode inLayoutMode)
{
	...
	if (inLayoutMode & TLM_VERTICAL)
	{
		VGraphicContext	*gc = _BeginUsingGdiplusContext( &inHwndBounds);
		if (gc)
		{
...
void VWinD2DGraphicContext::DrawText(const VString& inString, TextLayoutMode inLayoutMode)
{
	if (inString.IsEmpty())
		return;
	...
	if ((fTextRenderingMode & TRM_LEGACY_ON) || ((!(fTextRenderingMode & TRM_LEGACY_OFF)) && (!fTextFont->IsTrueTypeFont())))
	{
		...
		DrawText( inString, VPoint(fTextRect.left, fTextRect.top), inLayoutMode);
		return;
	}
...
void VWinD2DGraphicContext::DrawText(const VString& inString, const VPoint& inPos, TextLayoutMode inLayoutMode)
{
	if (inString.IsEmpty())
		return;
	...
	if ((fTextRenderingMode & TRM_LEGACY_ON) || ((!(fTextRenderingMode & TRM_LEGACY_OFF)) && (!fTextFont->IsTrueTypeFont())))
	{
		...
		if (hdc)
		{
			if (fGDI_GC)
			{
				...
				fGDI_GC->DrawText( inString, inLayoutMode);
				if (!(inLayoutMode & TLM_TEXT_POS_UPDATE_NOT_NEEDED))
				{
					VPoint pos;
					fGDI_GC->GetTextPos( pos);
					fTextRect.left = pos.GetX();
					fTextRect.top = pos.GetY();
				}
			}
			else
			{
				...
				gcGDI->DrawText( inString, inLayoutMode);
				if (!(inLayoutMode & TLM_TEXT_POS_UPDATE_NOT_NEEDED))
				{
					VPoint pos;
					gcGDI->GetTextPos( pos);
					fTextRect.left = pos.GetX();
					fTextRect.top = pos.GetY();
				}
				gcGDI->Release();
	...
	if (inLayoutMode & TLM_VERTICAL)
	{
		...
		if (gc)
		{
			gc->DrawText( inString, inPos, inLayoutMode);
			_EndUsingGdiplusContext();
			return;
		}
	}
	...
	*/
	...
	//draw full text
	fRenderTarget->DrawTextLayout(	D2D1::Point2F( pos.GetX(), pos.GetY()-baseline),
									textLayout,
									fTextBrush,
									D2D1_DRAW_TEXT_OPTIONS_NONE);
...
void VWinD2DGraphicContext::_DrawTextLayout( VTextLayout *inTextLayout, const VPoint& inTopLeft)
{
	...
	if (inTextLayout->fCurOverhangLeft > 0.0f || 
		inTextLayout->fCurOverhangRight > 0.0f || 
		inTextLayout->fCurOverhangTop > 0.0f || 
		inTextLayout->fCurOverhangBottom > 0.0f || 
		inflateX != 0.0f)
	{
...
void VWinD2DGraphicContext::_UpdateTextLayout( VTextLayout *inTextLayout)
{
	...
	if (inTextLayout->fLayoutD2D)
	{
		...
		inTextLayout->fCurOverhangRight = textOverhangMetrics.right > 0.0f ? textOverhangMetrics.right : 0.0f;
		inTextLayout->fCurOverhangTop = textOverhangMetrics.top > 0.0f ? textOverhangMetrics.top : 0.0f;
...
void  VWinD2DGraphicContext::_GetStyledTextBoxBounds( const VString& inText, VTreeTextStyle *inStyles, AlignStyle inHAlign, AlignStyle inVAlign, TextLayoutMode inLayoutMode, 
													 ...
													 const GReal inRefDocDPI, bool inEnableLegacy, bool inReturnLayoutDesignBounds, void *inTextMetrics, void *inTextOverhangMetrics)
{
...
// if input bounds width is not null, use it as max line wrapping width (or max text height in vertical layout mode)
// if input bounds height is not null, use it as max text height (or max line wrapping width in vertical layout mode)
void VWinD2DGraphicContext::GetStyledTextBoxBounds( const VString& inText, VTreeTextStyle *inStyles, VRect& ioBounds, const GReal inRefDocDPI)
{
	_GetStyledTextBoxBounds( inText, inStyles, AL_LEFT, AL_TOP, TLM_NORMAL, ioBounds, NULL, inRefDocDPI);
}
 ...
 */
bool VWinD2DGraphicContext::DrawLayerOffScreen(const VRect& inBounds, VImageOffScreenRef inBackBuffer)
{
	if (!inBackBuffer)
		return false;
	...
	D2D1_BITMAP_INTERPOLATION_MODE interpolationMode = D2D1_BITMAP_INTERPOLATION_MODE_LINEAR;
	if (floor(ctmTranslation.GetX()) == ctmTranslation.GetX()
		&&
