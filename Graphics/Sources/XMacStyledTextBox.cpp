...
//		xbox::DebugMsg("XMacStyledTextBox::_ComputeCurFrame2 call count = %d\n", loopCounter);
	...
	fCurLayoutOffset.SetPosBy(x,0.0f);
}
	...
	if (justisset != JST_Notset && begin <= 0 && end >= textLength)
	{
		...
		if(justisset != JST_Notset)
		{
			...
			if (CTHelper::UseCocoaAttributes())
			{
				...
				{
					NSRange nsRangeOut = { nsRange.location, 0 };
					...
					if (paraStyleActual && nsRangeOut.location == nsRange.location && nsRangeOut.length == nsRange.length)
						//merge with actual para settings
...
#endif
			{
				...
				if (paraActual && range.location == rangeOut.location && range.length == rangeOut.length)
				{
					//merge with actual para settings
