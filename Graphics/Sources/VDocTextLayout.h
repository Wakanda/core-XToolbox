...
@remarks
	...
	- we distinguish two scopes for properties: container & paragraph scope
		for instance, borders can be defined separatly for the container - SetBorders - and for any paragraph which intersects a specified text range - SetParaBorders
		for some properties like SetLineHeight, scope is implicitly paragraph(s)
		Actually only common properties like margin, padding, border, etc... have double read/write accessors to distinguish between container element & paragraph element scope;
		but for paragraph-only properties like line height or tab, scope is only paragraph.
...
protected:
							...
							*/
							VDocContainerLayout(const VDocNode *inNode, bool inShouldEnableCache = true);
...
virtual	VDocParagraph*		CreateParagraph(sLONG inStart = 0, sLONG inEnd = -1);
		...
		*/
virtual	void				ApplyParagraphStyle( const VDocParagraph *inPara, bool inResetStyles = false, bool inApplyParagraphPropsOnly = false, sLONG inStart = 0, sLONG inEnd = -1);
...
virtual	void				SetParaMargins(const GReal inMarginLeft, const GReal inMarginRight = 0.0f, const GReal inMarginTop = 0.0f, const GReal inMarginBottom = 0.0f, sLONG inStart = 0, sLONG inEnd = -1)
		{
			ITextLayout::SetParaMargins(inMarginLeft, inMarginRight, inMarginTop, inMarginBottom, inStart, inEnd);
		}
...
virtual	void				GetParaMargins(GReal *outMarginLeft, GReal *outMarginRight, GReal *outMarginTop, GReal *outMarginBottom, sLONG inStart = 0, sLONG inEnd = -1) const
		{
			ITextLayout::GetParaMargins(outMarginLeft, outMarginRight, outMarginTop, outMarginBottom, inStart, inEnd);
		}
...
typedef	std::vector<VPoint>	VectorOfPoint;
		...
		void				_InitFromNode( const VDocNode *inNode);
...
public:
							...
							@param inShouldEnableCache
								enable/disable cache (default is true): see ShouldEnableCache
							*/
							VDocTextLayout(const VDocText *inDocText, bool inShouldEnableCache = true):VDocContainerLayout( static_cast<const VDocNode *>(inDocText), inShouldEnableCache) {}
