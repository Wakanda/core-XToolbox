...
@remarks
	...
	TODO: when VDocumentLayout is created, VDocumentLayout & VTextLayout should share common interface & VTextLayout no more reference VDocParagraphLayout
*/
...
public:
			...
			*/
								VTextLayout(bool inShouldEnableCache = true, bool inShouldUseNativeLayout = true);
	virtual						~VTextLayout();
			...
			*/
		    ...
		    @see 
				VDocParagraphLayout
			*/
			void				ShouldUseNativeLayout(bool inShouldUseNativeLayout);
;			bool				ShouldUseNativeLayout() const { return fShouldUseNativeLayout; }
			...
			*/
			...
			@remarks
				if true, selection is custom painted: if selection color is opaque, selection color is painted before the text (Mac-like) otherwise it is painted after (Word-like)
				if false, selection is not painted (but caller might use extended styles to apply native selection)
			*/
			...
			*/
								VTextLayout(const VDocParagraph *inPara, bool inShouldEnableCache = true, bool inShouldUseNativeLayout = true);
			...
			*/
            ...
            void                GetMargins( sDocPropRect& outMargins);
    
            /** get margins (in TWIPS) */
            void                SetMargins( const sDocPropRect& inMargins);
			...
			*/
			GReal				GetTextIndent() const { return fTextIndent; }

			/** set first line padding offset in point 
			@remarks
				might be negative for negative padding (that is second line up to the last line is padded but the first line)
			*/
			void				SetTextIndent(const GReal inPadding);
...
protected:
	...
	mutable VTreeTextStyle*		fDefaultStyle;
			...
			*/
			...
			@remarks
				used only if fShouldUseNativeLayout == false
			*/
			VDocParagraphLayout*	fParagraphLayout;

			/** text box layout used by Quartz2D or GDIPlus/GDI (Windows legacy impl) 
			@remarks
				used only if fShouldUseNativeLayout == true
			*/
			...
			*/
			...
			bool				fSelectionIsDirty;
			VectorOfRect		fSelectionRenderInfo;

			VPoint				fCurTopLeft;
			VRect				fCurLayerBounds;
