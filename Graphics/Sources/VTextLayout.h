...
public:
			...
			*/
								VTextLayout(bool inShouldEnableCache = true, bool inShouldUseNativePlatformForParagraphLayout = true);
	virtual						~VTextLayout();
			...
			*/
		    ...
		    @see 
				VTextLayout::VParagraphLayout
			*/
			void				ShouldUseNativePlatformForParagraphLayout(bool inShouldUseNativePlatformForParagraphLayout);
;			bool				ShouldUseNativePlatformForParagraphLayout() const { return fShouldUseNativePlatformForParagraphLayout; }
			...
			*/
								VTextLayout(const VDocParagraph *inPara, bool inShouldEnableCache = true, bool inShouldUseNativePlatformForParagraphLayout = true);
			...
			*/
			bool				GetCharIndexFromPos( VGraphicContext *inGC, const VPoint& inTopLeft, const VPoint& inPos, VIndex& outCharIndex)
			{
				...
				if (fParagraphLayout)
					//platform-independent impl
					inside	= fParagraphLayout->GetCharIndexFromPos( inGC, inTopLeft, inPos, outCharIndex);
				else
					//native impl
...
protected:
	...
	/** paragraph layout
	@remarks
		this class is dedicated to manage platform-independent (ICU & custom algorithms) paragraph layout
		only layout basic runs (same direction & style) are still computed & rendered with native impl (GDI/D2D on Windows, CoreText on Mac)

		text might be a single paragraph or multiple paragraphs (but with common paragraph style)

		this class is embedded in VTextLayout: VTextLayout internally uses it 
		if and only if VTextLayout::ShouldUseNativePlatformForParagraphLayout() is false (true on default for compatibility with v14)

		important: all metrics (ascent for instance) here are in res-independent point 
		(onscreen, it is equal to pixel on Windows platform as 1pt=1px onscreen in 4D form on Windows:
		 if 4D form becomes later dpi-aware on Windows, as form should be zoomed to screen dpi/72 as for printing or with Quartz2D on Mac, it will be ok too 
		 ...
		 TODO:  
				- determine bidi runs using ICU
				- borders
				- bullets
				- embedded objects (images,...)
				- etc...
	*/
	class	VParagraphLayout: public VObject
	{
	public:
								VParagraphLayout(VTextLayout *inLayout);
	virtual						~VParagraphLayout();
	...
	protected:
			...
			{
				kPARA_LTR	= 0,	//left to right layout
				kPARA_RTL	= 1,	//right to left layout
				kPARA_MIXED = 2		//bidirectional layout
			} eDirection;
			...
			*/
			...
			{
			public: 
										Run(Line *inLine, const sLONG inIndex, const sLONG inStart, const sLONG inEnd, 
											const eDirection inDirection = kPARA_LTR, VFont *inFont = NULL, const VColor *inTextColor = NULL, const VColor *inBackColor = NULL);
										Run(const Run& inRun);
			...
			virtual						~Run() { ReleaseRefCountable(&fFont); }
					...
					const VRect&		GetHitBounds() const { return fHitBounds; }
					...
					/** first pass: compute run metrics (metrics will be adjusted later in ComputeMetrics - to take account bidi for instance)
									here only run width is used before ComputeMetrics by VParagraph to check word-wrap
					...
					*/
					...
					void				ComputeMetrics(GReal& inBidiCurDirStartOffset, GReal& inBidiCurDirWidth, sLONG& inBidiCurDirStartIndex);
			protected:
					friend class VParagraphLayout;
					...
					*/
					...
					@remarks
						suitable for clipping & bounding box if word-wrap enabled (only for line last run) */
					VRect				fTypoBounds;

					/** run typo bounds with trailing spaces 
					@remarks
						suitable for clipping & bounding box if word-wrap disabled or for runs but the last one
					*/
					VRect				fTypoBoundsWithTrailingSpaces;

					/** run hit bounds 
					@remarks
						might be less than typo bounds as it is adjusted with kerning to avoid overlapping with adjacent run hit bounds: 
						suitable for hit testing (includes trailing spaces) */
					VRect				fHitBounds;
			...
			*/
			...
			virtual						~Line();
					...
					/** read/write accessor on runs as it is build by VParagraph while computing paragraph layout per line */
					...
					@param inWithTrailingSpaces
						true: include line trailing spaces bounds (suitable for painting and hit testing)
						false: does not include line trailing spaces bounds (suitable to determine bounding box & scrolling max width)
					*/
					const VRect&		GetTypoBounds(bool inWithTrailingSpaces = true) const { if (inWithTrailingSpaces) return fTypoBoundsWithTrailingSpaces; else return fTypoBounds; }
			...
			public:
					...
					typedef	std::vector<DirectionRange>			Directions;
										...
										VParagraph(VParagraphLayout *inParaLayout, const sLONG inIndex, const sLONG inStart, const sLONG inEnd);
			virtual						~VParagraph();
					...
					eDirection			GetDirection() const { return fDirection; }
					const Directions&	GetDirections() const { return fDirections; }
					...
					/** get typo bounds
					@param inWithTrailingSpaces
						only if word-wrap (otherwise typo bounds include always trailing spaces if any)
					...
					*/
					...
					void				ComputeMetrics(bool inUpdateBoundsOnly, const GReal inMaxWidth = 0, const GReal inMarginLeft = 0, const GReal inMarginRight = 0, const GReal inMarginTop = 0, const GReal inMarginBottom = 0);

			protected:
					friend class VParagraphLayout;

					void				_NextRun( Run* &run, Line *line, GReal& lineWidth, sLONG i, bool isTab = false);
					void				_NextRun( Run* &run, Line *line, eDirection dir, GReal& lineWidth, sLONG i, bool isTab = false);
					void				_NextRun( Run* &run, Line *line, eDirection dir, VFont* font, VColor textColor, VColor backColor, GReal& lineWidth, sLONG i, bool isTab = false);
					void				_NextRun( Run* &run, Line *line, eDirection dir, VFont* &font, VTextStyle* style, GReal& lineWidth, sLONG i);
					...
					@remarks
						if mixed, fDirections stores direction for sub-ranges

						(determined with ICU)
					*/
					eDirection			fDirection;
					Directions			fDirections;
			...
			void				_EndLocalTransform( VGraphicContext *inGC, const VAffineTransform& inCTM, bool inDraw = false);
...
static		GReal				_RoundMetrics( VGraphicContext *inGC, GReal inValue);

			bool				fLayoutIsValid;
			
			bool				fShouldEnableCache;
			bool				fShouldUseNativePlatformForParagraphLayout;
	...
	mutable VTreeTextStyle*		fDefaultStyle;
			...
			*/
			...
			/** platform-independent paragraph layout
			@remarks
				used only if fShouldUseNativePlatformForParagraphLayout == false
			*/
			VParagraphLayout*	fParagraphLayout;

			/** text box layout used by Quartz2D or GDIPlus/GDI (Windows legacy impl) 
			@remarks
				used only if fShouldUseNativePlatformForParagraphLayout == true
			*/
