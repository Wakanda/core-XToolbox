...
*/
VDocParagraphLayout::VDocParagraphLayout(const VDocParagraph *inPara, bool inShouldEnableCache, VDocClassStyleManager *ioCSMgr, const VTextStyle *inDefaultCharStyle):VDocBaseLayout()
{
	_Init();
	...
	if (!inPara)
	{
		...
		if (ioCSMgr)
		{
			//we need to apply default style here if any is defined in cs manager (because ResetParagraphStyle() cannot apply it as for now this node has no class style manager)
			VDocClassStyle *defaultStyle = ioCSMgr->RetainClassStyle( kDOC_NODE_TYPE_PARAGRAPH, CVSTR(""));
			if (defaultStyle)
				InitPropsFrom( static_cast<const VDocNode *>(defaultStyle), true, false, ioCSMgr);
		}
		if (inDefaultCharStyle)
			ApplyStyle( inDefaultCharStyle);
		return;
	}

	InitPropsFrom( static_cast<const VDocNode *>(inPara), false, true, ioCSMgr);
...
void VDocParagraphLayout::_Init()
{
	...
	fListStyleType					= kDOC_LIST_STYLE_TYPE_NONE;
	fListStringFormatLTR			= NULL;
	fListStringFormatRTL			= NULL;
	fListFontFamily					= NULL;
	fListStyleImage					= NULL;
	fListStyleImageHeight			= 0;
	fListHasStartNumber				= false;
	fListStartNumber				= 1;
	fListNumber						= 1;
...
void VDocParagraphLayout::_UpdateListNumber(eListStyleTypeEvent inListEvent)
{
	//TODO: maybe we should optimize here for larger documents (using a map of list nodes per class for instance)
	//(but it is ok for documents with only a few pages)

	eDocPropListStyleType type = GetListStyleType();
	bool isOrdered = type >= kDOC_LIST_STYLE_TYPE_OL_FIRST && GetListStyleImage().IsEmpty();
	if (!isOrdered)
	{
		fListNumber = 1;
		return;
	...
	switch (inListEvent)
	{
		case kDOC_LSTE_BEFORE_REMOVE_FROM_LIST:
			{
				//update next list items
				VDocBaseLayout *node = _GetNextSibling();
				while (node && node->GetDocType() == kDOC_NODE_TYPE_PARAGRAPH)
				{
					ITextLayout *textLayout = node->QueryTextLayoutInterface();
					if (textLayout && textLayout->GetListStart())
						break;
					bool nodeIsOrdered = textLayout->GetListStyleType() >= kDOC_LIST_STYLE_TYPE_OL_FIRST && textLayout->GetListStyleImage().IsEmpty();

					//we scan all next siblings (because class is used for titles for instance & there might be paragraphs or list items from other class between list items of the same class)

					if (nodeIsOrdered && node->GetClass().EqualToString(fClass))
					{
						VDocParagraphLayout *para = dynamic_cast<VDocParagraphLayout *>(node);
						xbox_assert(para);
						para->fListNumber--;
						para->SetDirty();
					}
					node = node->_GetNextSibling();
				}
			}
			break;
		case kDOC_LSTE_AFTER_ADD_TO_LIST:
		case kDOC_LSTE_AFTER_UPDATE_START_NUMBER:
			{
			sLONG start = 1;
			if (GetListStart( &start))
				fListNumber = start;
			else 
			{
				//update current list item number relatively to previous list items if any
				fListNumber = 1;
				
				VDocBaseLayout *node = NULL;
				if (fParent && fChildIndex > 0)
					node = fParent->fChildren[fChildIndex-1].Get();
				...
				while (node)
				{
					...
					//we scan all prev siblings (because class is used for titles for instance & there might be paragraphs or list items from other class between list items of the same class)

					if (nodeIsOrdered && node->GetClass().EqualToString(fClass))
					{
						VDocParagraphLayout *para = dynamic_cast<VDocParagraphLayout *>(node);
						xbox_assert(para);
						fListNumber = para->fListNumber+1;
						break;
					}
					if (fParent && node->fChildIndex > 0)
						node = fParent->fChildren[node->fChildIndex-1].Get();
					else
						node = NULL;
				}
			}
			...
			//update next list items
			sLONG numberNext = fListNumber+1;
			VDocBaseLayout *node = _GetNextSibling();
			while (node)
			{
				ITextLayout *textLayout = node->QueryTextLayoutInterface();
				...
				//if class is not empty, we scan all next siblings (because class is used for titles for instance & there might be paragraphs but lists between list items of the same class)

				if (nodeIsOrdered && node->GetClass().EqualToString(fClass))
				{
					VDocParagraphLayout *para = dynamic_cast<VDocParagraphLayout *>(node);
					xbox_assert(para);
					para->fListNumber = numberNext++;
					para->SetDirty();
				}
				node = node->_GetNextSibling();
			}
...
void VDocParagraphLayout::SetPropsTo( VDocNode *outNode)
{
	...
	if (IsRTL())
		outNode->SetDirection( kDOC_DIRECTION_RTL);
	else
		outNode->SetDirection( kDOC_DIRECTION_LTR);

	//if (fLineHeight != -1)
	{
		if (fLineHeight < 0)
			outNode->SetLineHeight( (sLONG)-floor((-fLineHeight)*100+0.5)); //percentage
		else
			outNode->SetLineHeight( ICSSUtil::PointToTWIPS( fLineHeight)); //absolute line height
	}

	//if (fTextIndent != 0)
	{
		if (fTextIndent >= 0)
			outNode->SetTextIndent( ICSSUtil::PointToTWIPS( fTextIndent)); 
		else
			outNode->SetTextIndent( -ICSSUtil::PointToTWIPS( -fTextIndent));
	}

	uLONG tabStopOffsetTWIPS = (uLONG)ICSSUtil::PointToTWIPS( fTabStopOffset);
	if (tabStopOffsetTWIPS != (uLONG)outNode->GetDefaultPropValue( kDOC_PROP_TAB_STOP_OFFSET))
		outNode->SetTabStopOffset( tabStopOffsetTWIPS);
	if (fTabStopType != TTST_LEFT)
		outNode->SetTabStopType( (eDocPropTabStopType)fTabStopType);

	outNode->SetNewParaClass( fNewParaClass);

	if (GetListStyleType() != kDOC_LIST_STYLE_TYPE_NONE)
	{
		outNode->SetListStyleType( GetListStyleType());

		if (fListStringFormatLTR && *fListStringFormatLTR != VDocNode::GetDefaultListStringFormatLTR( GetListStyleType()))
			outNode->SetListStringFormatLTR( *fListStringFormatLTR);
		if (fListStringFormatRTL && *fListStringFormatRTL != VDocNode::GetDefaultListStringFormatRTL( GetListStyleType()))
			outNode->SetListStringFormatRTL( *fListStringFormatRTL);
		if (fListFontFamily && *fListFontFamily != VDocNode::GetDefaultPropValue( kDOC_PROP_LIST_FONT_FAMILY).GetString())
			outNode->SetListFontFamily( *fListFontFamily);
		if (fListStyleImage && *fListStyleImage != VDocNode::GetDefaultPropValue( kDOC_PROP_LIST_STYLE_IMAGE).GetString())
			outNode->SetListStyleImage( *fListStyleImage);
		if (GetListStyleImageHeight() && GetListStyleImageHeight() > 0)
			outNode->SetListStyleImageHeight( ICSSUtil::PointToTWIPS( GetListStyleImageHeight()));

		if (outNode->GetType() == kDOC_NODE_TYPE_PARAGRAPH)
		{
			sLONG start = 1;
			if (GetListStart( &start))
				outNode->SetListStart( start);
		}
	}

	if (outNode->GetType() == kDOC_NODE_TYPE_CLASS_STYLE)
	{
		//copy default character styles too
		VTreeTextStyle *styles = RetainDefaultStyle();
		outNode->VTextStyleCopyToCharProps( styles->GetData());
		//text back color is stored only in fStyles as it might not be confused with element back color
		if (fStyles && fStyles->GetData()->GetHasBackColor() && fStyles->GetData()->GetBackGroundColor() != RGBACOLOR_TRANSPARENT)
			outNode->SetTextBackgroundColor( fStyles->GetData()->GetBackGroundColor());
		ReleaseRefCountable(&styles);
	}
}

void VDocParagraphLayout::_InitPropsFrom( const VDocNode *inNode, bool inIgnoreIfInheritedDefault, bool inClassStyleOverrideOnlyUniformCharStyles, VDocClassStyleManager *ioCSMgr, bool inApplyParaProps, bool inApplyCharProps)
{
	if (!ioCSMgr && fDoc)
	...
	{
		const VDocClassStyle *defaultStyle = ioCSMgr ? ioCSMgr->RetainClassStyle(kDOC_NODE_TYPE_PARAGRAPH, CVSTR("")) : NULL;
		if (defaultStyle && static_cast<const VDocNode *>(defaultStyle) != inNode)
		{
			_InitPropsFrom( static_cast<const VDocNode *>(defaultStyle), inIgnoreIfInheritedDefault, (inNode->GetType() != kDOC_NODE_TYPE_CLASS_STYLE) ? false : inClassStyleOverrideOnlyUniformCharStyles, ioCSMgr, inApplyParaProps, inApplyCharProps);
			inIgnoreIfInheritedDefault = true;
		}
		ReleaseRefCountable(&defaultStyle);
	}

	if (!inIgnoreIfInheritedDefault && inApplyCharProps && (inNode->GetType() != kDOC_NODE_TYPE_CLASS_STYLE || !inClassStyleOverrideOnlyUniformCharStyles))
	{
		ReleaseRefCountable(&fStyles);
		ReleaseRefCountable(&fExtStyles);
		ReleaseRefCountable(&fDefaultFont);
	...
	if (inApplyParaProps)
	{
		...
		if (!inIgnoreIfInheritedDefault 
			|| 
			(!inNode->IsInheritedDefault( kDOC_PROP_DIRECTION)))
		{
			if (inNode->GetDirection() == kDOC_DIRECTION_RTL)
				SetRTL( true);
			else
				SetRTL( false);
		}
		...
		if (!inIgnoreIfInheritedDefault 
			|| 
			(!inNode->IsInheritedDefault( kDOC_PROP_LINE_HEIGHT)))  
		{
			GReal lineHeight;
			if (inNode->GetLineHeight() >= 0)
				lineHeight = ICSSUtil::TWIPSToPoint(inNode->GetLineHeight());
			else if (inNode->GetLineHeight() == kDOC_PROP_LINE_HEIGHT_NORMAL)
				lineHeight = -1;
			else
			{
				lineHeight = -inNode->GetLineHeight();
		...
		if (!inIgnoreIfInheritedDefault 
			|| 
			(!inNode->IsInheritedDefault( kDOC_PROP_TEXT_INDENT)))
		{
			if (inNode->GetTextIndent() >= 0)
				SetTextIndent( ICSSUtil::TWIPSToPoint(inNode->GetTextIndent()));
			else
				SetTextIndent( -ICSSUtil::TWIPSToPoint(-inNode->GetTextIndent()));
		}
		...
		if (!inIgnoreIfInheritedDefault 
			|| 
			(!inNode->IsInheritedDefault( kDOC_PROP_TAB_STOP_OFFSET)))
		{
			updateTabStop = true;
			offset = ICSSUtil::TWIPSToPoint(inNode->GetTabStopOffset());
		}
		if (!inIgnoreIfInheritedDefault 
			|| 
			(!inNode->IsInheritedDefault( kDOC_PROP_TAB_STOP_TYPE)))
		{
			updateTabStop = true;
			type = (eTextTabStopType)inNode->GetTabStopType();
		}
		if (updateTabStop)
		...
		if (!inIgnoreIfInheritedDefault 
			|| 
			(!inNode->IsInheritedDefault( kDOC_PROP_LIST_STYLE_TYPE)))
			SetListStyleType( inNode->GetListStyleType());

		if (GetListStyleType() != kDOC_LIST_STYLE_TYPE_NONE)
		{
			if (!inIgnoreIfInheritedDefault 
				|| 
				(!inNode->IsInheritedDefault( kDOC_PROP_LIST_STRING_FORMAT_LTR)))
				SetListStringFormatLTR( inNode->GetListStringFormatLTR());

			if (!inIgnoreIfInheritedDefault 
				|| 
				(!inNode->IsInheritedDefault( kDOC_PROP_LIST_STRING_FORMAT_RTL)))
				SetListStringFormatRTL( inNode->GetListStringFormatRTL());

			if (!inIgnoreIfInheritedDefault 
				|| 
				(!inNode->IsInheritedDefault( kDOC_PROP_LIST_FONT_FAMILY)))
				SetListFontFamily( inNode->GetListFontFamily());

			if (!inIgnoreIfInheritedDefault 
				|| 
				(!inNode->IsInheritedDefault( kDOC_PROP_LIST_STYLE_IMAGE)))
				SetListStyleImage( inNode->GetListStyleImage());

			if (!inIgnoreIfInheritedDefault 
				|| 
				(!inNode->IsInheritedDefault( kDOC_PROP_LIST_STYLE_IMAGE_HEIGHT)))
				SetListStyleImageHeight( ICSSUtil::TWIPSToPoint(inNode->GetListStyleImageHeight()));

			if (!inIgnoreIfInheritedDefault 
				|| 
				(!inNode->IsInheritedDefault( kDOC_PROP_LIST_START)))
			{
				if (inNode->GetListStart() == kDOC_LIST_START_AUTO)
					SetListStart( 1, 0, -1, true);
				else
					SetListStart( inNode->GetListStart());
			}
		}

		if (!inIgnoreIfInheritedDefault 
			|| 
			(!inNode->IsInheritedDefault( kDOC_PROP_NEW_PARA_CLASS)))
			SetNewParaClass( inNode->GetNewParaClass());
	}

	if (inNode->GetType() == kDOC_NODE_TYPE_CLASS_STYLE && inApplyCharProps)
	{
		if (inClassStyleOverrideOnlyUniformCharStyles && fStyles)
		{
			VTextStyle *style = new VTextStyle();  
			inNode->CharPropsCopyToVTextStyle( *style, true, !inIgnoreIfInheritedDefault);
			if (!style->IsUndefined())
			{
				style->SetRange(0, fText.GetLength());
				if (_ApplyStyle( style, false, true))
				{
					SetDirty();
					fSelIsDirty = true;
				}
...
void VDocParagraphLayout::InitPropsFrom( const VDocNode *inNode, bool inIgnoreIfInheritedDefault, bool inClassStyleOverrideOnlyUniformCharStyles, VDocClassStyleManager *ioCSMgr)
{
	_InitPropsFrom( inNode, inIgnoreIfInheritedDefault, inClassStyleOverrideOnlyUniformCharStyles, ioCSMgr, true, true);
}
...
void VDocParagraphLayout::Run::PreComputeMetrics(bool inIgnoreTrailingSpaces)
{
	...
	if (fIsListItem)
	{
		fIsLeadingSpace = true; 
		
		GReal width = 0;
		if (fNodeLayout)
			width = fEndPos.x - fStartPos.x;
		else
		{
			width = RoundMetrics( gc, metrics->GetTextWidth( fLine->fParagraph->fParaLayout->fListItemText, fDirection == kPARA_RTL));
			fEndPos = fStartPos;
			fEndPos.x += width;
		}

		GReal space = RoundMetrics(gc, 4*fLine->fParagraph->fCurDPI/72); //leave 4pt space between list item bullet & text content (TODO: maybe we should bind this value with a css property ?)
		fEndPos.x = fStartPos.x; //it should be set yet to list item indent
		width = width+space;
		fStartPos.x = fEndPos.x-width;	//align to the right (if paragraph direction is ltr) or left (if paragraph direction is rtl) 
										//(remember here that computed offsets here are paragraph direction independent (always > 0): paragraph direction dependent offsets are computed in Run::ComputeMetrics)
...
void VDocParagraphLayout::Run::ComputeMetrics(GReal& inBidiCurDirStartOffset)
{
	...
	if (fDirection == kPARA_LTR) 
	{
		...
		if (IsListItem())
		{
			indent = inBidiCurDirStartOffset;
			inBidiCurDirStartOffset = fStartPos.x;
			if (paraDirection == kPARA_RTL)
				inBidiCurDirStartOffset = -inBidiCurDirStartOffset;
		}
		...
		if (IsListItem())
			inBidiCurDirStartOffset += indent;
	}
	else //run is RTL
	{
		GReal indent = 0;
...
void VDocParagraphLayout::Line::ComputeMetrics()
{
				...
				if (!isTrailingSpaceRun)
				{
					if (addRunsWhitespace)
					{
						if (run->IsTab() || run->IsLeadingSpace())
						{
							...
							if (run->IsListItem())
								startOffset = run->GetEndPos().GetX()+startIndent;
							else
	...
	if (runsWithNodeLayout.size())
	{
		...
		for(;itRunNode != runsWithNodeLayout.end(); itRunNode++, itFontMetrics++)
		{
			...
			IDocBaseLayout *nodeLayout = run->GetNodeLayout();
...
void VDocParagraphLayout::VParagraph::_ApplyTab( GReal& pos, VFont *font)
{
	...
	xbox_assert(pos >= 0);
	sLONG abspos = ICSSUtil::PointToTWIPS(pos + 2*fCurDPI/72);
	sLONG taboffset = ICSSUtil::PointToTWIPS( fParaLayout->GetTabStopOffset()*fCurDPI/72);
	if (!taboffset)
		taboffset = ICSSUtil::PointToTWIPS(ICSSUtil::TWIPSToPoint((uLONG)VDocNode::GetDefaultPropValue(kDOC_PROP_TAB_STOP_OFFSET))*fCurDPI/72);
	xbox_assert(taboffset);
...
void VDocParagraphLayout::VParagraph::_NextRun( Run* &run, Line *line, eDirection dir, sLONG bidiVisualIndex, GReal& fontSize, VFont* &font, VTextStyle* style, GReal& lineWidth, sLONG i)
{
	...
	if (style->IsSpanRef() && testAssert(i < fEnd && style->GetSpanRef()->GetType() == kSpanTextRef_Image && (style->GetSpanRef()->ShowRef() & kSRTT_Value)))
		run->SetNodeLayout( dynamic_cast<IDocBaseLayout *>( style->GetSpanRef()->GetLayoutDelegate()));
}
...
void VDocParagraphLayout::VParagraph::_NextLine( Run* &run, Line* &line, sLONG i)
{
	...
	IDocBaseLayout *runNodeLayout = run->GetNodeLayout();
	...
	if (i < fEnd)
	{
		...
		//(here we use fTextIndentBackup backup as fTextIndent may have been reset by list item & we cannot restore fTextIndent before line metrics have been computed for previous line)

		if (fTextIndentBackup > 0)
			fTextIndentBackup = 0;
		if (fLines.size() == 2 && prevLine->fRuns[0]->IsListItem())
			//lines should be indented according to list item run end position
			fTextIndentBackup += -(prevLine->fRuns[0]->GetEndPos().GetX());
		if (fTextIndentBackup)
		{
			run->SetStartPos( VPoint( -fTextIndentBackup, 0));
			run->SetEndPos( VPoint( -fTextIndentBackup, 0));
		}
	}
	prevLine->ComputeMetrics(); //now compute previous line metrics
	fTextIndent = fTextIndentBackup;  //restore text indent for next line
}
...
void VDocParagraphLayout::VParagraph::ComputeMetrics(bool inUpdateBoundsOnly, bool inUpdateDPIMetricsOnly, const GReal inMaxWidth)
{
						...
						if (style->IsSpanRef())
						{
							...
							if (styleSource->GetSpanRef()->GetType() == kSpanTextRef_Image && (styleSource->GetSpanRef()->ShowRef() & kSRTT_Value))
							{
								...
								if (styleSource->GetSpanRef()->GetLayoutDelegate())
								{
									...
									xbox_assert(imageLayout->fDoc == fParaLayout->fDoc); //should be bound to document yet
									xbox_assert(imageLayout);
									imageLayout->Retain();
									imageLayout->fEmbeddingParent = static_cast<VDocBaseLayout*>(fParaLayout);
									imageLayout->fTextStart = rangeStart;
									if (fParaLayout->fDoc && !imageLayout->fDoc)
									{
										//bind image to owner document
										imageLayout->_OnAttachToDocument( fParaLayout->fDoc);
										if (!imageLayout->GetClass().IsEmpty() && styleSource->GetSpanRef()->GetImage()->GetClass().IsEmpty())
											styleSource->GetSpanRef()->GetImage()->SetClass( imageLayout->GetClass()); //sync VDocImage class with VDocImageLayout class
									}
								}
								else
								{
									//create new image layout & store it in source span ref 
									imageLayout = new VDocImageLayout( styleSource->GetSpanRef()->GetImage(), fParaLayout->fDoc ? fParaLayout->fDoc->fClassStyleManager.Get() : NULL);
									imageLayout->SetClass( styleSource->GetSpanRef()->GetImage()->GetClass());
									imageLayout->Retain();
	...
	if (fLines.empty() || !inUpdateBoundsOnly || (!inUpdateDPIMetricsOnly && fWordWrap && maxWidth != fMaxWidth))
	{
		...
		bool isListItem = fParaLayout->GetListStyleType() != kDOC_LIST_STYLE_TYPE_NONE && fStart == 0; //only for paragraph first line
		if (isListItem)
		{
			//add list item run 

			VString fontListName = fParaLayout->GetListFontFamily();
			VFont *fontList = NULL;
			if (!fontListName.EqualToString( font->GetName()))
				fontList = VFont::RetainFont( fontListName, font->GetFace(), font->GetPixelSize(), 72);
			else
				fontList = RetainRefCountable(font);
			Run *run = new Run(line, 0, fDirection == kPARA_RTL ? bidiVisualIndex+1 : bidiVisualIndex-1, fStart, fStart, fDirection, fontSize, fontList, &(fParaLayout->fDefaultTextColor));
			run->SetIsListItem();
			ReleaseRefCountable(&fontList);

			fListItemIndent = RoundMetrics(gc, fParaLayout->GetTabStopOffset()*fCurDPI/72); //list item indent is equal to first tab offset
			run->SetStartPos( VPoint(fListItemIndent,0));
			run->SetEndPos( run->GetStartPos());

			if (fParaLayout->GetListStyleImage().IsEmpty())
				//get list item text according to list style type, direction & current list item number
				VDocNode::GetListItemValue( fParaLayout->GetListStyleType(), fParaLayout->fListItemText, fParaLayout->fListNumber, fDirection == kPARA_RTL, &(fParaLayout->GetListStringFormatLTR()), &(fParaLayout->GetListStringFormatRTL()));
			else
			{
				//it is a list image

				VDocImageLayout *imageLayout = NULL;
				if (fParaLayout->fListImageLayout.Get() && fParaLayout->fListImageLayout.Get()->GetURL().EqualToString( fParaLayout->GetListStyleImage()))
				{
					//pick cached image
					imageLayout = fParaLayout->fListImageLayout.Get();
					if (fParaLayout->fDoc && !imageLayout->fDoc)
						//bind image to owner document
						imageLayout->_OnAttachToDocument( fParaLayout->fDoc);
				}
				...
				{
					//create new image & cache it
					imageLayout = new VDocImageLayout( fParaLayout->GetListStyleImage());
					fParaLayout->fListImageLayout = imageLayout;
					if (fParaLayout->fDoc)
						//bind image to owner document
						imageLayout->_OnAttachToDocument( fParaLayout->fDoc);
					imageLayout->Release();
				}
				if (fParaLayout->fListStyleImageHeight)
					imageLayout->SetHeight( ICSSUtil::PointToTWIPS( fParaLayout->fListStyleImageHeight));
				else
					imageLayout->SetHeight(0);
				imageLayout->SetDPI( fCurDPI);
				imageLayout->UpdateLayout( gc);
				if (!imageLayout->ImageExists())
					//backfall to list item text if image is not found (disc on default)
					VDocNode::GetListItemValue( fParaLayout->GetListStyleType(), fParaLayout->fListItemText, fParaLayout->fListNumber, fDirection == kPARA_RTL, &(fParaLayout->GetListStringFormatLTR()), &(fParaLayout->GetListStringFormatRTL()));
				else
					run->SetNodeLayout( static_cast<VDocBaseLayout *>(imageLayout));

				imageLayout->fEmbeddingParent = static_cast<VDocBaseLayout*>(fParaLayout);
				imageLayout->fTextStart = fStart;
			}

			line->thisRuns().push_back( run);
			run->PreComputeMetrics(false);

			lineWidth = run->GetEndPos().GetX();
		}

		Run *run = new Run(line, isListItem ? 1 : 0, bidiVisualIndex, fStart, fStart, fDirection, fontSize, font, &(fParaLayout->fDefaultTextColor));
		line->thisRuns().push_back( run);
		...
		if (!lenText)
		{
			...
			if (fTextIndent > 0)
			{
				lineWidth = lineWidth+fTextIndent;
				run->SetStartPos( VPoint( lineWidth, 0));
				run->SetEndPos( run->GetStartPos());
			}
		...
		else
		{
			...
			GReal lineWidthBackup = lineWidth;
	...
	else
	{
		...
		if (inUpdateDPIMetricsOnly)
		{
			...
			for(;itLine != fLines.end(); itLine++)
			{
				...
				{
					Run *run = (*itRun);
					if (run->fIndex == 0) //first logical run
					{
						if (!run->IsListItem()) 
							offset += textIndent;
						else
							offset += fListItemIndent; //if list item run, append list item indent
					}
					...
					if (run->IsListItem() && fTextIndent) //we test again fTextIndent here as it may have been reset by run->PreComputeMetrics for list item
						//if list item run, append now text indent 
						offset += textIndent;
				}

				line->ComputeMetrics();
				
				fTextIndent = fTextIndentBackup;
			}
		}
	}
...
void VDocParagraphLayout::_BeginLocalTransform( VGraphicContext *inGC, VAffineTransform& outCTM, const VPoint& inTopLeft, bool inDraw, const VPoint& inPreDecal)
{
...
void VDocParagraphLayout::Draw( VGraphicContext *inGC, const VPoint& inTopLeft, const GReal inOpacity)
{
	...
	VDocBaseLayout::_BeginLocalTransform( inGC, ctm, inTopLeft, true); //here (0,0) is sticked to text box origin & coordinate space pixel is mapped to fDPI (72 on default so on default 1px = 1pt)
	...
	//clip with padding box
	VRect boundsClip = _GetBackgroundBounds( inGC, kDOC_BACKGROUND_PADDING_BOX);
	boundsClip.Inset(-10000,0); //clip only vertically
	inGC->SaveClip();
	inGC->ClipRect( boundsClip);
	
	VRect boundsClipBackColor = _GetBackgroundBounds( inGC, kDOC_BACKGROUND_CONTENT_BOX);
	boundsClipBackColor.Inset(0,-10000); 
	...
	IDocNodeLayoutUserDelegate *userDelegate = !inGC->IsPrintingContext() && fDoc && fDoc->GetLayoutUserDelegate() && fDoc->GetLayoutUserDelegate()->ShouldHandleLayout( static_cast<VDocBaseLayout*>(this)) ? fDoc->GetLayoutUserDelegate() : NULL;
		...
		for (;itPara != fParagraphs.end(); itPara++)
		{
			...
			for (;itLine != para->GetLines().end(); itLine++)
			{
				...
				for (;itRun != line->fVisualRuns.end(); itRun++)
				{
					...
					if (run->GetEnd() == run->GetStart() && !run->IsListItem())
						continue;
					...
					if (drawCharBackColor)
					{
						if (run->fBackColor.GetAlpha() && !run->fTypoBounds.IsEmpty())
						{
							if (backColor.GetAlpha() && run->fBackColor != backColor)
							{
								...
								boundsBackColor.Intersect(boundsClipBackColor);
					...
					if (drawText)
					{
						...
						if (userDelegate)
						{
							boundsRun = VRect( run->fTypoBounds.GetX(), -run->fLine->fAscent, run->fTypoBounds.GetWidth(), run->fLine->GetLineHeight());
							boundsRun.SetPosBy( linePos.x, linePos.y);
							userDelegate->OnBeforeDrawRun( this, fDoc->fLayoutUserDelegateData, inGC, boundsRun, run->fStart, run->fEnd, run->fDirection == kPARA_RTL);
						}
						...
						if (run->GetNodeLayout())
						{
							...
							//here pos is sticked to x = node layout bounds left & y = baseline (IDocBaseLayout::Draw will adjust y according to ascent as while drawing text)
							...
							GReal opacity = 1.0f;
							if (!run->IsListItem() && (run->fStart >= fSelStart && run->fEnd <= fSelEnd))
							{
								//if embedded node is selected, highlight it (Word-like highlighting)
								if (!(fDoc && fDoc->fLayoutUserDelegate && fDoc->fLayoutUserDelegate->ShouldHandleLayout(dynamic_cast<VDocBaseLayout *>(nodeLayout))))
									opacity = 0.7f;
							}
						...
						if (userDelegate)
							userDelegate->OnAfterDrawRun( this, fDoc->fLayoutUserDelegateData, inGC, boundsRun, run->fStart, run->fEnd, run->fDirection == kPARA_RTL);
					}                                                                                                                                                  
				}
...
void VDocParagraphLayout::GetLayoutBoundsFromRange( VGraphicContext *inGC, VRect& outBounds, const VPoint& inTopLeft, sLONG inStart, sLONG inEnd, const eDocNodeType inChildDocNodeType)
{
	outBounds.SetEmpty();
	if (inChildDocNodeType == kDOC_NODE_TYPE_PARAGRAPH)
		GetLayoutBounds( inGC, outBounds, inTopLeft, true);
	else if (inChildDocNodeType == kDOC_NODE_TYPE_IMAGE) //TODO: will need to manage tables too here
	{
		sLONG start = GetTextLength()+1;
		sLONG end = 0;

		if (fStyles && fStyles->HasSpanRefs())
		{
			VIndex count = fStyles->GetChildCount();
			for (int i = 1; i <= count; i++)
			{
				const VTextStyle *style = fStyles->GetNthChild( i)->GetData();
				sLONG refStart, refEnd;
				style->GetRange( refStart, refEnd);
				if (refEnd <= inStart)
					continue;
				if (refStart > inEnd || (refStart == inEnd && inStart < inEnd))
					break;
		...
		if (start < end)
		{
			std::vector<VRect> bounds;
			GetRunBoundsFromRange( inGC, bounds, inTopLeft, start, end);
			std::vector<VRect>::const_iterator itBounds = bounds.begin();
			for(;itBounds != bounds.end(); itBounds++)
			{
				if (outBounds.IsEmpty())
					outBounds = *itBounds;
				else
					outBounds.Union( *itBounds);
			}
		}
	}
}
...
/** set class */
void VDocParagraphLayout::SetClass( const VString& inClass)
{
	if (fClass.EqualToStringRaw( inClass))
		return;

	if (GetListStyleType() >= kDOC_LIST_STYLE_TYPE_OL_FIRST)
	{
		_UpdateListNumber( kDOC_LSTE_BEFORE_REMOVE_FROM_LIST);
		VDocBaseLayout::SetClass( inClass);
...
bool VDocParagraphLayout::ApplyClassStyle( const VDocClassStyle *inClassStyle, bool inResetStyles, bool inReplaceClass, sLONG inStart, sLONG inEnd, eDocNodeType inTargetDocType)
{
	if (GetDocType() != inTargetDocType)
	{
		if (inTargetDocType == kDOC_NODE_TYPE_IMAGE) //TODO: will need to manage tables here too
		{
			//recurse down on embedded image span references
			if (fStyles && fStyles->HasSpanRefs())
			{
				bool result = false;
				...
				for (int i = 1; i <= count; i++)
				{
					...
					if (style->IsSpanRef() && style->GetSpanRef()->GetType() == kSpanTextRef_Image && style->GetSpanRef()->GetLayoutDelegate())
					{
						IDocBaseLayout *layout = dynamic_cast<IDocBaseLayout *>(style->GetSpanRef()->GetLayoutDelegate());
						xbox_assert(layout);
						bool done = layout->ApplyClassStyle( inClassStyle, inResetStyles, inReplaceClass, 0, -1, inTargetDocType);
						if (done)
						{
							//sync now VDocImage with VDocImageLayout
							VDocBaseLayout *layoutBase = dynamic_cast<VDocBaseLayout *>(layout);
							xbox_assert(layoutBase);
							style->GetSpanRef()->GetImage()->ClearAllProps();
							layoutBase->SetPropsTo( static_cast<VDocNode *>(style->GetSpanRef()->GetImage()));
						}
						result = done || result;
					}
				}
...
void VDocParagraphLayout::ResetParagraphStyle(bool inResetParagraphPropsOnly, sLONG /*inStart*/, sLONG /*inEnd*/)
{
	...
	_ResetStyles(true, !inResetParagraphPropsOnly);
...
void VDocParagraphLayout::_ReplaceText( const VDocParagraph *inPara)
{
	...
	if (fStyles)
		fStyles->ExpandAtPosBy(0, fText.GetLength());

	_UpdateTextInfoEx(true);
}

void VDocParagraphLayout::_ApplyParagraphProps( const VDocParagraph *inPara, bool inIgnoreIfInheritedDefault)
{
	InitPropsFrom( static_cast<const VDocNode *>(inPara), inIgnoreIfInheritedDefault);
}

void VDocParagraphLayout::_ResetStyles( bool inResetParagraphProps, bool inResetCharStyles)
{
	VDocClassStyle *defaultStyle = fDoc ? fDoc->GetClassStyleManager()->RetainClassStyle( kDOC_NODE_TYPE_PARAGRAPH, CVSTR("")) : NULL;
	if (!defaultStyle)
		defaultStyle = new VDocClassStyle();
	_InitPropsFrom( static_cast<const VDocNode *>(defaultStyle), false, false, NULL, inResetParagraphProps, inResetCharStyles);
...
void VDocParagraphLayout::_ClearListItem()
{
	fListStyleType = kDOC_LIST_STYLE_TYPE_NONE;
	if (fListStringFormatLTR)
	{
		delete fListStringFormatLTR;
		fListStringFormatLTR = NULL;
	}
	if (fListStringFormatRTL)
	{
		delete fListStringFormatRTL;
		fListStringFormatRTL = NULL;
	}
	if (fListFontFamily)
	{
		delete fListFontFamily;
		fListFontFamily = NULL;
	}
	if (fListStyleImage)
	{
		delete fListStyleImage;
		fListStyleImage = NULL;
	}
	fListStyleImageHeight = 0;
	fListHasStartNumber = false;
...
/** set list style type */
void VDocParagraphLayout::SetListStyleType( eDocPropListStyleType inType, sLONG /*inStart*/, sLONG /*inEnd*/)
{
	if (fListStyleType == inType)
		return;
	if (inType == kDOC_LIST_STYLE_TYPE_NONE)
	{
		_UpdateListNumber( kDOC_LSTE_BEFORE_REMOVE_FROM_LIST);
		_ClearListItem();
	}
	...
	{
		if (fListStyleType != kDOC_LIST_STYLE_TYPE_NONE)
			_UpdateListNumber( kDOC_LSTE_BEFORE_REMOVE_FROM_LIST);

		fListStyleType = inType;
...
void VDocParagraphLayout::SetListStringFormatLTR( const VString& inStringFormat, sLONG /*inStart*/, sLONG /*inEnd*/)
{
	if (fListStringFormatLTR && fListStringFormatLTR->EqualToString( inStringFormat, true))
		return;

	if (!fListStringFormatLTR)
		fListStringFormatLTR = new VString( inStringFormat);
	else
		fListStringFormatLTR->FromString( inStringFormat);

	SetDirty();
}

/** get list string format LTR */
const VString& VDocParagraphLayout::GetListStringFormatLTR( sLONG /*inStart*/, sLONG /*inEnd*/) const
{
	if (fListStringFormatLTR)
		return *fListStringFormatLTR;
	return VDocNode::GetDefaultListStringFormatLTR( fListStyleType);
}
		

/** set list string format RTL */
void VDocParagraphLayout::SetListStringFormatRTL( const VString& inStringFormat, sLONG /*inStart*/, sLONG /*inEnd*/)
{
	if (fListStringFormatRTL && fListStringFormatRTL->EqualToString( inStringFormat, true))
		return;

	if (!fListStringFormatRTL)
		fListStringFormatRTL = new VString( inStringFormat);
	else
		fListStringFormatRTL->FromString( inStringFormat);

	SetDirty();
}

/** get list string format RTL */
const VString& VDocParagraphLayout::GetListStringFormatRTL( sLONG /*inStart*/, sLONG /*inEnd*/) const
{
	if (fListStringFormatRTL)
		return *fListStringFormatRTL;
	return VDocNode::GetDefaultListStringFormatRTL( fListStyleType);
}

/** set list font family */
void VDocParagraphLayout::SetListFontFamily( const VString& inFontFamily, sLONG /*inStart*/, sLONG /*inEnd*/)
{
	if (fListFontFamily && fListFontFamily->EqualToString( inFontFamily, true))
		return;

	if (!fListFontFamily)
		fListFontFamily = new VString( inFontFamily);
	else
		fListFontFamily->FromString( inFontFamily);

	SetDirty();
}

/** get list font family */
const VString& VDocParagraphLayout::GetListFontFamily( sLONG /*inStart*/, sLONG /*inEnd*/) const
{
	if (fListFontFamily)
		return *fListFontFamily;

	return VDocNode::GetDefaultListFontFamily( fListStyleType);
}


/** set list image url */
void VDocParagraphLayout::SetListStyleImage( const VString& inURL, sLONG /*inStart*/, sLONG /*inEnd*/)
{
	if (fListStyleImage && fListStyleImage->EqualToString( inURL, true))
		return;
	else if (!fListStyleImage && inURL.IsEmpty())
		return;

	if (fListImageLayout.Get())
	{
		//clear cached image layout
		if (fDoc)
			fListImageLayout->_OnDetachFromDocument( fDoc);
		fListImageLayout.Clear();
	}
	...
	if (inURL.IsEmpty())
	{
		...
		if (!fListStyleImage)
			fListStyleImage = new VString( inURL);
		else
			fListStyleImage->FromString( inURL);

	_UpdateListNumber( kDOC_LSTE_AFTER_ADD_TO_LIST);

	if (!inURL.IsEmpty() && GetListStyleType() == kDOC_LIST_STYLE_TYPE_NONE)
...
/** get list image url */
const VString& VDocParagraphLayout::GetListStyleImage( sLONG /*inStart*/, sLONG /*inEnd*/) const
{
	if (fListStyleImage)
		return *fListStyleImage;
	return VDocNode::GetDefaultPropValue( kDOC_PROP_LIST_STYLE_IMAGE).GetString();
}



/** set list image height in point (0 for auto) */
void VDocParagraphLayout::SetListStyleImageHeight( GReal inHeight, sLONG /*inStart*/, sLONG /*inEnd*/)
{
	if (fListStyleImageHeight == inHeight)
		return;
	fListStyleImageHeight = inHeight;
	SetDirty();
}

/** set or clear list start number */
void VDocParagraphLayout::SetListStart( sLONG inListStartNumber, sLONG /*inStart*/, sLONG /*inEnd*/, bool inClear)
{
	if (inClear)
	{
		if (!fListHasStartNumber)
			return;
		fListHasStartNumber = false;
		fListStartNumber = 1;
	...
	{
		if (fListHasStartNumber && fListStartNumber == inListStartNumber)
			return;
		fListHasStartNumber = true;
		fListStartNumber = inListStartNumber;
...
bool VDocParagraphLayout::_CheckLayerBounds(const VPoint& inTopLeft, VRect& outLayerBounds)
{
	...
	if (!fLayerViewRect.IsEmpty() && !fIsPrinting)
	{
...
void VDocParagraphLayout::SetText( const VString& inText, VTreeTextStyle *inStyles, bool inCopyStyles)
{
	...
	fTextLength = fPrevTextLength >= 0 ? (uLONG)fPrevTextLength : previousTextLength;
	fPrevTextLength = -1;
	_UpdateTextInfoEx(true);
}
									...
									void *inUserData, VTreeTextStyle::fnBeforeReplace inBeforeReplace, VTreeTextStyle::fnAfterReplace inAfterReplace)
{
	...
	fPrevTextLength = (sLONG)fText.GetLength();
	...
	else
	{
												...
												inPreserveSpanTextRef, NULL, NULL, NULL))
		{
			...
			if (fStyles && !inStyles && inText.GetLength() == 1 && inText[0] == 13 && inStart+1 < fText.GetLength() && fText[inStart+1] == 13)
			{
				//ensure new line inherits style from previous line if we insert a CR at the end of a line
				VTreeTextStyle *stylesCR = NULL;
				stylesCR = RetainStyles(inStart, inStart+1);
				stylesCR->Translate(inStart+1);
				if (stylesCR)
					ApplyStyles( stylesCR);
				ReleaseRefCountable(&stylesCR);
			}
		}
	}
...
void VDocParagraphLayout::_AttachEmbeddedImages(sLONG inStart, sLONG inEnd)
{
	_NormalizeRange( inStart, inEnd);
	if (fStyles && fDoc && fStyles->HasSpanRefs())
	{
		while (inStart < inEnd)
		{
			const VTextStyle *style = fStyles->GetStyleRefAtRange( inStart, inEnd);
			if (!style)
				break;
			...
			if (style->IsSpanRef() && style->GetSpanRef()->GetType() == kSpanTextRef_Image && style->GetSpanRef()->GetLayoutDelegate())
			{
				...
				VDocBaseLayout* layout = dynamic_cast<VDocBaseLayout *>(dynamic_cast<IDocBaseLayout*>(style->GetSpanRef()->GetLayoutDelegate()));
				if (testAssert(layout))
				{
					xbox_assert( layout->fDoc == NULL); 

					//bind to this document 
					layout->_OnAttachToDocument( fDoc); 

					if (!layout->GetClass().IsEmpty() && style->GetSpanRef()->GetImage()->GetClass().IsEmpty())
						style->GetSpanRef()->GetImage()->SetClass( layout->GetClass()); //sync VDocImage class with VDocImageLayout class
...
void VDocParagraphLayout::_DetachEmbeddedImages(sLONG inStart, sLONG inEnd)
{
	if (!fDoc)
		return;
	...
	if (fListImageLayout.Get())
		fListImageLayout.Get()->_OnDetachFromDocument( fDoc);
	...
	if (fStyles && fStyles->HasSpanRefs())
	{
		while (inStart < inEnd)
		{
			...
			if (style->IsSpanRef() && style->GetSpanRef()->GetType() == kSpanTextRef_Image && style->GetSpanRef()->GetLayoutDelegate())
			{
				...
				if (testAssert(layout))
				{
					xbox_assert( layout->fDoc == fDoc); 

					//unbind from document 
					layout->_OnDetachFromDocument( fDoc); 
				}
			}
			sLONG start, end;
			style->GetRange( start, end);
...
void VDocParagraphLayout::_OnAttachToDocument(VDocTextLayout *inDocLayout)
{
	VDocBaseLayout::_OnAttachToDocument( inDocLayout);
	_AttachEmbeddedImages();
}

void VDocParagraphLayout::_OnDetachFromDocument(VDocTextLayout *inDocLayout)
{
	_DetachEmbeddedImages();
...
void VDocParagraphLayout::_AfterReplace(void *inUserData, sLONG inStartReplaced, sLONG inEndReplaced)
{
	...
	layout->fPrevTextLength = -1;

	if (layout->fStyles && inEndReplaced-inStartReplaced == 1 && inStartReplaced+1 < layout->fText.GetLength() && layout->fText[inStartReplaced] == 13 && layout->fText[inStartReplaced+1] == 13)
	{
		//ensure new line inherits style from previous line if we insert a CR at the end of a line
		VTreeTextStyle *stylesCR = NULL;
		stylesCR = layout->RetainStyles( inStartReplaced, inStartReplaced+1);
		stylesCR->Translate(inStartReplaced+1);
		if (stylesCR)
			layout->ApplyStyles( stylesCR);
		ReleaseRefCountable(&stylesCR);
	}
...
bool  VDocParagraphLayout::_ApplyStyle(const VTextStyle *inStyle, bool inIgnoreIfEmpty, bool inOverrideOnlyUniformCharStyles)
{
	if (!inStyle)
		return false;
	sLONG start, end;
	...
	{
		if (inOverrideOnlyUniformCharStyles)
			return false;
	...
	xbox_assert(!inOverrideOnlyUniformCharStyles || !inStyle->IsSpanRef());
	...
	if (!inStyle->IsUndefined())
	{
		if (fStyles)
		{
			if (inOverrideOnlyUniformCharStyles)
			{
				//here we update only uniform text background color - as uniform font name, size, style & fore color has been applied above  
				//(uniform text back color is stored in fStyles to distinguish from paragraph back color)
				if (inStyle->GetHasBackColor())
				{
					needUpdate = true;
					fStyles->GetData()->SetHasBackColor(true);
					fStyles->GetData()->SetBackGroundColor( inStyle->GetBackGroundColor());
				}
			}
			else
			{
...
void VDocParagraphLayout::_UpdateTextInfo(bool /*inUpdateTextLength*/, bool /*inApplyOnlyDeltaTextLength*/, sLONG /*inDeltaTextLength*/)
{
	VDocBaseLayout::_UpdateTextInfo(false); //update fTextStart
...
bool VDocParagraphLayout::UnMergeParagraphs( sLONG inStart, sLONG inEnd)
{
	...
	VDocParagraph *para = new VDocParagraph();
	SetPropsTo( para);
	para->RemoveProp( kDOC_PROP_LIST_START); //list start number should not be inherited by new paragraphs

	VTreeTextStyle *stylesDefault = RetainDefaultStyle();
	...
	VDocClassStyle *classStyle = NULL;
	if (!para->GetNewParaClass().IsEmpty())
	{
		//use styles from the specifed class for new paragraph
		classStyle = fDoc->fClassStyleManager->RetainClassStyle( kDOC_NODE_TYPE_PARAGRAPH, para->GetNewParaClass());
		xbox_assert(classStyle);
	}
	for (;itRange != ranges.end(); itRange++)
	{
		//init paragraph with default character styles & properties
		VDocParagraphLayout *paraLayout = NULL;
		if (classStyle)
		{
			paraLayout = new VDocParagraphLayout( NULL, fShouldEnableCache, fDoc ? fDoc->fClassStyleManager.Get() : NULL);
			paraLayout->ApplyClassStyle( classStyle, false, true);
		}
		else
		{
			paraLayout = new VDocParagraphLayout(para, fShouldEnableCache, fDoc ? fDoc->fClassStyleManager.Get() : NULL, stylesDefault->GetData());
			paraLayout->SetClass( para->GetClass());
		}
		if (!paraLayout->GetClass().IsEmpty() && !paraLayout->GetNewParaClass().IsEmpty())
			if (paraLayout->GetClass().EqualToString( paraLayout->GetNewParaClass()))
				paraLayout->SetNewParaClass(CVSTR(""));

		//set text & character styles - but default
		VString text;
		if (itRange->second > itRange->first)
			fText.GetSubString(itRange->first+1, itRange->second-itRange->first, text);
		
		if (classStyle)
			paraLayout->SetText( text, NULL);
		else
		{
			VTreeTextStyle *styles = RetainStyles( itRange->first, itRange->second, true, false, false);
			paraLayout->SetText( text, styles, true);
			ReleaseRefCountable(&styles);
			if (fExtStyles)
			{
	...
	ReleaseRefCountable(&classStyle);
...
VDocBaseLayout* VDocParagraphLayout::RetainLayoutFromCharPos( sLONG inPos, const eDocNodeType inDocNodeType)
{
	if (inDocNodeType == kDOC_NODE_TYPE_PARAGRAPH)
		return static_cast<VDocBaseLayout *>(RetainRefCountable(this));

	if (inDocNodeType == kDOC_NODE_TYPE_IMAGE)
	{
		const VTextStyle *styleRef = GetStyleRefAtPos( inPos);
		if (styleRef && styleRef->GetSpanRef()->GetType() == kSpanTextRef_Image)
		{
			IDocSpanTextRefLayout *layout = styleRef->GetSpanRef()->GetLayoutDelegate();
			if (layout)
			{
				VDocBaseLayout *layoutBase = dynamic_cast<VDocBaseLayout *>(dynamic_cast<IDocBaseLayout*>(layout));
				return RetainRefCountable(layoutBase);
			}
		}
	}
	return NULL;
