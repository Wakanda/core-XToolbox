...
*/
VDocBorderLayout::VDocBorderLayout( const sDocBorder *inTop, const sDocBorder *inRight, const sDocBorder *inBottom, const sDocBorder *inLeft):VObject()
{
	if (inTop)
		fBorders.fTop = *inTop;
	else
...
VDocBorderLayout::VDocBorderLayout(const VDocNode *inNode):VObject()
{
	...
	fBorders.fTop.fStyle = (eDocPropBorderStyle)(border.top >= 0 ? border.top : kDOC_BORDER_STYLE_NONE);
	fBorders.fRight.fStyle = (eDocPropBorderStyle)(border.right >= 0 ? border.right : kDOC_BORDER_STYLE_NONE);
	fBorders.fBottom.fStyle = (eDocPropBorderStyle)(border.bottom >= 0 ? border.bottom : kDOC_BORDER_STYLE_NONE);
	fBorders.fLeft.fStyle = (eDocPropBorderStyle)(border.left >= 0 ? border.left : kDOC_BORDER_STYLE_NONE);

	border = inNode->GetBorderWidth();
	fBorders.fTop.fWidth = ICSSUtil::TWIPSToPoint(border.top);
	if (fBorders.fTop.fWidth < 0)
		fBorders.fTop.fWidth = 0;
	fBorders.fRight.fWidth = ICSSUtil::TWIPSToPoint(border.right);
	if (fBorders.fRight.fWidth < 0)
		fBorders.fRight.fWidth = 0;
	fBorders.fBottom.fWidth = ICSSUtil::TWIPSToPoint(border.bottom);
	if (fBorders.fBottom.fWidth < 0)
		fBorders.fBottom.fWidth = 0;
	fBorders.fLeft.fWidth = ICSSUtil::TWIPSToPoint(border.left);
	if (fBorders.fLeft.fWidth < 0)
		fBorders.fLeft.fWidth = 0;

	border = inNode->GetBorderColor();
	RGBAColor color = (RGBAColor)border.top;
	if (color == RGBACOLOR_UNDEFINED)
		color = RGBACOLOR_TRANSPARENT;
	fBorders.fTop.fColor.FromRGBAColor( color);
...
{
	if ((fBorders.fLeft.fStyle > kDOC_BORDER_STYLE_HIDDEN && fBorders.fLeft.fWidth > 0) 
		||
		(fBorders.fTop.fStyle > kDOC_BORDER_STYLE_HIDDEN && fBorders.fTop.fWidth > 0) 
		||
		(fBorders.fRight.fStyle > kDOC_BORDER_STYLE_HIDDEN && fBorders.fRight.fWidth > 0)  
		||
		(fBorders.fBottom.fStyle > kDOC_BORDER_STYLE_HIDDEN && fBorders.fBottom.fWidth > 0) )
	{
		...
		if (!(VDocProperty(width) == outNode->GetDefaultPropValue( kDOC_PROP_BORDER_WIDTH)))
			outNode->SetBorderWidth( width);
		if (!(VDocProperty(color) == outNode->GetDefaultPropValue( kDOC_PROP_BORDER_COLOR)))
...
bool sDocBorder::operator == (const sDocBorder& inBorder)
{
...
void VDocBorderLayout::_CheckClip()
{
	...
	if (fBorders.fLeft.fStyle <= kDOC_BORDER_STYLE_HIDDEN)
		fBorders.fLeft.fWidth = 0;
	else if (fBorders.fLeft.fWidth == 0)
		fBorders.fLeft.fWidth = 2;

	if (fBorders.fRight.fStyle <= kDOC_BORDER_STYLE_HIDDEN)
		fBorders.fRight.fWidth = 0;
	else if (fBorders.fRight.fWidth == 0)
		fBorders.fRight.fWidth = 2;

	if (fBorders.fTop.fStyle <= kDOC_BORDER_STYLE_HIDDEN)
		fBorders.fTop.fWidth = 0;
	else if (fBorders.fTop.fWidth == 0)
		fBorders.fTop.fWidth = 2;

	if (fBorders.fBottom.fStyle <= kDOC_BORDER_STYLE_HIDDEN)
		fBorders.fBottom.fWidth = 0;
	else if (fBorders.fBottom.fWidth == 0)
		fBorders.fBottom.fWidth = 2;
...
*/
void VDocBorderLayout::GetClipRect(VGraphicContext *inGC, VRect& outClipRect, const VPoint& inTopLeft, bool inOutside) const
{
	GReal widthLeft = VDocBaseLayout::RoundMetrics(NULL,fBorders.fLeft.fWidth*fDPI/72); 
	GReal widthRight = VDocBaseLayout::RoundMetrics(NULL,fBorders.fRight.fWidth*fDPI/72); 
	GReal widthTop = VDocBaseLayout::RoundMetrics(NULL,fBorders.fTop.fWidth*fDPI/72); 
	GReal widthBottom = VDocBaseLayout::RoundMetrics(NULL,fBorders.fBottom.fWidth*fDPI/72); 
	...
	else
	{
		...
		outClipRect.SetPosBy(widthLeft*0.5,widthTop*0.5);
		outClipRect.SetSizeBy( -(widthLeft+widthRight)*0.5, -(widthTop+widthBottom)*0.5);
		if (outClipRect.GetWidth() < 0)
			outClipRect.SetWidth(0);
		if (outClipRect.GetHeight() < 0)
			outClipRect.SetHeight(0);
	}
}
...
void VDocBorderLayout::Draw( VGraphicContext *inGC, const VPoint& inTopLeft)
{
	...
	if (!fNeedClip)
	{
		if (!fBorders.fLeft.fColor.GetAlpha() || fBorders.fLeft.fStyle <= kDOC_BORDER_STYLE_HIDDEN)
			return;
	}
	...
	if (!fNeedClip)
	{
		...
		GReal width = VDocBaseLayout::RoundMetrics(NULL,fBorders.fLeft.fWidth*fDPI/72); 

		_SetLinePatternFromBorderStyle( inGC, fBorders.fLeft.fStyle);

		inGC->SetLineColor( curColor = fBorders.fLeft.fColor);

		if (fBorders.fLeft.fStyle == kDOC_BORDER_STYLE_DOUBLE)
		{
	...
	else
	{
		...
		if (fBorders.fRight.fColor.GetAlpha() && fBorders.fRight.fStyle > kDOC_BORDER_STYLE_HIDDEN)
		{
			if (fClipping.fRight)
			{
				inGC->SaveClip();
			...
			GReal width = VDocBaseLayout::RoundMetrics(NULL,fBorders.fRight.fWidth*fDPI/72);

			_SetLinePatternFromBorderStyle( inGC, fBorders.fRight.fStyle);

			if (fBorders.fRight.fStyle == kDOC_BORDER_STYLE_DOUBLE)
			{
				GReal strokeWidthDouble = width/3;
		...
		if (fBorders.fTop.fColor.GetAlpha() && fBorders.fTop.fStyle > kDOC_BORDER_STYLE_HIDDEN)
		{
			if (fClipping.fTop)
			{
				inGC->SaveClip();
			...
			GReal width = VDocBaseLayout::RoundMetrics(NULL,fBorders.fTop.fWidth*fDPI/72);

			_SetLinePatternFromBorderStyle( inGC, fBorders.fTop.fStyle);

			if (fBorders.fTop.fStyle == kDOC_BORDER_STYLE_DOUBLE)
			{
				GReal strokeWidthDouble = width/3;
		...
		if (fBorders.fBottom.fColor.GetAlpha() && fBorders.fBottom.fStyle > kDOC_BORDER_STYLE_HIDDEN)
		{
			if (fClipping.fBottom)
			{
				inGC->SaveClip();
			...
			GReal width = VDocBaseLayout::RoundMetrics(NULL,fBorders.fBottom.fWidth*fDPI/72);

			_SetLinePatternFromBorderStyle( inGC, fBorders.fBottom.fStyle);

			if (fBorders.fBottom.fStyle == kDOC_BORDER_STYLE_DOUBLE)
			{
				GReal strokeWidthDouble = width/3;
