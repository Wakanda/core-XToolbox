...
CFMutableDictionaryRef CTHelper::CreateStringAttributes( const VFont *inFont, const VColor& inColorFill, TextRenderingMode inMode, TextLayoutMode inLayoutMode, GReal inCharKerning)
{
	...
	if (CTHelper::UseCocoaAttributes())
	{
		...
		if (inLayoutMode & (TLM_VERTICAL|TLM_PARAGRAPH_STYLE_MASK))
		{
			...
			if (inLayoutMode & TLM_LEFT_TO_RIGHT)
			{
...
void *CTHelper::CreateNSColor( const VColor& inColor)
{
	...
	NSColor *color = [NSColor colorWithDeviceRed:(CGFloat)(inColor.GetRed()/255.0f) green:(CGFloat)(inColor.GetGreen()/255.0f) blue:(CGFloat)(inColor.GetBlue()/255.0f) alpha:(CGFloat)(inColor.GetAlpha()/255.0f)];
	xbox_assert(color);
	
	[color retain];
#if VERSIONDEBUG
	sLONG refcount = [color retainCount];
#endif
	[innerPool release]; //here autoreleased color is released 
#if VERSIONDEBUG
	refcount = [color retainCount];
	xbox_assert(refcount == 1);
#endif
...
{
#if CT_USE_COCOA	
	NSColor *color = (NSColor *)inNSColor;
...
void XMacFontMetrics::GetMetrics( CGFloat& outAscent, CGFloat& outDescent, CGFloat& outInternalLeading, CGFloat& outExternalLeading) const
{
	const VFont* font = fMetrics->GetFont();
    outAscent = font->GetImpl().GetAscent();
    outDescent = font->GetImpl().GetDescent();
...
GReal XMacTextFactory::GetTextWidth( const VString& inString) 
{
...
void XMacTextFactory::MeasureText( const VString& inString, std::vector<GReal>& outOffsets)
{
	CTLineRef line = CreateLine(inString);
