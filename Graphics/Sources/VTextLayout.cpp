...
VDocParagraph *VTextLayout::CreateParagraph(sLONG inStart, sLONG inEnd)
{
	...
	if (fHAlign != AL_DEFAULT)
		para->SetTextAlign( (eStyleJust)VDocBaseLayout::JustFromAlignStyle(fHAlign));
	if (fVAlign != AL_DEFAULT)
		para->SetVerticalAlign( (eStyleJust)VDocBaseLayout::JustFromAlignStyle(fVAlign));
...
{
	if (inTargetDocType != kDOC_NODE_TYPE_PARAGRAPH)
		return false;
	...
	inClassStyle->CharPropsCopyToVTextStyle( *style, true, false);
	...
	_ApplyParagraphProps( inClassStyle, !inResetStyles);
...
void VTextLayout::_ApplyParagraphProps( const VDocClassStyle *inParaStyle, bool inIgnoreIfInheritedDefault)
{
	...
	if (!inIgnoreIfInheritedDefault 
		|| 
		(!inParaStyle->IsInheritedDefault( kDOC_PROP_MARGIN)))
		SetMargins( inParaStyle->GetMargin());   

	if (!inIgnoreIfInheritedDefault 
		|| 
		(!inParaStyle->IsInheritedDefault( kDOC_PROP_TEXT_ALIGN)))
	{
		eStyleJust just = (eStyleJust)inParaStyle->GetTextAlign();
		SetTextAlign( VDocBaseLayout::JustToAlignStyle( just));
	}

	if (!inIgnoreIfInheritedDefault 
		|| 
		(!inParaStyle->IsInheritedDefault( kDOC_PROP_VERTICAL_ALIGN)))
	{
		eStyleJust just = (eStyleJust)inParaStyle->GetVerticalAlign();
		SetVerticalAlign( VDocBaseLayout::JustToAlignStyle( just));
	}

	if (!inIgnoreIfInheritedDefault 
		|| 
		(!inParaStyle->IsInheritedDefault( kDOC_PROP_BACKGROUND_COLOR)))
	{
		RGBAColor color = inParaStyle->GetBackgroundColor();
		VColor xcolor;
...
void VTextLayout::_ApplyParagraphProps( const VDocParagraph *inPara, bool inIgnoreIfInheritedDefault)
{
	//remark: as measure are in TWIPS, we convert from TWIPS to point & then from point to VTextLayout DPI 

	if (!inIgnoreIfInheritedDefault 
		|| 
		(!inPara->IsInheritedDefault( kDOC_PROP_MARGIN)))
		SetMargins( inPara->GetMargin());

	if (!inIgnoreIfInheritedDefault 
		|| 
		(!inPara->IsInheritedDefault( kDOC_PROP_TEXT_ALIGN)))
	{
	...
	if (!inIgnoreIfInheritedDefault 
		|| 
		(!inPara->IsInheritedDefault( kDOC_PROP_VERTICAL_ALIGN)))
	{
	...
	if (!inIgnoreIfInheritedDefault 
		|| 
		(!inPara->IsInheritedDefault( kDOC_PROP_BACKGROUND_COLOR)))
	{
...
void VTextLayout::SetMargins( const sDocPropRect& inMargins, bool /*inCalcAllMargin*/) //inCalcAllMargin is managed only by new layout
{
	...
	fMargin.left	= VDocBaseLayout::RoundMetrics(NULL,ICSSUtil::TWIPSToPoint( fMarginTWIPS.left) * fDPI/72);
	fMargin.right	= VDocBaseLayout::RoundMetrics(NULL,ICSSUtil::TWIPSToPoint( fMarginTWIPS.right) * fDPI/72);
	fMargin.top		= VDocBaseLayout::RoundMetrics(NULL,ICSSUtil::TWIPSToPoint( fMarginTWIPS.top) * fDPI/72);
	fMargin.bottom	= VDocBaseLayout::RoundMetrics(NULL,ICSSUtil::TWIPSToPoint( fMarginTWIPS.bottom) * fDPI/72);
...
VTreeTextStyle *VTextLayout::_RetainDefaultTreeTextStyle() const
{
	if (!fDefaultStyle)
	{
		...
		if (fHAlign != AL_DEFAULT)
			...
			style->SetJustification( VDocBaseLayout::JustFromAlignStyle( fHAlign));
...
bool  VTextLayout::_ApplyStyle(const VTextStyle *inStyle, bool inIgnoreIfEmpty)
{
	...
	if (start <= 0 && end >= fTextLength)
	{
		...
		if (inStyle->GetJustification() != JST_Notset)
		{
			...
			AlignStyle align = VDocBaseLayout::JustToAlignStyle( inStyle->GetJustification());
			if (align != fHAlign)
			{
				fHAlign = align;
				needUpdate = true;
