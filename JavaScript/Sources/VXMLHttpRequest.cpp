...
XBOX::VError XMLHttpRequest::SetSystemProxy(bool inUseSystemProxy)
{
...
XBOX::VError XMLHttpRequest::SetClientCertificate(const XBOX::VString& inKeyPath, const XBOX::VString& inCertPath)
{
	if(fReadyState!=UNSENT && fReadyState!=OPENED)
        return VE_XHRQ_INVALID_STATE_ERROR; 
...
XBOX::VError XMLHttpRequest::SetTimeout(XBOX::VLong inConnectMs, XBOX::VLong inTotalMs)
{
    if(fReadyState!=OPENED)
        return VE_XHRQ_INVALID_STATE_ERROR;
...
XBOX::VError XMLHttpRequest::SendBinary(const void* data, sLONG datalen, XBOX::VError* outImplErr)
{
	if(fReadyState!=OPENED)
		return VE_XHRQ_INVALID_STATE_ERROR;
...
XBOX::VError XMLHttpRequest::SetResponseType(const XBOX::VString& inValue)
{
...
XBOX::VError XMLHttpRequest::GetResponseType(XBOX::VString* outValue)
{
	XBOX::VError verr=XBOX::VE_OK;
...
XBOX::VError XMLHttpRequest::GetResponse(XBOX::VJSValue& outValue) const
{
    if(fReadyState!=LOADING && fReadyState!=DONE)
        return VE_XHRQ_INVALID_STATE_ERROR;
	...
	case BLOB :
		{
			...
			XBOX::VJSObject newInstance = XBOX::VJSBlob::CreateInstance(outValue.GetContext(), slice);
			outValue.SetValueRef(newInstance);
...
XBOX::VError XMLHttpRequest::GetResponseBinary(BinaryPtr& outPtr, XBOX::VSize& outLen) const
{
	if(fReadyState!=LOADING && fReadyState!=DONE)
		return VE_XHRQ_INVALID_STATE_ERROR;
...
XBOX::VString XMLHttpRequest::GetContentType(const XBOX::VString inDefaultType) const
{
    const char* c_header=fCWImpl->fReq->GetResponseHeader("Content-Type");
...
XBOX::VJSObject	VJSXMLHttpRequest::MakeConstructor (const XBOX::VJSContext& inContext)
{
...
void VJSXMLHttpRequest::_Construct(XBOX::VJSParms_callAsConstructor& ioParms)
{
    ...
    ioParms.ReturnConstructedObject(VJSXMLHttpRequest::CreateInstance(ioParms.GetContext(), xhr));
...
void VJSXMLHttpRequest::_SetClientCertificate(XBOX::VJSParms_callStaticFunction& ioParms, XMLHttpRequest* inXhr)
{
...
void VJSXMLHttpRequest::_SetTimeout(XBOX::VJSParms_callStaticFunction& ioParms, XMLHttpRequest* inXhr)
{
    sLONG pConnect=-1;
    sLONG pTotal=-1;
...
bool VJSXMLHttpRequest::_OnReadyStateChange(XBOX::VJSParms_setProperty& ioParms, XMLHttpRequest* inXhr)
{
    ...
    ioParms.GetPropertyValue().GetObject(pHandler);
...
bool VJSXMLHttpRequest::_SetResponseType(XBOX::VJSParms_setProperty& ioParms, XMLHttpRequest* inXhr)
{
...
void VJSXMLHttpRequest::_GetResponseType(XBOX::VJSParms_getProperty& ioParms, XMLHttpRequest* inXhr)
{
	if(inXhr)
    {
		XBOX::VString type;
...
void VJSXMLHttpRequest::_GetResponse(XBOX::VJSParms_getProperty& ioParms, XMLHttpRequest* inXhr)
{
