...
,fValue(NULL)
{
	...
	Context::Scope context_scope(fContext,V4DContext::GetPersistentContext(fContext));
	if ( inOther.fValue == NULL)
	{
		fValue = NULL;
	}
	else
...
VJSValue::VJSValue( JS4D::ContextRef inContext, JS4D::HandleValueRef inValue) : fContext(inContext), fValue(NULL)//,fPersValue(NULL)
{
	//DebugMsg("VJSValue::VJSValue6( CALLED!!!!\n");
	HandleScope scope(fContext);
	Context::Scope context_scope(fContext,V4DContext::GetPersistentContext(fContext));
	if ( inValue->val_ == NULL)
	{
		fValue = NULL;
	}
	else
	{
		internal::Object** p = reinterpret_cast<internal::Object**>(inValue->val_);
  		fValue = reinterpret_cast<v8::Value*>(V8::GlobalizeReference(
							reinterpret_cast<internal::Isolate*>(fContext),
                             p));
	}
...
{
    if (fValue->IsExternal())
    {
        External* extVal = External::Cast(fValue);
        return extVal->Value();
    }
    return NULL;
}
...
void VJSValue::GetObject( VJSObject& outObject, JS4D::ExceptionRef *outException) const
{
	...
	for(;;)
		DebugMsg("%s  NOT IMPL\n",__PRETTY_FUNCTION__ );
	outObject.SetObjectRef( ((fValue != NULL) && fValue->IsObject()) ? fValue : NULL);
...
{
	JS4D::VFileToObject( fContext, inFile, fValue, VJSException::GetExceptionRefIfNotNull(outException));
}

void VJSValue::SetFilePathAsFileOrFolder( const XBOX::VFilePath& inPath, VJSException *outException)
{
	JS4D::VFilePathToObjectAsFileOrFolder(	fContext,
											inPath,
											fValue,
...
{ 
	...
	Handle<String>		result = v8::String::New(bufferTmp.GetCPointer());
...
{ 
	JS4D::BoolToValue( fContext, inValue, fValue); 
}
...
{ 
	JS4D::VDurationToValue( fContext, inDuration,fValue); 
}
...
{
	VJSValue	tmpVal = JS4D::VValueToValue(fContext, inValue, VJSException::GetExceptionRefIfNotNull(outException), simpleDate);
	fValue = tmpVal.fValue;
}

void VJSValue::SetJSONValue( const VJSONValue& inValue, VJSException *outException)
{
	VJSValue	tmpVal = JS4D::VJSONValueToValue(fContext, inValue, VJSException::GetExceptionRefIfNotNull(outException));
	fValue = tmpVal.fValue;
}
...
{
#if USE_V8_ENGINE
	JS4D::VFolderToObject( fContext, inFolder, fValue, VJSException::GetExceptionRefIfNotNull(outException));
#else
	fValue = JS4D::ObjectToValue( 
						fContext,
...
const VJSObject& VJSObject::operator=( const VJSObject& inOther)
{ 
...
void VJSObject::MakeFile( VFile *inFile, JS4D::ExceptionRef *outException)
{
	...
	{
#if USE_V8_ENGINE
		JS4D::VFileToObject( fContext, inFile, fObject, outException);
#else
...
{ 
	...
	{
#if USE_V8_ENGINE
		JS4D::VFolderToObject( fContext, inFolder, fObject, outException);
#else
...
{ 
	...
	{
#if USE_V8_ENGINE
		JS4D::VFilePathToObjectAsFileOrFolder( fContext, inPath, fObject, outException);
#else
...
bool VJSObject::HasProperty( const VString& inPropertyName) const
{
    ...
    VStringConvertBuffer	bufferTmp(inPropertyName,VTC_UTF_8);
    Local<Object>			obj = fObject->ToObject();
	ok = obj->Has(v8::String::NewSymbol(bufferTmp.GetCPointer()));
#else
...
void VJSObject::SetProperty( const XBOX::VString& inPropertyName, const VJSArray& inArray, JS4D::PropertyAttributes inAttributes, VJSException* outException) const
{
	...
	JS4D::ObjectRef		arrayRef = inArray.GetObjectRef();
	if (testAssert(arrayRef->IsArray()))
	{
		VJSObject	tmpObj(fContext, arrayRef);
...
VJSObject VJSObject::GetPropertyAsObject( const VString& inPropertyName, JS4D::ExceptionRef *outException) const
{
	...
	if (fObject && fObject->IsObject())
	{
		...
		VJSObject				result(fContext,propVal.val_);
		return result;
	}
	else
	{
...
{
#if USE_V8_ENGINE
	return JS4D::ObjectIsFunction(fContext, fObject);
#else
...
void VJSObject::MakeEmpty()
{
	...
	if (emptyObj.val_)
	{
		internal::Object** p = reinterpret_cast<internal::Object**>(emptyObj.val_);
  		fObject = reinterpret_cast<v8::Value*>(V8::GlobalizeReference(
							reinterpret_cast<internal::Isolate*>(v8::Isolate::GetCurrent()),
                             p));
	}
...
void VJSObject::MakeCallback( JS4D::ObjectCallAsFunctionCallback inCallbackFunction)
{
	...
	//Handle<Function>	fn = v8Ctx->CreateFunction(inCallbackFunction);
	Handle<Function>	fn = v8::Function::New(v8::Isolate::GetCurrent(),inCallbackFunction);
	//Local<Function>		locFunc = handle_scope.Close(fn);
	//Handle<v8::Boolean>	fn = v8::Boolean::New(true);
...
void VJSObject::MakeConstructor( JS4D::ClassRef inClassRef, JS4D::ObjectCallAsConstructorCallback inConstructor)
{
	...
	contextRef = fContext;
    V4DContext* v8Ctx = (V4DContext*)(contextRef->GetData());
	Handle<Function>	fn = v8Ctx->CreateFunctionTemplateForClass(inClassRef);
	if (fn.val_)
	{
		internal::Object** p = reinterpret_cast<internal::Object**>(fn.val_);
  		fObject = reinterpret_cast<v8::Value*>(V8::GlobalizeReference(
...
, fCount(0)
{
	...
	if (testAssert(fObject.fObject->IsObject()))
	{
		...
		for (uint32_t idx = 0; idx < fCount; idx++)
		{
			...
			if (testAssert(tmpVal.val_->IsString()))
			{
...
{
#if USE_V8_ENGINE
	DebugMsg("VJSPropertyIterator::GetPropertyAsObject called\n");
...
#if USE_V8_ENGINE
	DebugMsg("VJSPropertyIterator::_GetProperty called\n");
	xbox_assert(fObject.fContext == ioValue.fContext);
	HandleScope scope(ioValue.fContext);
	Context::Scope context_scope(fObject.fContext, V4DContext::GetPersistentContext(fObject.fContext));
