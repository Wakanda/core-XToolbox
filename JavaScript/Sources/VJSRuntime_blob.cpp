...
VJSBlobData::VJSBlobData( void *inDataPtr, XBOX::VSize inDataSize)
: fDataPtr( inDataPtr)
...
VError VJSBlobValue::CopyTo( XBOX::VFile *inDestination, bool inOverwrite)
{
	VError err = VE_OK;
	...
	if (slice != NULL)
	{
		...
		else
		{
			if (inDestination->Exists())
				err = vThrowError( VE_FILE_ALREADY_EXISTS);
			else
...
/*
	static
*/
VJSBlobValue_Slice*	VJSBlobValue_Slice::Create( const void* inBlobData, XBOX::VSize inBlobSize, const XBOX::VString& inContentType)
{
	if (inBlobSize == 0)
		return new VJSBlobValue_Slice( NULL, 0, 0, inContentType);
...
XBOX::VJSObject VJSBlob::NewInstance (XBOX::VJSContext inContext, VSize inLength, void *inData, const XBOX::VString &inContentType)
{
	XBOX::VJSObject		createdObject(inContext);
	VJSBlobValue_Slice	*blob;
	
	if ((blob = VJSBlobValue_Slice::Create(inData, inLength, inContentType)) == NULL) {
...
void VJSBlob::Construct( VJSParms_construct &ioParms)
{
	...
	if (value != NULL)
	{
		ioParms.ReturnConstructedObject( CreateInstance( ioParms.GetContextRef(), value));
	}
	else
	{
