...
XBOX::VJSObject	VJSTextStream::MakeConstructor (XBOX::VJSContext inContext)
{
	return VJSTextStream::CreateInstance(inContext, NULL);
}

void VJSTextStream::do_TextStream (VJSParms_callStaticFunction &ioParms)
{
	ioParms.ReturnValue(VJSTextStream::_Construct(ioParms));
}

void VJSTextStream::_CallAsFunction (VJSParms_callAsFunction &ioParms)
{
	ioParms.ReturnValue(VJSTextStream::_Construct(ioParms));
}

void VJSTextStream::_CallAsConstructor (VJSParms_callAsConstructor &ioParms) 
{
	ioParms.ReturnConstructedObject(VJSTextStream::_Construct(ioParms));
}
...
bool VJSTextStream::_HasInstance (const	VJSParms_hasInstance &inParms) 
{
	...
	xbox_assert(inParms.GetContext().GetGlobalObject().GetPropertyAsObject("TextStream") == inParms.GetPossibleContructor());
...
XBOX::VJSObject VJSTextStream::_Construct (VJSParms_withArguments &ioParms)
{
	...
	if (stream != NULL && err == XBOX::VE_OK) {
		...
		if ((textStreamState = new VJSTextStreamState()) == NULL) {
			...
			constructedObject = VJSTextStream::CreateInstance(ioParms.GetContext(), textStreamState);
