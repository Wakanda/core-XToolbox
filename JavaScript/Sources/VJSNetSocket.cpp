...
VJSNetSocketObject::VJSNetSocketObject (bool inIsSynchronous, sLONG inType, bool inAllowHalfOpen)
{
...
void VJSNetSocketClass::Construct (XBOX::VJSParms_callAsConstructor &ioParms)
{
	...
	nullObject.SetNull();
	ioParms.ReturnConstructedObject(nullObject);
	allowHalfOpen = false;
	...
	if ((socketObject = new VJSNetSocketObject(false, ipType, allowHalfOpen)) != NULL) {
...
void VJSNetSocketClass::Connect (XBOX::VJSParms_withArguments &ioParms, XBOX::VJSObject	&inSocket, bool inIsSSL)
{
...

	{
		...
		socketObject->fEndPoint = VTCPEndPointFactory::CreateClientConnection(host, port, false, true, 1000, VJSNetSocketObject::sSelectIOPool, error);
...
void VJSNetSocketClass::_Initialize (const XBOX::VJSParms_initialize &inParms, VJSNetSocketObject *inSocket)
{
	...
	inSocket->SetupEventEmitter(inParms);
...
void VJSNetSocketClass::_setEncoding (XBOX::VJSParms_callStaticFunction &ioParms, VJSNetSocketObject *inSocket)
{
	...
	if (!ioParms.CountParams() || ioParms.IsNullParam(1)) 
...
void VJSNetSocketSyncClass::Construct (XBOX::VJSParms_callAsConstructor &ioParms)
{
	bool	allowHalfOpen;
	sLONG	ipType;	
	...
	if ((socketObject = new VJSNetSocketObject(true, ipType, allowHalfOpen)) != NULL) {
