...
XBOX::VJSObject VJSGlobalObject::RequireNative (const XBOX::VJSContext &inContext, const XBOX::VString &inClassName)
{
	...
	// if you add a class here,
	// you also need to update //depot/Wakanda/main/walib/WAF/Core/Native/requireFunction.js
	
	if (inClassName.EqualToUSASCIICString( "net"))
	{
		VJSNetClass::Class();
		object = VJSNetClass::CreateInstance(inContext, NULL);
	}
	else if (inClassName.EqualToUSASCIICString( "tls"))
	{
		VJSTLSClass::Class();
		object = VJSTLSClass::CreateInstance(inContext, NULL);
	}
	else if (inClassName.EqualToUSASCIICString( "events"))
	{
		VJSEventEmitterClass::Class();
		object = VJSEventEmitterClass::MakeModuleObject(inContext);
	}
	else if (inClassName.EqualToUSASCIICString( "crypto"))
	{
		VJSCryptoClass::Class();
		object = VJSCryptoClass::CreateInstance( inContext, NULL);
	}
	else
	{
		object.SetUndefined();
...
DebugMsg("VJSGlobalClass::Initialize called\n");
...
#if  USE_V8_ENGINE
	XBOX::VJSObject		globalObject((VJSContext*)NULL);
	globalObject.fIsGlobalObject = true;
#else
	...
	if ((eventEmitter = new VJSEventEmitter()) != NULL) {
		...
		globalObject.SetProperty("process", VJSProcess::CreateInstance(context, eventEmitter), JS4D::PropertyAttributeDontDelete | JS4D::PropertyAttributeReadOnly);
...
void VJSGlobalClass::AddConstructorObjectStaticValue( const char *inClassName, JS4D::ObjectCallAsConstructorCallback inCallback)
{
	...
	{
#if  USE_V8_ENGINE
DebugMsg("VJSGlobalClass::AddConstructorObjectStaticValue called for '%s'\n",inClassName);
#else
		AddStaticValue( inClassName, JS4D::GetConstructorObject, NULL, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete | JS4D::PropertyAttributeDontEnum);
		#endif
	}
}
...
void VJSGlobalClass::CreateGlobalClasses()
{
	...
	xbox_assert(false);
#else
	AddStaticValue( "FileError", VJSFileErrorConstructorClass::js_getProperty<VJSFileErrorConstructorClass::_getConstructorObject>, NULL, kJSPropertyAttributeReadOnly | kJSPropertyAttributeDontDelete | kJSPropertyAttributeDontEnum);
#endif
}
	...
	else
	{
		...
		if (inParms.IsStringParam(1) && inParms.GetStringParam( 1, pathname))
		{
			...
			if (inParms.CountParams() >= 2) {
				...
				if (inParms.IsBooleanParam(2)) {

					pathType = 0;
					inParms.GetBoolParam(2, &refresh);

				} else if (inParms.IsStringParam(2)) {
			...
			else if (pathType == 2) 
			{
				...
				if ((folder = new VFolder(parentPath)) == NULL) {
