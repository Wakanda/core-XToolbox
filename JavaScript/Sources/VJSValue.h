...
public:
	...
	explicit					VJSValue( const VJSContext& inContext, const XBOX::VJSException& inException ) : fContext(inContext), fValue((JS4D::ValueRef)inException.GetValue()) {}
			...
			const VJSValue&		operator=( const VJSValue& inOther ){ fValue = inOther.fValue; fContext=inOther.fContext;/*xbox_assert( fContext == inOther.fContext);*/return *this;}
			// operators needed because VJSStructuredClone uses VJSValues as map indexes
			bool				operator==(const VJSValue& inOther ) const { return fValue == inOther.fValue; }
			bool				operator<(const VJSValue& inOther) const { return fValue < inOther.fValue; }
			bool				operator<=(const VJSValue& inOther) const { return fValue <= inOther.fValue; }
			...
			bool				IsFunction() const;
			bool				IsInstanceOf( const XBOX::VString& inConstructorName, VJSException* outException = NULL) const;
			bool				IsInstanceOfBoolean() const		{ return IsInstanceOf("Boolean", (JS4D::ExceptionRef *)NULL); }
			bool				IsInstanceOfNumber() const		{ return IsInstanceOf("Number", (JS4D::ExceptionRef *)NULL); }
			bool				IsInstanceOfString() const		{ return IsInstanceOf("String", (JS4D::ExceptionRef *)NULL); }
			bool				IsArray() const					{ return IsInstanceOf("Array", (JS4D::ExceptionRef *)NULL); }
			bool				IsInstanceOfDate() const		{ return IsInstanceOf("Date", (JS4D::ExceptionRef *)NULL); }
			bool				IsInstanceOfRegExp() const		{ return IsInstanceOf("RegExp", (JS4D::ExceptionRef *)NULL); }
#endif
...
#if USE_V8_ENGINE
			template<class Type>
			void				SetNumber( const Type& inValue, VJSException *outException = NULL){	JS4D::NumberToValue(fContext,inValue,fValue); }
...
friend class VJSParms_withPropertyCallbackReturnValue;
...
public:
	...
	bool	IsGlobalObject() const { return fIsGlobalObject; }
								~VJSObject();
#endif
	...
	explicit					VJSObject( const VJSContext& inContext);
			...
			const VJSObject&	operator=( const VJSObject& inOther);
			...
			typename NATIVE_CLASS::PrivateDataType*	GetPrivateData() const
			{
				...
				return static_cast<typename NATIVE_CLASS::PrivateDataType*>(
#if USE_V8_ENGINE
										JS4D::ValueIsObjectOfClass( fContext, fObject, classRef )
