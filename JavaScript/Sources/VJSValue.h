...
public:
	...
	explicit					VJSValue( JS4D::ContextRef inContext, JS4D::ValueRef inValue) : fContext( inContext), fValue( inValue)	{}
			...
			void				SetFile( XBOX::VFile *inFile, JS4D::ExceptionRef *outException = NULL)							{ fValue = JS4D::ObjectToValue( fContext, JS4D::VFileToObject( fContext, inFile, outException));}
			void				SetFolder( XBOX::VFolder *inFolder, JS4D::ExceptionRef *outException = NULL)					{ fValue = JS4D::ObjectToValue( fContext, JS4D::VFolderToObject( fContext, inFolder, outException));}
			void				SetFilePathAsFileOrFolder( const XBOX::VFilePath& inPath, JS4D::ExceptionRef *outException = NULL)		{ fValue = JS4D::ObjectToValue( fContext, JS4D::VFilePathToObjectAsFileOrFolder( fContext, inPath, outException)); }
...
public:
	...
	explicit					VJSObject( JS4D::ContextRef inContext, JS4D::ObjectRef inObject) : fContext( inContext), fObject( inObject)	{}
			...
			void				MakeFile( VFile *inFile, JS4D::ExceptionRef *outException = NULL)									{ if (testAssert( inFile != NULL)) fObject = JS4D::VFileToObject( fContext, inFile, outException); }
			void				MakeFolder( VFolder *inFolder, JS4D::ExceptionRef *outException = NULL)								{ if (testAssert( inFolder != NULL)) fObject = JS4D::VFolderToObject( fContext, inFolder, outException); }
			void				MakeFilePathAsFileOrFolder( const VFilePath& inPath, JS4D::ExceptionRef *outException = NULL)		{ if (testAssert( inPath.IsValid())) fObject = JS4D::VFilePathToObjectAsFileOrFolder( fContext, inPath, outException); }
