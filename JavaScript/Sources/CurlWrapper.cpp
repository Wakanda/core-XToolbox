...
namespace CW
{
    ...
    size_t ResponseHeaders::WriteFunction(void *ptr, size_t size, size_t nmemb, void* thisPtr)
    {
        ...
        me->fHeaders.insert(make_pair(std::string(key_start, key_past-key_start),
                                      std::string(value_start, value_past-value_start)));
    ...
    bool ResponseHeaders::GetHeader(const XBOX::VString& inKey, XBOX::VString* outValue) const
    {
        ...
        std::string inKeyTmp=StdStringFromAsciiVString(inKey);

        cit=fHeaders.find(inKeyTmp);

        if(cit!=fHeaders.end())
        {
            *outValue=VStringFromAsciiStdString(cit->second);
    ...
    void ResponseHeaders::GetAllHeaders(XBOX::VString* outValue) const
    {
        ...
        *outValue=VStringFromAsciiStdString(oss.str());
    }
    ...
    void RequestHeaders::SetHeader(const XBOX::VString& inKey, const XBOX::VString& inValue)
    {
        ...
        std::string inKeyTmp=StdStringFromAsciiVString(inKey);
        std::string inValueTmp=StdStringFromAsciiVString(inValue);
    ...
    {
        char url[CW_URL_SIZE];
        fUrl.ToCString(url, sizeof(url));

        curl_easy_setopt(fHandle, CURLOPT_URL, url);
