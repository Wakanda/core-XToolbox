...
bool JS4D::DateObjectToVTime( ContextRef inContext, ObjectRef inObject, VTime& outTime, ExceptionRef *outException, bool simpleDate)
{
	...
	if (simpleDate)
	{
		...
		JSValueRef day =  JSObjectCallAsFunction( inContext, JSValueToObject( inContext, getDate, outException), inObject, 0, NULL, outException);
		...
		case kJSTypeObject:
			{
				...
				else
				{
					...
					else
					{
						...
						{
							JSValueRef jsonValue = JSObjectCallAsFunction( inContext, toJSONObject, jsObject, 0, NULL, &exception);
