...
XBOX::VString			VJSModuleState::sRequireFunctionURL = CVSTR("");

void VJSModuleState::SetRequireFunctionURL(const XBOX::VString& inURL)
{
	sRequireFunctionURL = inURL;
}

VJSModuleState::VJSModuleState (XBOX::VJSContext inContext)
{
	XBOX::VError	error;
	...
	error = XBOX::VE_OK;
	if (sRequireFunctionURL.GetLength() > 0)
	{
		urlString = sRequireFunctionURL;
	}
	else
	{
	...
	if (!sIsCodeLoaded) {
		...
		if (!sIsCodeLoaded) {
			...
			error = LoadScript(urlString, &url, &sRequireFunctionScript);
	...
	if (error != XBOX::VE_OK) {
	...
	} else if (!inContext.CheckScriptSyntax(sRequireFunctionScript, &url, NULL)) {
		...
		if (!inContext.EvaluateScript(sRequireFunctionScript, &url, &result, &exception)) {
			...
			fRequireFunctionRef = (XBOX::JS4D::ObjectRef) result.GetValueRef();
			JS4D::ProtectValue(inContext, fRequireFunctionRef);
