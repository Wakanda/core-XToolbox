...
VJSTimerContext::~VJSTimerContext () 
{
	// All timer events should have been cleared, so fTimers should be empty.
		
	xbox_assert(fTimers.empty());
}

VJSTimer *VJSTimerContext::LookUpTimer (sLONG inID)
{
	std::map<sLONG, VJSTimer *>::iterator	it;
...
{
	xbox_assert(inFunctionObject.GetObjectRef() != NULL);
	xbox_assert(inInterval >= 0 || inInterval == VJSTimer::kTimeOut);

	fTimerContext = NULL;
	fID = -1;
	fInterval = inInterval;
	fFunctionObject = inFunctionObject.GetObjectRef();	
}
