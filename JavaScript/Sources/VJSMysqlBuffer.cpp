        ...
        if(inCount <= (fRowsCount[fCoordIter] - fRowPosition)) {
            for(uLONG i = 0; i < inCount ; i ++) {
                ...
                for (uLONG t = 0; t < inFieldCount; t ++) {
                    ...
                    sLONG lcb = ReadLCB();
                    if(lcb >= 0) {
                        VString value = ReadLCBString(lcb);
...
void VJSMysqlBufferObject::FormatPropertyValue(VJSContext &inContext, VJSObject &row, VString &inValue, VString &inTitle, uLONG inType) {
    ...
    switch(inType) {
    case eFIELD_TYPE_TINY:
    case eFIELD_TYPE_SHORT:
    case eFIELD_TYPE_LONG:
    case eFIELD_TYPE_LONGLONG:
    case eFIELD_TYPE_INT24:
    case eFIELD_TYPE_YEAR:
    case eFIELD_TYPE_BIT:
        row.SetProperty(inTitle, inValue.GetLong8());
        break;

    case eFIELD_TYPE_FLOAT:
    case eFIELD_TYPE_DOUBLE:
        row.SetProperty(inTitle, inValue.GetReal());
        break;

    case eFIELD_TYPE_NULL:
        row.SetNullProperty(inTitle);
        break;

    case eFIELD_TYPE_DATE:
    case eFIELD_TYPE_NEWDATE:
    case eFIELD_TYPE_DATETIME:
        inValue.GetTime(time);
        value.SetTime(time);
        row.SetProperty(inTitle, value, JS4D::PropertyAttributeDontDelete);
        break;

    case eFIELD_TYPE_TINY_BLOB:
    case eFIELD_TYPE_MEDIUM_BLOB:
    case eFIELD_TYPE_LONG_BLOB:
    case eFIELD_TYPE_BLOB:
        length = VJSBufferObject::FromString(inValue, VJSBufferObject::GetEncodingType("binary"), &encodedBuffer);
        if(length >= 0) {
            VJSBufferObject *buffer = new VJSBufferObject(length, encodedBuffer);
            ::free(encodedBuffer);
            type = CVSTR("");
            VJSBlobValue_Slice *blob = VJSBlobValue_Slice::Create( buffer->GetDataPtr(), buffer->GetDataSize(), type);
            if(blob == NULL) {
                row.SetNullProperty(inTitle);
            } else {
                row.SetProperty(inTitle, VJSBlob::NewInstance(inContext, length, buffer, type));
            }
        } else {
            row.SetNullProperty(inTitle);
        }
    ...
    case eFIELD_TYPE_SET:
        inValue.GetSubStrings(",", v);
        for(uLONG i = 0; i < v.size(); ++i) {
            stringArray.PushString(v[i]);
        }
        row.SetProperty(inTitle, stringArray);
        break;

    case eFIELD_TYPE_VARCHAR:
    case eFIELD_TYPE_ENUM:
    case eFIELD_TYPE_VAR_STRING:
    case eFIELD_TYPE_STRING:
    case eFIELD_TYPE_GEOMETRY:
    case eFIELD_TYPE_DECIMAL:
    case eFIELD_TYPE_NEWDECIMAL:
    default:
        row.SetProperty(inTitle, inValue);
        break;
    }
}
...
void VJSMysqlBufferClass:: _fetch( VJSParms_callStaticFunction& ioParms, VJSMysqlBufferObject *inMysqlBuffer) {
    sLONG inCount;
    sLONG fieldCount;
    VJSContext inContext(ioParms.GetContextRef());
