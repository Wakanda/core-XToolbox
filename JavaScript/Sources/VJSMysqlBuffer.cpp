                ...
                if ( lcb >= 0 )
                {
                              ...
                              type == eFIELD_TYPE_BLOB )
                    {
                        if ( lcb >= 0 )
                        {
                            ...
                            row.SetProperty ( title, VJSBlob::NewInstance ( inContext, lcb, buffer, "" ), JS4D::PropertyAttributeDontDelete );
...
void VJSMysqlBufferClass::Construct ( VJSParms_construct &ioParms )
{
    ...
    ioParms.ReturnConstructedObject ( VJSMysqlBufferClass::CreateInstance ( ioParms.GetContext(), inMysqlBuffer ) );
}
