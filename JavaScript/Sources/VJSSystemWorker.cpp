...
VError VJSSystemWorker::InitFileSystems ( )
{
	VError						vError = VE_OK;
	...
	if ( vfsNamespace != NULL )
	{
		...
		vfsNamespace-> RegisterFileSystem ( CVSTR ( ".USR_BIN" ), vfPathUsrBin, eFSO_Private );

		vfPathUsrLocal. FromFullPath ( vstrProgramFiles, FPS_SYSTEM );
		vfsNamespace-> RegisterFileSystem ( CVSTR ( ".USR_LOCAL" ), vfPathUsrLocal, eFSO_Private );
	#else
...
VError VJSSystemWorkerClass::_ReadParametersFS ( XBOX::VJSParms_withArguments& ioParms, XBOX::VString& outCommandLine, XBOX::VString& outRootFolderPath, XBOX::VString& outStdInContent )
{
	...
	if ( bOk && ioParms. CountParams ( ) >= 5 && ioParms. IsStringParam ( 5 ) )
	{
...
#endif
{
	...
	sLONG	*panicTerminationFlag;

	if ((panicTerminationFlag = new sLONG()) == NULL) {
...
#define BUFFER_SIZE	(1 << 14)
	...
	isOk = !VProcessLauncher::ExecuteCommandLine(commandLine, OPTION, stdinBufferPointer, NULL, NULL, NULL, defaultDirectory, NULL, &processLauncher);
	...
	if (isOk) {
        ...
        if ((stderrBytesRead = processLauncher->ReadErrorFromChild(buffer, BUFFER_SIZE)) > 0) {
	...
	if (isOk) {
		...
		for ( ; ; ) {
			...
			if ((stdoutBytesRead = processLauncher->ReadFromChild(buffer, BUFFER_SIZE)) > 0) {
