...
//#error "GH SHOULD NOT PASS HERE on V8!!!!!"
	...
	enum {
		...
		PropertyAttributeReadOnly     = 1<<1,	// PropertyAttribute.ReadOnly,
		PropertyAttributeDontEnum     = 1<<2,	// PropertyAttribute.DontEnum,
		PropertyAttributeDontDelete   = 1<<3	// PropertyAttribute.DontDelete
#else
...
//	typedef void (__cdecl *ObjectCallAsConstructorCallback) (const v8::FunctionCallbackInfo<v8::Value>& args);
	...
	typedef V8ObjectGetPropertyCallback* (*ObjectGetPropertyCallback)();
	class V8ObjectSetPropertyCallback;
	typedef V8ObjectSetPropertyCallback* (*ObjectSetPropertyCallback)();
#else
	...
	static	VJSValue				VValueToValue(ContextRef inContext, const XBOX::VValueSingle& inValue, ExceptionRef *outException, bool simpleDate);
	static	VJSValue				_VJSONValueToValue(ContextRef inContext, const VJSONValue& inValue, JS4D::ExceptionRef *outException, std::map<VJSONObject*, JS4D::ObjectRef>& ioConvertedObjects);

	// converts a JavaScriptCore value into a xbox VJSONValue.
	static	VJSValue				VJSONValueToValue( ContextRef inContext, const XBOX::VJSONValue& inValue, ExceptionRef *outException);
...
#if USE_V8_ENGINE
	static	void					VFileToObject( ContextRef inContext, XBOX::VFile *inFile, v8::Value* ioValue, ExceptionRef *outException);
	static	void					VFolderToObject( ContextRef inContext, XBOX::VFolder *inFolder, v8::Value* ioValue, ExceptionRef *outException);
	static	void					VFilePathToObjectAsFileOrFolder( ContextRef inContext, const XBOX::VFilePath& inPath, v8::Value* ioValue, ExceptionRef *outException);
	static	bool					StringToVString( void* inJSString, XBOX::VString& outString);
#else
	static	ObjectRef				VFileToObject( ContextRef inContext, XBOX::VFile *inFile, ExceptionRef *outException);
	static	ObjectRef				VFolderToObject( ContextRef inContext, XBOX::VFolder *inFolder, ExceptionRef *outException);
	...
	static	bool					StringToLong( XBOX::VString inJSString, sLONG *outValue);
	// produces a js null value if VString is null
	//static	void					VStringToValue( ContextRef inContext, const XBOX::VString& inString, v8::Value* ioValue);
#else
	// returns false if the string doesn't appear to be a valid positive or negative 32bits integer.
...
#if USE_V8_ENGINE
	static	void					BoolToValue( ContextRef inContext, bool inValue, v8::Value* ioPersVal);
#else
	static	ValueRef				BoolToValue( ContextRef inContext, bool inValue);
#endif
	
#if USE_V8_ENGINE
	static	void					DoubleToValue( ContextRef inContext, double inValue, v8::Value* ioPersVal);
#else
...
template<class Type>
#if USE_V8_ENGINE
	static	void					NumberToValue( ContextRef inContext, const Type& inValue, v8::Value* ioPersVal)
				{
					DoubleToValue( inContext, static_cast<double>( inValue), ioPersVal);
				}
#else
...
#if USE_V8_ENGINE
	static	void					VTimeToObject( ContextRef inContext, const XBOX::VTime& inDate, v8::Value* ioPersVal, ExceptionRef *outException, bool simpleDate);
#else
...
#if USE_V8_ENGINE
	static	bool					DateObjectToVTime( ContextRef inContext, v8::Value*, XBOX::VTime& outTime, ExceptionRef *outException, bool simpleDate);
#else
...
#if USE_V8_ENGINE
	static	void					VDurationToValue( ContextRef inContext, const XBOX::VDuration& inDuration,v8::Value* ioPersVal);
#else
...
#if USE_V8_ENGINE
	static	void					MakeUndefined( ContextRef inContext, v8::Value* ioPersVal);
	static	void					MakeNull( ContextRef inContext, v8::Value* ioPersVal);
#else
...
#if USE_V8_ENGINE
	static	bool					ValueIsObjectOfClass( ContextRef inContext, v8::Value* inValue, ClassRef inClassRef);
	...
	*/
	...
	static	void*					GetObjectPrivate( ContextRef inContext, v8::Value* inObject);
	static	void					SetObjectPrivate( ContextRef inContext, v8::Value* inObject, void* inData);
#else
	// Gets an object's private data.
