...
VFile *VJSPath::ResolveFileParam( const VJSContext& inContext, const VString& inJSPathParam)
{
	VFile *file = NULL;
	VURL url;
	if (JS4D::GetURLFromPath( inJSPathParam, url))
	{
		...
		if (runtimeDelegate != NULL)
		{
			...
			ok = fsNameSpace->ParsePathWithFileSystem( urlPath, &file, NULL);
...
VFolder *VJSPath::ResolveFolderParam( const VJSContext& inContext, const VString& inJSPathParam)
{
	...
	if (JS4D::GetURLFromPath( jsPathParam, url))
	{
		...
		if (runtimeDelegate != NULL)
		{
			...
			ok = testAssert(fsNameSpace != NULL) ? fsNameSpace->ParsePathWithFileSystem( urlPath, NULL, &folder) : NULL;
...
bool VJSFolderIterator::parseFolder (VFolder* folder, VJSParms_callStaticFunction& ioParms, VJSObject& objfunc, std::vector<VJSValue>& params, sLONG& counter, VJSObject& thisParam)
{
	...
	XBOX::JS4D::ProtectValue(ioParms.GetContext(), folderObject.GetObjectRef());
	...
	XBOX::JS4D::UnprotectValue(ioParms.GetContext(), folderObject.GetObjectRef());
...
VJSObject VJSFileIterator::_Construct (VJSParms_withArguments &ioParms)
{
	XBOX::VJSObject	constructedObject(ioParms.GetContext());
...
XBOX::VJSObject VJSFileIterator::_ConstructFromW3CFS (VJSParms_withArguments &ioParms)
{
	...
	if (object.IsOfClass(VJSFileEntryClass::Class()) || object.IsOfClass(VJSFileEntrySyncClass::Class()))
	{
		...
		{
			entry->File(ioParms.GetContext(), constructedObject);
		}		
	}
	else
	{
