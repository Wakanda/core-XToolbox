...
XBOX::VJSObject VJSBufferClass::NewInstance (XBOX::VJSContext inContext, VSize inLength, void *inBuffer)
{
	xbox_assert(inLength >= 0);
	xbox_assert(!(inLength > 0 && inBuffer == NULL));
	...
	if ((buffer = new VJSBufferObject(inLength, inBuffer)) == NULL) {
...
XBOX::VJSObject	VJSBufferClass::MakeConstructor (XBOX::VJSContext inContext)
{
	XBOX::VJSObject	bufferConstructor(inContext);
	XBOX::VJSObject	isBufferObject(inContext);
	XBOX::VJSObject	byteLengthObject(inContext);

	bufferConstructor.MakeConstructor(Class(), js_constructor<_Construct>);
	isBufferObject.MakeCallback(XBOX::js_callback<VJSBufferObject, VJSBufferClass::_IsBuffer>);
	byteLengthObject.MakeCallback(XBOX::js_callback<VJSBufferObject, VJSBufferClass::_ByteLength>);

	bufferConstructor.SetProperty("isBuffer", isBufferObject);
	bufferConstructor.SetProperty("byteLength", byteLengthObject);	

	return bufferConstructor;
}

void VJSBufferClass::_Construct (XBOX::VJSParms_callAsConstructor &ioParms)
{
	...
	if (ioParms.IsNumberParam(1)) {
		...
		if (buffer != NULL && buffer->fBuffer != NULL) {
			...
			XBOX::VJSPropertyIterator	i(XBOX::VJSObject(ioParms.GetContext(), arrayObject.GetObjectRef()));
			VSize						j;
	...
	if (buffer == NULL || (buffer->fLength && buffer->fBuffer == NULL)) {
		...
		ioParms.ReturnConstructedObject(VJSBufferClass::CreateInstance(ioParms.GetContextRef(), buffer));
		buffer->Release();

	}
}

void VJSBufferClass::_Initialize (const VJSParms_initialize &inParms, VJSBufferObject *inBuffer)
{
...
void VJSBufferClass::_ByteLength (XBOX::VJSParms_callStaticFunction &ioParms, VJSBufferObject *)
{
	...
	switch (encoding) {
		...
		case VJSBufferObject::eENCODING_ASCII:
		case VJSBufferObject::eENCODING_BINARY: {
		...
		case VJSBufferObject::eENCODING_HEX: {
		...
		case VJSBufferObject::eENCODING_BASE64: 
		default: {
...
void VJSBufferClass::_toBlob(XBOX::VJSParms_callStaticFunction &ioParms, VJSBufferObject *inBuffer)
{
	...
	if (!ioParms.GetStringParam(1, type))
		type = CVSTR( "");					// Default is actually "" if unknown.
...
void VJSBufferClass::_slice (XBOX::VJSParms_callStaticFunction &ioParms, VJSBufferObject *inBuffer)
{
	...
	if ((buffer = new VJSBufferObject(inBuffer, start, end)) == NULL) {
		...
		ioParms.ReturnValue(VJSBufferClass::CreateInstance(ioParms.GetContextRef(), buffer));
		buffer->Release();
