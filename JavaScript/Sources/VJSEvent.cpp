...
//** Hence termination must be processed, otherwise it will never be garbage collected.
			...
			JS4D::UnprotectValue(inContext, proxyObject.GetObjectRef());
...
void VJSW3CFSEvent::Process ( XBOX::VJSContext inContext, VJSWorker *inWorker)
{
	...
	if (errorContext.GetLastError() != VE_OK)
	{
		if (fErrorCallback != NULL)
		{
			...
			VJSFunction callbackObject( VJSObject( inContext, fErrorCallback), inContext);
	...
	else
	{
		...
		{
			VJSFunction callbackObject( VJSObject( inContext, fSuccessCallback), inContext);
...
VJSWebSocketEvent *VJSWebSocketEvent::CreateOpenEvent (XBOX::JS4D::ObjectRef inObjectRef, const XBOX::VString &inProtocol, const XBOX::VString &inExtensions)
{
	...
	webSocketEvent->fObjectRef = inObjectRef;
...
VJSWebSocketEvent *VJSWebSocketEvent::CreateMessageEvent (XBOX::JS4D::ObjectRef inObjectRef, bool inIsText, uBYTE *inData, VSize inSize)
{
	xbox_assert(inData != NULL && inSize > 0);
...
VJSWebSocketEvent *VJSWebSocketEvent::CreateErrorEvent (XBOX::JS4D::ObjectRef inObjectRef)
{
	VJSWebSocketEvent	*webSocketEvent;
...
VJSWebSocketEvent *VJSWebSocketEvent::CreateCloseEvent (XBOX::JS4D::ObjectRef inObjectRef, bool inWasClean, sLONG inCode, const XBOX::VString &inReason)
{
	VJSWebSocketEvent	*webSocketEvent;
...
void VJSWebSocketEvent::Process (XBOX::VJSContext inContext, VJSWorker *inWorker)
{
	...
	XBOX::VJSObject				webSocket(inContext, fObjectRef);
	std::vector<XBOX::VJSValue>	callbackArguments;

	if (fSubType == eTYPE_OPEN) {
				
		webSocket.SetProperty("protocol", fProtocol);
		webSocket.SetProperty("extensions", fExtensions);

	} else if (fSubType == eTYPE_MESSAGE) {

		XBOX::VJSObject	messageObject(inContext);
		XBOX::VJSValue	value(webSocket.GetProperty("binaryType"));
		XBOX::VString	string;
		bool			isString;
		...
		value.SetValueRef(messageObject.GetObjectRef());
		...
		VJSContext	vjsContext(inContext);
		XBOX::VJSValue	value(vjsContext, closeEventObject.GetObjectRef());
	...
	if (webSocket.IsObject()) {
		...
		XBOX::VJSValue	value	= webSocket.GetProperty(callbackName);

		if (value.IsFunction())
		
			webSocket.CallMemberFunction(callbackName, &callbackArguments, &value, NULL);
