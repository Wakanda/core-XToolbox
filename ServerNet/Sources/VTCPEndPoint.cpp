...
VError VTCPEndPoint::DoRead ( void *outBuff, uLONG *ioLen, sLONG inMsTimeout, sLONG* outMsSpent)
{
	...
	if ( fIsSelectIO && !fIsWatching)
		return _Read_SelectIO ( ( char* ) outBuff, ioLen, inMsTimeout );
...
class VTCPEndPoint::ReadNotificationMessage : public VMessage
{

public:

	ReadNotificationMessage(VTCPEndPoint* inEndPoint) : fEndPoint(inEndPoint)
	{
...
protected:
	...
	virtual void DoExecute()
	{
		...
		if(shouldTrace)
		{
			...
			ILoggerBagKeys::message.Set(tBag, CVSTR("VTCPEndPoint::ReadNotificationMessage::DoExecute"));
