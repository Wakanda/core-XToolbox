...
public:
	...
	// It makes no sense to make the frames too small.
	...
	// Parse data read from TCP socket. It is mandatory to call this method until the state is STATE_CLOSED as it handles closing handshake 
	// and ping/pong frames (pings are automatically answered with pongs). If the closing handshake has been initiated by user, then it is
	// up to him or her to check that data sent in close control frame has been mirrored in the response.
	//
	// There are four cases according to the error code returned :  
	//
	// 1) VE_SRVR_WEBSOCKET_HEADER_TOO_SHORT: More data needs to be read, no byte(s) have been consumed. Read more bytes from TCP socket 
	//    then retry.
	// 
	...
	// Send a WebSocket message, fragment it if needed.
	...
	// ContinueMessage() to send the remaining fragments of it, last call will automatically send the FIN flag. 
	//
	...
	XBOX::VError				StartMessage (VSize inTotalLength, bool inIsText, uBYTE inRSVFlags, const uBYTE *inMessageStart, VSize inStartLength);
	XBOX::VError				ContinueMessage (uBYTE inRSVFlags, const uBYTE *inData, VSize inLength);
...
private:
	...
	VSize						fBytesLeft;					// Number of byte(s) left from a StartMessage().
	VWebSocketFrame				fEncodingWebSocketFrame;
	XBOX::VMemoryBuffer<>		fBuffer;					// Send buffer.
...
public:
	...
	static bool			IsValidURL (const XBOX::VURL &inURL); 
	
						VWebSocketConnector (const XBOX::VString &inURL);
						VWebSocketConnector (const XBOX::VURL &inURL);
	virtual				~VWebSocketConnector ();

	XBOX::VError		Connect (const XBOX::VString &inUserAgent);
