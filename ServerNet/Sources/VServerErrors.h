...
// WebSocket errors:	//** TODO: Update resource text file

// Need to read more byte(s) to get complete header.
 
const VError	VE_SRVR_WEBSOCKET_HEADER_TOO_SHORT			= MAKE_VERROR(kSERVER_NET_SIGNATURE, 401);

// The frame header was read correctly, but need to read more byte(s) to get complete payload data.

const VError	VE_SRVR_WEBSOCKET_FRAME_TOO_SHORT			= MAKE_VERROR(kSERVER_NET_SIGNATURE, 402);	

// Frame header is incorrect (length not encoded correctly).

const VError	VE_SRVR_WEBSOCKET_HEADER_INCORRECT			= MAKE_VERROR(kSERVER_NET_SIGNATURE, 403);

// Frames are supposed to be read in "open" or "closing" (for closing handshake) states.

const VError	VE_SRVR_WEBSOCKET_INVALID_STATE_FOR_READ	= MAKE_VERROR(kSERVER_NET_SIGNATURE, 404);

// Operation is possible only in "open" state.

const VError	VE_SRVR_WEBSOCKET_NOT_OPEN_STATE			= MAKE_VERROR(kSERVER_NET_SIGNATURE, 405);

// Closing handshake can be initiated in "open" state and answered in "closing" state.

const VError	VE_SRVR_WEBSOCKET_INVALID_STATE_FOR_CLOSE	= MAKE_VERROR(kSERVER_NET_SIGNATURE, 406);

// Trying to start sending a message while in the middle of another one.

const VError	VE_SRVR_WEBSOCKET_MESSAGE_ALREADY_STARTED	= MAKE_VERROR(kSERVER_NET_SIGNATURE, 407);

// Trying to continue (VWebSocket::ContinueMessage()) when message hasn't been started (VWebSocket::StartMessage()).

const VError	VE_SRVR_WEBSOCKET_MESSAGE_NOT_STARTED		= MAKE_VERROR(kSERVER_NET_SIGNATURE, 408);

// Trying to continue a message with too much data.

const VError	VE_SRVR_WEBSOCKET_TOO_MUCH_DATA				= MAKE_VERROR(kSERVER_NET_SIGNATURE, 409);
