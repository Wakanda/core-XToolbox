...
VError VPrivateKey::XKey::Init(const VString& inPemKey)
{
	...
	XBOX::VStringConvertBuffer convBuf(inPemKey, XBOX::VTC_US_ASCII);
...
VError VPrivateKey::XKey::InitFromCertificate(const VString& inPemCert)
{
	...
	XBOX::VStringConvertBuffer convBuf(inPemCert, XBOX::VTC_US_ASCII);
...
VError VVerifier::XDigestContext::Init()
{
...
VVerifier::VVerifier()
{
	...
	VError verr=fDigestContext->Init();
	...
	if(verr!=VE_OK)
	{
		...
		vThrowError(VE_SSL_FAIL_TO_INIT_VERIFIER);
	}
}
...
#endif	//WITH_CRYPTO_VERIFIER_API
