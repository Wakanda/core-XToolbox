...
// WAK0087124: Spurious out of memory exception fixed out.
		...
		if (NULL != fTCPEndPoint && error == XBOX::VE_OK)
		{
...
void VHTTPClient::GetResponseBodyString (XBOX::VString& outBodyString)
{
	...
	if (fResponseBody.GetDataSize() > 0)
	{
		...
		if (XBOX::MIMETYPE_TEXT == XBOX::HTTPTools::GetMimeTypeKind (contentType))
		{
			...
			XBOX::VTextConverters::Get()->ConvertToVString (fResponseBody.GetDataPtr(), fResponseBody.GetDataSize(), charSet, outBodyString);
		}
	}
}
