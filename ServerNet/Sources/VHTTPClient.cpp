...
{	
	...
	for ( ; ; ) {
		...
		if ((error = _ReadFromSocket(buffer, HTTP_CLIENT_BUFFER_SIZE - 1, bufferSize)) == XBOX::VE_OK) {
			...
			if ((error = ContinueReadingResponseHeader(buffer, bufferSize, &isComplete)) != XBOX::VE_OK || isComplete)
...
XBOX::VError VHTTPClient::ContinueReadingResponseHeader (const char *inBuffer, VSize inBufferSize, bool *outIsComplete)
{
	...
	if (!fResponseHeaderBuffer.PutDataAmortized(fResponseHeaderBuffer.GetDataSize(), inBuffer, inBufferSize)) 
	...
	else if (fPreviousLineCRLF && (inBufferSize == 2) && (inBuffer[0] == '\r') && (inBuffer[1] == '\n')) {
		...
		*outIsComplete = true;
		...
		fPreviousLineCRLF = (inBufferSize >= 2) && (inBuffer[inBufferSize - 2] == '\r') && (inBuffer[inBufferSize - 1] == '\n');
		*outIsComplete = false;
		error = XBOX::VE_OK;
