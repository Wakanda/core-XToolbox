...
void VTCPSessionManager::Start ( )
{
	...
	if ( fWorkerTask == 0 )
	{
		...
		vstrTaskName. AppendCString ( "ServerNet session manager" );
		fWorkerTask-> SetName( vstrTaskName );
		fWorkerTask-> SetKind ( kServerNetTaskKind );
		fWorkerTask-> SetKindData ( kSNET_SessionManagerTaskKindData );
