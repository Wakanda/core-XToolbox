...
{	
	...
	//jmo - trucs temporaires
	//VFilePath path("C:\\Users\\jmontmartin\\Perforce\\pc_jmo_new_servernet\\depot\\Tools\\demoCA\\cacert.pem");
	//verr=SslFramework::AddRootCertificates(path);
	//xbox_assert(verr==VE_OK);
...
VString ServerNetTools::BeautifyIPString( const VString& inIP)
{
	...
	else
	{
		...
		if (addr.IsV4MappedV6() && testAssert( inIP.FindRawString( CVSTR( "::ffff:")) == 1) )
		{
			ipString.Remove( 1, 7);
		}
	}
