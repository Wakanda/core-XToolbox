...
*/
sLONG VIntlMgr::GetBidiRuns( const UniChar* inText, const sLONG inLength, BidiRuns& outBidiRuns, BidiVisualToLogicalRunIndexMap& outMapVisualToLogical, sLONG inBaseBidiLevel, bool inIgnoreLastCR)
{
	outBidiRuns.clear();
						...
						if (direction == UBIDI_MIXED) 
						{
							...
							if (U_SUCCESS(error) && testAssert(count))
							{
								for (int i = 0; i < count; i++)
								{
									...
									outBidiRuns.push_back( BidiRun( start, start+length, i, direction & 1));
								}
								...
								BidiRuns::const_iterator itRun = outBidiRuns.begin();
	...
	{
		//one run only
		outBidiRuns.push_back( BidiRun( 0, length, 0, paraDirection & 1));
