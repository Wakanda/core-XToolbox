...
UniChar VLexerStringInput::MoveToNextChar()
{
	if (fInput != NULL)
	{
		...
		if( CHAR_CONTROL_000A	== uchResult ||
			CHAR_CONTROL_000D	== uchResult ||
			(UniChar)0x2028		== uchResult ||
			(UniChar)0x2029		== uchResult )
		{
			// line feed
			fCurrentLine++;
		}
...
VIndex VLexerStringInput::GetCurrentPosition()
{
	return fCurrentChar;
}
...
VIndex VLexerFileInput::GetCurrentPosition()
{
