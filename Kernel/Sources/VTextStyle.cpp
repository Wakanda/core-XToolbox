...
*/
void VTreeTextStyle::ExpandAtPosBy(sLONG inStart, sLONG inInflate, const VDocSpanTextRef *inInflateSpanRef, VTextStyle **outStyleToPostApply, bool inPrevChildHasInflated)
{
	if (inInflate <= 0)
		return;
	...
	else
	{
		...
		if (GetData()->IsSpanRef())
		{
			...
			if (start >= inStart)
				Translate( inInflate);
			else if (end == inStart && outStyleToPostApply)
			{
		...
		if (start >= inStart) 
		{
			if (inPrevChildHasInflated  //if previous child has inflated, we cannot inflate otherwise we might overlap previous child range so translate
				|| 
				(start > 0 && end > start))
			{
				Translate(inInflate);
	...
	bool prevChildHasInflated = false;
	...
	if (!outStyleToPostApply)
	{
		...
		for( VectorOfStyle::const_iterator i = fChildren.begin() ; i != fChildren.end() ; ++i)
		{
			...
			if (!prevChildHasInflated)
				vstyle->GetData()->GetRange( beforeChildStart, beforeChildEnd);
			vstyle->ExpandAtPosBy( inStart, inInflate, inInflateSpanRef, &postApplyStyle, prevChildHasInflated);
			if (!prevChildHasInflated)
			{
				vstyle->GetData()->GetRange( afterChildStart, afterChildEnd);
				if (afterChildEnd-afterChildStart > beforeChildEnd-beforeChildStart)
					prevChildHasInflated = true;
			}	
			if (postApplyStyle)
	...
	else
	{
		for( VectorOfStyle::const_iterator i = fChildren.begin() ; i != fChildren.end() ; ++i)
		{
			...
			if (!prevChildHasInflated)
				vstyle->GetData()->GetRange( beforeChildStart, beforeChildEnd);
			vstyle->ExpandAtPosBy( inStart, inInflate, inInflateSpanRef, NULL, prevChildHasInflated);
			if (!prevChildHasInflated)
			{
				vstyle->GetData()->GetRange( afterChildStart, afterChildEnd);
				if (afterChildEnd-afterChildStart > beforeChildEnd-beforeChildStart)
...
VTreeTextStyle *VTreeTextStyle::CreateTreeTextStyleOnRange( const sLONG inStart, const sLONG inEnd, bool inRelativeToStart, bool inNULLIfRangeEmpty, bool inNoMerge) const
{
	...
	if (style)
	{
		...
		bool doMerge = (!fNumChildSpanRef || e == s) && !inNoMerge; //if at least one span ref, do not merge child style with current style
		for (int i = 1; i <= GetChildCount(); i++)
		{
			VTreeTextStyle *childStyles = GetNthChild( i);
			...
			if (newChildStyles)
			{
				...
				if (doMerge && !newChildStyles->GetData()->IsSpanRef() && childStart == s && childEnd == e)
				{
										...
										void *inUserData, fnBeforeReplace inBeforeReplace, fnAfterReplace inAfterReplace)
{
	...
	//replace text inherits on default style from inserted position, not from replaced text;
	//but optionally it can inherit style from replaced text uniform style: normally if and only if we replace styled text with plain text 
	//as we should not reset replaced text uniform style if we replace only the styled text plain text
	//(for instance changing plain text of a styled text styled as bold should not reset bold style:
	// but replacing styled text with a new styled text should do)

	if (ioStyles)
	{
		if (inEnd > inStart)
			ioStyles->Truncate( inStart, inEnd-inStart, 
								(inInheritUniformStyle && pText->GetLength() > 0) ? false : true,	//true  -> new text will inherit only style from inserted position 
		...
		if (pText->GetLength() > 0)
			ioStyles->ExpandAtPosBy( inStart, pText->GetLength(), inPreserveSpanTextRef);
	}
										  ...
										  void *inUserData, fnBeforeReplace inBeforeReplace, fnAfterReplace inAfterReplace)
{
	...
	bool result = ReplaceStyledText( ioText, ioStyles, textSpanRef, styles, inStart, inEnd, inAutoAdjustRangeWithSpanRef, inSkipCheckRange, false, NULL, inUserData, inBeforeReplace, inAfterReplace) != 0;
	ReleaseRefCountable(&styles);
	return result;
}
