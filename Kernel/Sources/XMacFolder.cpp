...
VError XMacFolder::Move( const VFolder& inNewParent, VFolder** outFolder ) const
{
	// CFRunLoopGetMain is a 10.5 api that may exist in 10.4.
	...
	if ( _BuildFSRefFile( &srcRef))
	{
		...
		if (inNewParent.MAC_GetFSRef(&dstFolderRef))
		{
			if (FSMoveObjectSync != NULL)	// 10.4 api
			{
				...
				{
					macError = ::FSMoveObjectSync( &srcRef, &dstFolderRef, NULL, &dstRef, options);
...
XMacFolderIterator::XMacFolderIterator( const VFolder *inFolder, FileIteratorOptions inOptions):fOptions( inOptions)
{
	fIndex = 0;
	fIterator = NULL;
