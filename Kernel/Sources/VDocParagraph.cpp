...
/** set class */
void VDocParagraph::SetClass( const VString& inClass)
{
...
void VDocParagraph::_UpdateTextInfo(bool /*inUpdateTextLength*/, bool /*inApplyOnlyDeltaTextLength*/, sLONG /*inDeltaTextLength*/)
{
	VDocNode::_UpdateTextInfo(false); //for fTextStart

	fTextLength = fText.GetLength();
}


bool VDocParagraph::_EqualListStringFormat( const VDocNode *inNode)
{
	if (inNode->IsOverriden(kDOC_PROP_LIST_STRING_FORMAT_LTR) != IsOverriden(kDOC_PROP_LIST_STRING_FORMAT_LTR))
		return false;
	else if (IsOverriden(kDOC_PROP_LIST_STRING_FORMAT_LTR) && !GetListStringFormatLTR().EqualToString( inNode->GetListStringFormatLTR(), true))
		return false;
						
	if (inNode->IsOverriden(kDOC_PROP_LIST_STRING_FORMAT_RTL) != IsOverriden(kDOC_PROP_LIST_STRING_FORMAT_RTL))
		return false;
	else if (IsOverriden(kDOC_PROP_LIST_STRING_FORMAT_RTL) && !GetListStringFormatRTL().EqualToString( inNode->GetListStringFormatRTL(), true))
		return false;
	return true;
}

void VDocParagraph::_UpdateListNumber(eListStyleTypeEvent inListEvent)
{
	...
	switch (inListEvent)
	{
		case kDOC_LSTE_BEFORE_REMOVE_FROM_LIST:
			{
				...
				VDocNode *node = _GetNextSibling();
				while (node)
				{
					if (node->IsOverriden(kDOC_PROP_LIST_START) && GetListStart() != kDOC_LIST_START_AUTO)
						break;
					bool nodeIsOrdered = node->GetListStyleType() >= kDOC_LIST_STYLE_TYPE_OL_FIRST && node->GetListStyleImage().IsEmpty();
		...
		case kDOC_LSTE_AFTER_UPDATE_START_NUMBER:
			{
			if (IsOverriden(kDOC_PROP_LIST_START) && GetListStart() != kDOC_LIST_START_AUTO)
				fListNumber = GetListStart();
			else 
			{
...
*/
void VDocParagraph::NormalizeStyles(bool inOverrideStyles, bool inApplyOnlyToUniformCharStyles)
{
	if (fStyles)
	{
#if VERSIONDEBUG
		sLONG start, end;
		fStyles->GetData()->GetRange(start, end);
		xbox_assert(start == 0 && end == fText.GetLength());
#endif
		if (inApplyOnlyToUniformCharStyles)
			CharPropsMoveToVTextStyle( *(fStyles->GetData()), inOverrideStyles, false);
		else
		{
			VTextStyle *style = new VTextStyle();
			CharPropsMoveToVTextStyle( *style, true, false);
			if (!style->IsUndefined())
			{
				style->SetRange(0, fText.GetLength());
				fStyles->ApplyStyle( style);
			}
		}
	}
	else
	{
		...
		{
			style->SetRange(0, fText.GetLength());
			fStyles = new VTreeTextStyle( style);
		}
		else
			delete style;
	}
...
*/
void VDocParagraph::AppendPropsFrom(const VDocNode *inNode, bool inOnlyIfNotOverriden, bool inNoAppendSpanStyles, 
									sLONG /*inStart*/, sLONG /*inEnd*/, 
									uLONG inClassStyleTarget, bool inClassStyleOverrideOnlyUniformCharStyles, 
									bool inOnlyIfNotOverridenRecurseUp)
{
	if (inNode->GetType() == kDOC_NODE_TYPE_CLASS_STYLE)
	{
		if (inClassStyleTarget == kDOC_NODE_TYPE_PARAGRAPH)
		{
			//append only common properties
			VDocNode::AppendPropsFrom( inNode, inOnlyIfNotOverriden, inNoAppendSpanStyles, 0, -1, inClassStyleTarget, inClassStyleOverrideOnlyUniformCharStyles, inOnlyIfNotOverridenRecurseUp);
			//normalize character styles
			if (!inNoAppendSpanStyles)
				NormalizeStyles(!inOnlyIfNotOverriden, inClassStyleOverrideOnlyUniformCharStyles); //move character properties to fStyles
		}
		return;
	}
	...
	VDocNode::AppendPropsFrom( inNode, inOnlyIfNotOverriden, true, 0, -1, kDOC_NODE_TYPE_PARAGRAPH, true, inOnlyIfNotOverridenRecurseUp);
	...
	VTextStyle *styleFromProps = new VTextStyle();
	inNode->CharPropsCopyToVTextStyle( *styleFromProps, true, false);
	if (styleFromProps->IsUndefined())
	{
		delete styleFromProps;
		styleFromProps = NULL;
	}
	const VTextStyle *style = styleFromProps;
	...
	{
		if (styleFromProps)
			styleFromProps->AppendStyle( inNodePara->fStyles->GetData(), false);
		else
			style = inNodePara->fStyles->GetData();
	}
	if (style)
	{
		if (fStyles)
			fStyles->GetData()->AppendStyle( style, inOnlyIfNotOverriden);
		else
		{
			if (styleFromProps)
			{
				fStyles = new VTreeTextStyle( styleFromProps);
				styleFromProps = NULL;
			}
			else
...
void VDocParagraph::_AfterReplace(void *inUserData, sLONG inStartReplaced, sLONG inEndReplaced)
{
	...
	para->_NormalizeRange( inStartReplaced, inEndReplaced);
	if (para->fStyles && para->GetDocumentNode())
	{
		while (inStartReplaced < inEndReplaced)
		{
			...
			if (style->IsSpanRef() && style->GetSpanRef()->GetType() == kSpanTextRef_Image && para->GetDocumentNode() != style->GetSpanRef()->GetImage()->GetDocumentNode())
			{
...
void VDocParagraph::_OnAttachToDocument(VDocNode *inDocumentNode)
{
	VDocNode::_OnAttachToDocument( inDocumentNode);
	_AfterReplace( this, 0, -1);
}

void VDocParagraph::_OnDetachFromDocument(VDocNode *inDocumentNode)
{
#if kDOC_VERSION_MAX_SUPPORTED >= kDOC_VERSION_XHTML4D 
	sLONG textStart = 0, textEnd = fText.GetLength();
	if (fStyles && GetDocumentNode())
	{
		while (textStart < textEnd)
		{
			const VTextStyle *style = fStyles->GetStyleRefAtRange( textStart, textEnd);
			if (!style)
				break;

			if (style->IsSpanRef() && style->GetSpanRef()->GetType() == kSpanTextRef_Image && style->GetSpanRef()->GetImage()->GetDocumentNode())
			{
				//unbind image from document 
				
				xbox_assert( style->GetSpanRef()->GetImage()->GetDocumentNode() == GetDocumentNode()); //it should not belong to another document...

				//unbind from this document
				GetDocumentNode()->RemoveImage( style->GetSpanRef()->GetImage());
			}
			sLONG start, end;
			style->GetRange( start, end);
			if (!testAssert(start < end))
	...
	VDocNode::_OnDetachFromDocument( inDocumentNode);
}
