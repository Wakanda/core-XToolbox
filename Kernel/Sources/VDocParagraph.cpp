...
*/
sLONG VDocParagraph::GetPaddingFirstLine() const { return IDocProperty::GetProperty<sLONG>( static_cast<const VDocNode *>(this), kDOC_PROP_PADDING_FIRST_LINE); }

/** set first line padding offset in TWIPS 
@remarks
	might be negative for negative padding (that is second line up to the last line is padded but the first line)
*/
void VDocParagraph::SetPaddingFirstLine(const sLONG inValue) { IDocProperty::SetProperty<sLONG>( static_cast<VDocNode *>(this), kDOC_PROP_PADDING_FIRST_LINE, inValue); }
