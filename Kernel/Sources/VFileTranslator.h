...
#if VERSIONMAC && !VERSIONOSX && !VERSION_MACHO
...
class XTOOLBOX_API VNavTranslator : public VFileTranslator
{
public:
			VNavTranslator(const NavReplyRecord &inReply, VTranslationMode inMode = TRM_TRANSLATEINPLACE);
	virtual	~VNavTranslator();
	
	virtual TranslatorType	GetTranslatorType(void) { return TT_NAVIGATION; };
	virtual void GetTranslatorName(VString &inString) const;
	virtual Boolean DoTranslation(void);
	virtual void SetTranslationMode(VTranslationMode inMode);
	VTranslationMode GetTranslationMode(void) const { return fTranslationMode; } ;

protected:
	VTranslationMode fTranslationMode;
	NavReplyRecord fReply;
};
