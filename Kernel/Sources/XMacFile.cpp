...
VError XMacFile::Move( const VFilePath& inDestinationPath, VFileSystem *inDestinationFileSystem, VFile** outFile, FileCopyOptions inOptions ) const
{
	...
	{
		if (macError == noErr)
			*outFile = new VFile( inDestinationPath, inDestinationFileSystem);
		else
			*outFile = NULL;
	}
...
OSErr XMacFile::_DeleteFSRefBrutally(FSRef& inFileRef) const
{
	...
	do {
		...
		//ACI0082812, O.R., Aug 11th 2013, if FSDeleteObject failed during 2 s
		//previous code entered an infinite loop
