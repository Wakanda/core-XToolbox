...
VError XMacFile::Copy( const VFilePath& inDestinationPath, VFile** outFile, FileCopyOptions inOptions ) const
{
	// CFRunLoopGetMain is a 10.5 api that may exist in 10.4.
...
VError XMacFile::Move( const VFilePath& inDestinationPath, VFile** outFile, FileCopyOptions inOptions ) const
{
	// CFRunLoopGetMain is a 10.5 api that may exist in 10.4.
	// Let's load it dynamically.
#if MAC_OS_X_VERSION_MAX_ALLOWED <= MAC_OS_X_VERSION_10_4
...
VError XMacFile::Delete() const
{
	...
	FSRef fileRef;
	if (_BuildFSRefFile( &fileRef)){
		macErr = ::FSDeleteObject(&fileRef);
				
		if(macErr == fBsyErr){
			// We didn't achieve to delete the file. Certainly a Spotlight problem (Spotlight want to index the file while we want to delete it)
			...
			if ( _BuildParentFSRefFile( &parentRef, fileName) )
			{
				...
				do{
					...
					VString uniqueFileName; 
					uniqueID.GetString(uniqueFileName);
					uniqueFileName.Insert(".", 1); //The temporary file begins with a dot so it won't be seen
					uniqueFileName += ".temp";
					FSCatalogInfo tempFileInfo;
					::memset( &tempFileInfo, 0, sizeof( tempFileInfo));
					macErr = ::FSCreateFileUnicode( &parentRef, uniqueFileName.GetLength(), uniqueFileName.GetCPointer(), kFSCatInfoNone, &tempFileInfo, &tempFileRef, NULL);
					++tryIndex;
				}
				while(noErr != macErr && tryIndex < maximumTriesCount);
				
				// 2. Exchange the contents safely (files must be on the same physical volume). If we have errors here... well, we need to delete the files anyway (it may fail if the volumes don't support the function
				::FSExchangeObjects( &tempFileRef, &fileRef);
				...
				if (tempFileURLRef) {
					...
					if (tempFileURLStringRef) {
						...
						if (tempFileURLChar) {
							if(::CFStringGetCString(tempFileURLStringRef, tempFileURLChar, tempStringSize, kCFStringEncodingUTF8))
							{
								//With unlink the file will be deleted as soon as Spotlight let the temp file. There is one case where it could not work : if the file path is over 1024 bytes...
								if(unlink(tempFileURLChar) != 0)	{
									// II. If the safe method didn't work, let's try the old one
									macErr = _DeleteFSRefBrutally(tempFileRef);
								}
							}
							free(tempFileURLChar);
						}
				...
				//4. Delete the file
				macErr = _DeleteFSRefBrutally(fileRef);		
			}
		}
	}
	else
...
OSErr XMacFile::_DeleteFSRefBrutally(FSRef& inFileRef) const
{
	//We try to delete the file brutally during 2 seconds...
	OSErr macError = noErr;
	...
	double t2 = t1;
	do {
		macError = ::FSDeleteObject(&inFileRef);
		if(macError != noErr)
			usleep(50);
		
		gettimeofday(&tim, NULL);
		t2 = tim.tv_sec + (tim.tv_usec/1000000.0);
	} while(macError != noErr || t2 - t1 > 2.0);
...
VError XMacFile::GetFileAttributes( EFileAttributes &outFileAttributes ) const
{
	...
	VString fileName;
	FSCatalogInfo catInfo;
	OSStatus macError = fnfErr;

	outFileAttributes = 0;
	
	if ( _BuildFSRefFile( &fileRef) )
	{
		macError = ::FSGetCatalogInfo(&fileRef, kFSCatInfoNodeFlags | kFSCatInfoFinderInfo | kFSCatInfoPermissions, &catInfo, NULL, NULL, NULL);
		if ( (macError == noErr) && ( (catInfo.nodeFlags & kFSNodeLockedMask) != 0) )
			outFileAttributes |= FATT_LockedFile;
		
		// WAK0071919: Checking for locked file is not enough. So check permission and see if user has write permission, if not consider as read only.
		
#define UNIX_MODE_USER_WRITE_FLAG	0200
		
		uLONG	mode;
		
#ifdef __LP64__
		
		mode = catInfo.permissions.mode;
		
#else
		
		mode = ((FSPermissionInfo *) catInfo.permissions)->mode;	// In 32-bit permissions is defined as UInt32[4].
		
#endif

		if (macError == noErr && !(mode & UNIX_MODE_USER_WRITE_FLAG))
		...
		if (macError == noErr)
		{
			...
			macError = ::LSCopyItemInfoForRef( &fileRef, kLSRequestBasicFlagsOnly, &info);
...
VError XMacFile::SetFileAttributes( EFileAttributes inFileAttributes ) const
{
	...
	if ( _BuildFSRefFile( &fileRef) )
		macError = ::FSGetCatalogInfo(&fileRef, kFSCatInfoNodeFlags | kFSCatInfoFinderInfo, &catInfo, NULL, NULL, NULL);
	...
	if ( macError == noErr )
	{
		...
		macError = ::FSSetCatalogInfo(&fileRef, kFSCatInfoNodeFlags | kFSCatInfoFinderInfo, &catInfo);
	}
...
VError XMacFile::ResolveAlias( VFilePath &outTargetPath ) const
{
	...
	if ( _BuildFSRefFile( &fileRef) )
	{
		...
		macError = ::FSResolveAliasFile(&fileRef, true, &isFolder, &wasAliased);
		if ( macError == noErr && wasAliased )
		{
			VString fullPath;
			macError = FSRefToHFSPath(fileRef,fullPath);
			if ( macError == noErr )
			{
				if (isFolder)
...
VError XMacFile::CreateAlias( const VFilePath& inTargetPath, const VFilePath *inRelativeAliasSource) const
{
	...
	FSRef srcParentRef;
	FSRef dstAliasRef;
	FSRef dstAliasParentRef;
	
	OSErr macError = fnfErr;
	VError error = VE_OK;
	VString srcName;

	if ( _BuildParentFSRefFile( &srcParentRef, srcName ) )
	{
		Boolean isFolder = false;
		VString targetName;
		VFilePath targetParentPath;
		FSCatalogInfo catalog;
		FSCatalogInfoBitmap catalogBitmap = kFSCatInfoNone;
		AliasHandle hAlias = NULL;

		inTargetPath.GetName(targetName);
		inTargetPath.GetParent(targetParentPath);
		...
		if ( macError == noErr )
			macError = HFSPathToFSRef(targetParentPath.GetPath(), &dstAliasParentRef);

		if ( macError == noErr )
			macError = ::FSNewAliasUnicode ( &dstAliasRef, &dstAliasParentRef, targetName.GetLength(), targetName.GetCPointer(), &hAlias, &isFolder );
		if (macError == fnfErr)	// The alias is created anyway
			macError = noErr;

		if ( macError == noErr )
		{
			::FSCreateResFile( &srcParentRef, srcName.GetLength(), srcName.GetCPointer(), catalogBitmap, &catalog, &srcFileRef, NULL );
				
			short oldRef = ::CurResFile();
			short fileRef = ::FSOpenResFile( &srcFileRef, fsRdWrPerm );
			if ( fileRef != -1 )
			{
				Handle alisRes = (Handle)hAlias;
				::HandToHand(&alisRes);
				::UseResFile(fileRef);
				::HLock(alisRes);
				::AddResource(alisRes,'alis',0,(uBYTE*)"");
				::HUnlock(alisRes);
		...
		if ( macError == noErr )
		{
			// Set Finder infos
			FSCatalogInfo fileInfo;
			OSType aliasCreator = 0;
			OSType aliasType = 0;
			FSRef lastRefFile;
			Boolean wasChanged = false;
			
			macError = ::FSFollowFinderAlias( NULL, hAlias, false, &lastRefFile, &wasChanged );
					
			if (macError == noErr)
				macError = ::FSGetCatalogInfo(&lastRefFile, kFSCatInfoFinderInfo | kFSCatInfoParentDirID | kFSCatInfoSharingFlags, &fileInfo, NULL, NULL, NULL);
					
			if (macError == noErr)
			{
				if (isFolder)
				{
					// Target is a folder (currently support app packages, generic packages, harddisks, servers and generic folder)
					if (fileInfo.parentDirID == fsRtParID)
					{
						// Folder is a root directory (points directly to volume)
						aliasType = (fileInfo.sharingFlags & kioFlAttribSharePointMask) != 0 ? kContainerServerAliasType : kContainerHardDiskAliasType;
						aliasCreator = 'MACS';
					}
					else
					{
						// Folder may be a package or generic directory
						OSType	type, creator;

						CFURLRef bundleURL = ::CFURLCreateFromFSRef( NULL, &lastRefFile );
						if ( bundleURL != nil )
						{
							...
							if ( bundle != nil )
							{
								...
								aliasType = (type == 'APPL') ? kAppPackageAliasType : kPackageAliasType;
								aliasCreator = creator;
							}
							else
							{
				...
				{
					// Target is a file (currently support apps and generic files)
					aliasType = (((FileInfo*) fileInfo.finderInfo)->fileType == 'APPL') ? kApplicationAliasType : ((FileInfo*) fileInfo.finderInfo)->fileType;
					aliasCreator = ((FileInfo*) fileInfo.finderInfo)->fileCreator;
				}
			}

			if ( testAssert(macError == noErr))
				macError = ::FSGetCatalogInfo(&srcFileRef, kFSCatInfoFinderInfo, &fileInfo, NULL, NULL, NULL);
			if (testAssert(macError == noErr))
			{
				((FileInfo*) fileInfo.finderInfo)->finderFlags |= kIsAlias;
				((FileInfo*) fileInfo.finderInfo)->fileType = aliasType;
				((FileInfo*) fileInfo.finderInfo)->fileCreator = aliasCreator;
					
				macError = ::FSSetCatalogInfo(&srcFileRef, kFSCatInfoFinderInfo, &fileInfo);
			}
		}
...
bool XMacFile::IsAliasFile() const
{
	...
	{
		Boolean	isFolder;
		::FSIsAliasFile( &fileRef, &isAlias, &isFolder);
	}
...
XMacFileIterator::XMacFileIterator( const VFolder *inFolder, FileIteratorOptions inOptions):fOptions( inOptions)
{
	fIndex = 0;
	fIterator = NULL;
