...
VError VFileSystemNamespace::LoadFromDefinitionFile( VFile *inFile)
{
	...
	if (err != VE_OK)
	{
		StThrowError<> err( VE_FS_CANT_LOAD_DEFINITION_FILE);
		err->SetString( CVSTR( "path"), inFile->GetPath().GetPath());
	}
...
bool VFileSystemNamespace::ParsePathWithFileSystem( const VString& inPath, VFile **outFile, VFolder **outFolder)
{
	bool ok = false;
	...
	ok = ParsePathWithFileSystem(inPath, fs, path);
...
bool VFileSystemNamespace::ParsePathWithFileSystem( const VString& inPath, VFileSystem* &outFS, VFilePath& outPath)
{
	bool ok = false;
	outFS = nil;
	...
	if (!inPath.IsEmpty() && (inPath[0] == '/') )
	{
		...
		if (pos > 0)
		{
			...
			VFileSystem *fs = RetainFileSystem( fileSystemName, eFSN_Default);
			if (fs != NULL)
			{
