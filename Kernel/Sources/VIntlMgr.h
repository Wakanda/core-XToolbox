	...
	*/
	...
	static	DialectCode			ResolveDialectCode( DialectCode inDialect);
			...
			public:
				...
				BidiRun( const sLONG inStart, const sLONG inEnd, const sLONG inVisualIndex, const sLONG inBidiLevel)
				{
					fStart = inStart;
					fEnd = inEnd;
				...
				BidiRun& operator = ( const BidiRun& inRun)
				{
					if (this == &inRun)
						return *this;
					fStart = inRun.fStart;
				...
				bool operator <(const BidiRun& inRun) const
				{
			...
			/** vector of bidi runs */
	typedef	std::vector<BidiRun>	BidiRuns;

			/** bidi map from visual to logical run index */
	typedef std::vector<sLONG>		BidiVisualToLogicalRunIndexMap;
			...
			*/
			sLONG				GetBidiRuns( const UniChar* inText, const sLONG inLength, BidiRuns& outBidiRuns, BidiVisualToLogicalRunIndexMap& outMapVisualToLogical, sLONG inBaseBidiLevel = VINTLMGR_UBIDI_DEFAULT_LTR, bool inIgnoreLastCR = false);
