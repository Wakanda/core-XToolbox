...
/*
 *      $Id: mapm_div.c,v 1.7 2001/02/11 22:30:42 mike Exp $
 *
...
/****************************************************************************/
void	M_apm_sdivide(r,places,a,b)
M_APM	r, a, b;
int	places;
{
...
if (sign == 0)      /* one number is zero, result is zero */
  {
   if (b->m_apm_sign == 0)
     {
      fprintf(stderr,"Warning! ... \'M_apm_sdivide\', Divide by 0\n");
     }
...
if (k > r->m_apm_malloclength)
  {
   if ((vp = MAPM_REALLOC(r->m_apm_data, (k + 32))) == NULL)
     {
      fprintf(stderr,"\'M_apm_sdivide\', Out of memory\n");
      exit(16);
     }
