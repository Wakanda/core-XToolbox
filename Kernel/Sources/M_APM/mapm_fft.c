...
/*
 *      $Id: mapm_fft.c,v 1.8 2000/08/01 22:23:24 mike Exp $
 *
...
void	M_fast_mul_fft(ww,uu,vv,nbytes)
UCHAR   *ww, *uu, *vv;
int     nbytes;
{
static double   *aa_array, *bb_array;
static int	msize = -1;
...
if (msize < 0)                   /* if first time in, setup working arrays */
  {
   ...
   aa_array = (double *)MAPM_MALLOC(msize * sizeof(double));
   bb_array = (double *)MAPM_MALLOC(msize * sizeof(double));
   
   if ((aa_array == NULL) || (bb_array == NULL))
     {
      fprintf(stderr,"\'M_fast_mul_fft\', Out of memory\n");
      exit(16);
     }
  }
