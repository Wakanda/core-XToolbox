...
/*
 *      $Id: mapmutil.c,v 1.13 2001/02/11 22:36:43 mike Exp $
 *
...
if (i > ctmp->m_apm_malloclength)
  {
   if ((vp = MAPM_REALLOC(ctmp->m_apm_data, (i + 32))) == NULL)
     {
      fprintf(stderr,"\'M_apm_scale\', Out of memory\n");
      exit(18);
     }
   
   ctmp->m_apm_malloclength = i + 28;
...
if ((ct & 1) != 0)          /* move odd number first */
  {
   ...
   if (numb > M_div_rem_size)
     {
      if ((vp = MAPM_REALLOC(M_div_array,(numb + 32))) == NULL)
        {
         fprintf(stderr,"\'M_apm_scale\', Out of memory\n");
         exit(18);
        }
      ...
      M_div_rem_size = numb + 28;

      if ((vp = MAPM_REALLOC(M_rem_array,(numb + 32))) == NULL)
        {
         fprintf(stderr,"\'M_apm_scale\', Out of memory\n");
         exit(18);
        }
 ...
 *
 */
void    M_long_2_ascii(output,input)
char    *output;
long    input;
{
long    t, m;
