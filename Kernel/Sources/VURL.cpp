...
VURL::VURL( const VURL& inBaseURL, const VString& inRelativPath, bool inEncodedString)
{
	fBaseURL = NULL;
	fIsConform = false;
	FromRelativePath( inBaseURL, inRelativPath, inEncodedString);
}

void VURL::SetBaseURL( const VURL& inBaseURL)
{
	fURLStringEncoded.SetEmpty();
	if (fBaseURL != &inBaseURL)
	{
		delete fBaseURL;

		fBaseURL = new VURL( inBaseURL);

		if (fBaseURL)
			fBaseURL->_RemoveLastPathComponent();
	}
}
...
//	else
	...
	if (inURL.Find(CVSTR("://")) <= 0)
	{
		if (!inScheme.IsEmpty() && !inURL.BeginsWith(CVSTR("/")))
			fURLString.Insert((UniChar)'/', 1);
		fURLString.Insert( inScheme, 1);
//		if (inEncodedString)
//			fURLStringEncoded = fURLString;
	}
...
void VURL::FromRelativePath( const VURL& inBaseURL, const VString& inRelativPath, bool inEncodedString)
{
	if (fBaseURL != &inBaseURL)
	{
		delete fBaseURL;
...
void VURL::GetAbsoluteURL( VString& outURLString, bool inEncoded) const
{
	if (inEncoded)
	{
		...
		else
		{
			...
			fURLStringEncoded = outURLString;
		}
	}
	else
	{
...
void VURL::Encode (XBOX::VString& ioString, bool inConvertColon, bool inDecomposeString, bool inEncodeBrackets)
{
	...
	{
		uBYTE *buf2 = (uBYTE*)std::malloc( utf8CharsArray.size());
		if (buf2 != NULL)
		{
			std::size_t arrayPos = 0;
			while (arrayPos < utf8CharsArray.size())
			{
				*(buf2+arrayPos) = utf8CharsArray[arrayPos];
				++arrayPos;
			}
			ioString.AppendBlock( buf2, utf8CharsArray.size(), VTC_UTF_8);
			std::free( buf2);
		}
	}			
}
...
void VURL::Decode (XBOX::VString& ioString)
{
	...
	if (pos > 0)
	{
		...
		uBYTE *				buffer = (uBYTE *)std::malloc (bufferSize);
		...
		do {
			...
			if (!byteArray.empty())
			{
				...
				if (byteArray.size() > bufferSize)
				{
					...
					buffer = (uBYTE *)std::realloc (buffer, bufferSize);
				}
...
void VURL::_GetRelativePath(VString& outString) const
{
	...
	_GetAbsoluteURL(outString);
	pos = _GetBaseLength();
	if (fURLArray[e_Path].pos > pos) pos = fURLArray[e_Path].pos-1;
