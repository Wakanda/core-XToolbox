...
void VPageAllocationImpl::GetStats(VMemStats& stats, bool& isfull)
{
	...
	if (fNbFull == 0)
	{
		...
		fNbFull = fNbFull; // put a break here
	}
		...
		if (inSize < (2147483647 - 2048))
		{
			...
			for (pass = 1; pass <= 2; pass++)
			{
				...
				else // on sait qu'il exite des block libre de la bonne taille
				{
					...
					{
						VMemImplBlock* x = *p;
						assert(x->GetPrevious() == NULL);
						*p = x->GetNext();
						if (*p != NULL)
							(*p)->SetPrevious(NULL);
						result = ((char*)x) + SizeHeader;
						assert(x->GetLen() >= inSize); // on peut trouver un block un peu plus grand qui resulte du merge de deux ou trois plus petits
						VSize plus = isAnObject ? 1 : 0;
						VSize plus2 = inForceinMain ? 2 : 0;
...
void VMemCppImpl::Free( void *inBlock)
{
	...
	else
	{
		...
		else
		{
			...
			sLONG step;
			assert(res != NULL); // on devrait trouver la page d'allocation qui contient le ptr
			if (res != NULL)
			{
				VMemImplBlock* prev = NULL;
				VMemImplBlock* next = NULL;
				VSize len = x->GetLen();

				assert(len <= fUsedMem);
				fUsedMem = fUsedMem - len;

				if (x->GetPreviousLen() > 0)
				{
					prev = (VMemImplBlock*) ( ((char*)x) - x->GetPreviousLen() );
					assert( prev->GetLen() == x->GetPreviousLen() );
					if (prev->IsFree())
					{
						len = len + prev->GetLen();
						step = GetExactStepFromSize(prev->GetLen());
						RemoveBlockFromChainList(prev, &fFirstBlocks[step]);
					}
				}
				Boolean islastblock = res->IsLastBlock(x); // je le garde en local pour pouvoir debugger plus facilement
				if (! islastblock)
				{
					next = (VMemImplBlock*) ( ((char*)x) + x->GetLen() );
					assert(next->GetPreviousLen() == x->GetLen() );
					if (next->IsFree())
					{
						len = len + next->GetLen();
						step = GetExactStepFromSize(next->GetLen());
						RemoveBlockFromChainList(next, &fFirstBlocks[step]);
						if (!res->IsLastBlock(x))
						{
							VMemImplBlock *nextnext = (VMemImplBlock*) ( ((char*)next) + next->GetLen() );
							assert(next->GetLen() == nextnext->GetPreviousLen());
							nextnext->SetPreviousLen(len);
						}
					}
					else
						next->SetPreviousLen(len);
				...
				else // on est sur le dernier block de l'allocation, on reduit le fDataEnd
				{
					if (prev != NULL && prev->IsFree())
					{
						res->ReduceFrom(prev);
