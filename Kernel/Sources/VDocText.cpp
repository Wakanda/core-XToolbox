...
void VDocNode::Init()
{
	...
	sPropDefault[kDOC_PROP_BACKGROUND_REPEAT]		= new VDocProperty( (uLONG)0); //repeat 
	sPropDefault[kDOC_PROP_BACKGROUND_CLIP]			= new VDocProperty( (uLONG)kTEXT_BACKGROUND_BORDER_BOX);	//border-box 
	sPropDefault[kDOC_PROP_BACKGROUND_ORIGIN]		= new VDocProperty( (uLONG)kTEXT_BACKGROUND_PADDING_BOX);	//padding-box 
	sPropDefault[kDOC_PROP_BACKGROUND_SIZE]			= new VDocProperty( (sLONG)kTEXT_BACKGROUND_SIZE_AUTO);		//auto
	...
	sPropDefault[kDOC_PROP_IMG_SOURCE]				= new VDocProperty( CVSTR("")); 
	sPropDefault[kDOC_PROP_IMG_ALT_TEXT]			= new VDocProperty( CVSTR("")); //FIXME: maybe we should set default text for image element ?
...
{ 
	...
	if (testAssert(clip >= kTEXT_BACKGROUND_BORDER_BOX && clip <= kTEXT_BACKGROUND_CONTENT_BOX))
		return (eDocPropBackgroundBox)clip;
	else 
		return kTEXT_BACKGROUND_BORDER_BOX;
}
...
{ 
	...
	if (testAssert(repeat >= kTEXT_BACKGROUND_REPEAT && repeat <= kTEXT_BACKGROUND_NO_REPEAT))
		return (eDocPropBackgroundRepeat)repeat;
	else 
		return kTEXT_BACKGROUND_REPEAT;
}
...
{ 
	...
	if (testAssert(origin >= kTEXT_BACKGROUND_BORDER_BOX && origin <= kTEXT_BACKGROUND_CONTENT_BOX))
		return (eDocPropBackgroundBox)origin;
	else 
		return kTEXT_BACKGROUND_PADDING_BOX;
}
