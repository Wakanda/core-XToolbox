...
#elif VERSION_LINUX
	return XLinuxSystem::GetCurrentTime();
...
#if VERSIONMAC
	AbsoluteTime wakeup = AddDurationToAbsolute( inMilliseconds, UpTime());
	::MPDelayUntil( &wakeup);
...
sLONG VSystem::GetNumberOfProcessors()
{
	...
	#elif VERSIONMAC
		sNumProcessors = (sLONG) ::MPProcessors();
		if (sNumProcessors == 0) // strange isn't it?
			sNumProcessors = 1;
...
bool VSystem::GetProcessorsPercentageUse(Real& outPercentageUse)
{
	...
	if(host_processor_info(mach_host_self(), PROCESSOR_CPU_LOAD_INFO, &processorCount, (processor_info_array_t *)&processorTickInfo, &processorMsgCount) == KERN_SUCCESS){
...
sLONG8 VSystem::GetPhysicalMemSize()
{
	...
	int kerr = ::host_info( mach_host_self(), HOST_BASIC_INFO, (host_info_t) &hostinfo, &count);
    if (kerr) 
        result = 0;
    else 
...
#elif VERSION_LINUX
    xbox_assert(0); // Need a Linux Implementation !
#elif VERSIONWIN
	//	VStackCrawl::Init() called from VDebugger::Init has called SymInitialize()
	char buffer[1024];
