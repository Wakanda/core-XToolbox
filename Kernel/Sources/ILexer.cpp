...
bool VLexerBase::ConsumeString( VString* vstrBuffer, TokenList *ioTokens, sLONG inType, sLONG inValue )
{
	...
	while (fLexerInput->HasMoreChars()) {
		...
		if ( fLexerInput->HasMoreChars() && IsStringEscapeSequence( c, inType )) {
			...
			// Don't remove code until we're sure that something else is not broken
			//if (!ConsumeEscapeSequence( inType, characterToAdd )) {
			//	// If we couldn't consume the escape sequence, then we want to add what we thought was the
			//	// start of an escape sequence to the stream, and then just add characters as normal.
			//	AppendUniCharWithBuffer( c, vstrBuffer, szUCharBuffer, nBufferSize, 256 );
			//} else {
			//	// We were able to consume the escape sequence, which generates a single character for us
			...
			if (ioTokens)	ioTokens->push_back( new VLexerToken ( ILexerToken::TT_STRING, 
									nStartingPosition, vstrBuffer->GetLength(), *vstrBuffer, inValue ) );

			return true;
		} else if (IsStringContinuationSequence(c, inType) && ! fLexerInput->HasMoreChars() ) {
			if (ioTokens)
				ioTokens->push_back( new VLexerToken( ILexerToken::TT_OPEN_STRING,
							nStartingPosition, nBufferSize, *vstrBuffer, inValue ) );
			return false;
		} else {
