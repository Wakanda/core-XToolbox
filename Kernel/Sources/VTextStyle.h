...
{
public:
	static sLONG PointToTWIPS(Real inValue) { return floor(inValue*20+0.5f); }
...
typedef void (*fnAfterReplace)(void *inUserData, sLONG inStartReplaced, sLONG inEndReplaced);
	...
	*/
	static VIndex ReplaceStyledText(	VString& ioText, VTreeTextStyle*& ioStyles, const VString& inText, const VTreeTextStyle *inStyles = NULL, sLONG inStart = 0, sLONG inEnd = -1, 
									bool inAutoAdjustRangeWithSpanRef = true, bool inSkipCheckRange = false, bool inInheritUniformStyle = false, const VDocSpanTextRef * inPreserveSpanTextRef = NULL, void *inUserData = NULL, fnBeforeReplace inBeforeReplace = NULL, fnAfterReplace inAfterReplace = NULL);
	...
	*/
	void ExpandAtPosBy(sLONG inStart, sLONG inInflate = 1, const VDocSpanTextRef *inInflateSpanRef = NULL, VTextStyle **outStyleToPostApply = NULL, bool inPrevChildHasInflated = false);
...
/** replace styled text at the specified range with text 
@remarks
	if inInheritUniformStyle == true, replaced text will inherit uniform style on replaced range (false on default)
*/
virtual bool ReplaceStyledText(	VString& ioSpanText, const VString& inSpanTextOrPlainText, sLONG inStart = 0, sLONG inEnd = -1, bool inTextPlainText = false, bool inInheritUniformStyle = false, bool inSilentlyTrapParsingErrors = true) = 0;
...
*/
virtual VDocText *ParseDocument(	const VString& inDocText, const uLONG inVersion = kDOC_VERSION_SPAN4D_1, const uLONG inDPI = 72, 
