...
VError XWinFile::Copy( const VFilePath& inDestination, VFile** outFile, FileCopyOptions inOptions ) const
{
	...
	if (outFile)
	{
		*outFile = (winErr == 0) ? new VFile( newPath) : NULL;
	}
...
VError XWinFile::Move( const VFilePath& inNewPath, VFile** outFile, FileCopyOptions inOptions) const
{
...
XWinFileIterator::XWinFileIterator( const VFolder *inFolder, FileIteratorOptions inOptions):fOptions( inOptions)
{
	fIndex = 0;
	...
	else
	{
		...
		if ( !isFolder && ( !isInvisible || (fOptions & FI_WANT_INVISIBLES)))
		{
			if (fOptions & FI_WANT_FILES)
			{
				...
				VFile *theFile = new VFile( currentPath);
