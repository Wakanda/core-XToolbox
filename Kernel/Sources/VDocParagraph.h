...
static	IDocSpanTextRef*		GetDelegate();
		...
		{ 
			...
			fDefaultValue = fComputedValue = inImage->GetText();
			fValueDirty = false;
		...
		@remarks
			...
			note that class instance takes delegate ownership (so caller should not delete the delegate)
		*/
		void					SetLayoutDelegate( IDocSpanTextRefLayout* inDelegate)
		{
			if (fLayoutDelegate == inDelegate)
				return;
			if (fLayoutDelegate)
			{
				delete fLayoutDelegate;
		...
		*/
		...
		const VString&			GetDisplayValue(bool inResetDisplayValueDirty = true)  
		{
			...
			if (fType == kSpanTextRef_Image)
				//images are always equal to one no-breaking space (image run must always be 1 character width)
				return fNBSP; 
			else
				if (fShowRef)
