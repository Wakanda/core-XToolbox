...
void VString::RemoveWhiteSpaces( bool inLeadings, bool inTrailings, VIndex* outLeadingsRemoved, VIndex* outTrailingsRemoved, VIntlMgr *inIntlMgr)
{
	if (inIntlMgr)
	{
		if ( inLeadings )
		{
			...
			while ( i < fLength && inIntlMgr->IsSpace(fString[ i ]) )
				i++;
		...
		if ( inTrailings )
		{
			VIndex i = fLength;
			VIndex removed = 0;
			while ( i > 0 && inIntlMgr->IsSpace(fString[ i - 1 ]) )
			{
				removed++;
				i--;
			}
...
{
	UniChar *p = GetCPointerForWrite( 18);
	if (p != NULL)
	{
		VIndex length = 0;
		uLONG8 mask = 0xf;
		...
		if(inValue != 0)
		{
			for( sLONG shift = 60 ; shift >= 0 ; shift -= 4)
			{
		...
		{
			// si la valeur inValue vaut zero
			p[length] = '0';
			++length;
		}
	...
	while(*ptr)
	{
		if (*ptr == CHAR_AMPERSAND)
		{
			...
			if (replacement != 0)
			{
				...
				pFirst = ptr+toSkip;	// 4 is min size of all cases
			}
		}
		++ptr;
		pos++;
