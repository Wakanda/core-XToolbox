...
protected:
			...
			sLONG				fStart;
			bool				fIsInBody;
			bool				fIsNodeParagraph, fIsNodeSpan, fIsNodeImage;
								...
								:VSpanHandler(NULL, CVSTR("html"), NULL, NULL, NULL, inVersion, inLC)
			{
				...
				if (inVersion != kDOC_VERSION_SPAN4D)
				{
					...
					if (fMapOfStyle.empty())
					{
						...
						//(only mandatory for XHTML11 (because XHTML11 is parsed as a single paragraph) - SPAN4D & XHTML4D ignore it)
...
#if kDOC_VERSION_MAX_SUPPORTED >= kDOC_VERSION_XHTML4D  
					if (version >= kDOC_VERSION_XHTML4D && IsImageElementName( fElementName))
					{
						//image node
...
void VSpanHandler::EndElement( const VString& inElementName)
{
	...
	if (!fIsUnknownTag && fDocNode->GetDocumentNode()->GetVersion() != kDOC_VERSION_SPAN4D)
	{
		...
		//add break lines for elements which need it (normally only while parsing XHTML)
		if (fDocNode->GetDocumentNode()->GetVersion() == kDOC_VERSION_XHTML11 && VParserXHTML::NeedBreakLine(inElementName)) 
			*fText += "\r";
	}
...
void VSpanHandler::SetAttribute(const VString& inName, const VString& inValue)
{
	...
	else if (inName.EqualToString("style"))
	{
		...
		if (version != kDOC_VERSION_SPAN4D && !fSpanHandlerParagraph)
		{
			...
			//(if -d4-version is found, document version is modified - which changes further parsing)
	...
	else if (inName.EqualToString("xmlns:d4"))
...
void VSpanHandler::SetHTMLAttribute(const VString& inName, const VString& inValue)
{
	...
	if (inName.EqualToString(kCSS_NAME_WIDTH) && fDocNode->GetDocumentNode()->GetVersion() != kDOC_VERSION_SPAN4D) //elements as images might not use inline CSS style but html attribute 
	{
		...
		_CreateStyles();
		VSpanTextParser::Get()->ParseCSSToVDocNode( (VDocNode *)fDocNode, valueCSS);
	}
														...
														const VTextStyle *_inStyleInherit, bool inSkipCharStyles)
{
	...
	if (style->IsSpanRef())
	{
		...
		case kSpanTextRef_4DExp: //default value is NBSP
			...
			break;
#if kDOC_VERSION_MAX_SUPPORTED >= kDOC_VERSION_XHTML4D  
		case kSpanTextRef_Image: //NBSP
			{
			fIsOnlyPlainText = false;
			ioText.AppendString( VDocSpanTextRef::fNBSP);
			}
			break;
#endif
		...
		case kDOC_NODE_TYPE_IMAGE:
			{
				...
				if (isXHTML4D) //only XHTML4D format is capable to manage all paragraph properties
				{
					while (cssParser.GetNextAttribute(attribut, value))
					{
						//node common properties
...
void VSpanTextParser::_SerializeVDocNodeToCSS(  const VDocNode *inDocNode, VString& outCSSDeclarations, const VDocNode *inDocNodeDefault, bool inIsFirst)
{
	...
	if (inDocNode->GetType() == kDOC_NODE_TYPE_DOCUMENT)
	{
		...
		if (inDocNode->GetDocumentNode()->GetVersion() >= kDOC_VERSION_XHTML4D) 
		{
			...
			//CSS d4-dwrite
			isOverriden = inDocNode->IsOverriden( kDOC_PROP_DWRITE) && !inDocNode->IsInherited( kDOC_PROP_DWRITE);
			if (isOverriden)
			{
				if (count)
					outCSSDeclarations.AppendUniChar(';');
				...
				outCSSDeclarations.AppendUniChar( ':');
				if (inDocNode->GetDocumentNode()->GetDWrite())
	...
	if (inDocNode->GetType() == kDOC_NODE_TYPE_PARAGRAPH)
	{
		...
		//CSS text-indent
		isOverriden = inDocNode->IsOverriden( kDOC_PROP_TEXT_INDENT);
		if (isOverriden)
		{
...
bool VSpanTextParser::ParseAttributeImage( VDocImage *ioDocImage, const uLONG inProperty, const VString& inValue)
{
	...
	else if (inProperty == kDOC_PROP_IMG_ALT_TEXT)
	{
		...
		if (!value.IsEmpty())
			...
			value = VDocSpanTextRef::fNBSP; //at least one no-breaking space
		...
		if (valueCSS)
		{
			...
			else 
			{
				...
				case CSSProperty::kBR_REPEAT:
					ioDocNode->SetBackgroundImageRepeat( kTEXT_BACKGROUND_REPEAT);
					break;
				case CSSProperty::kBR_REPEAT_X:
					ioDocNode->SetBackgroundImageRepeat( kTEXT_BACKGROUND_REPEAT_X);
					break;
				case CSSProperty::kBR_REPEAT_Y:
					ioDocNode->SetBackgroundImageRepeat( kTEXT_BACKGROUND_REPEAT_Y);
					break;
				case CSSProperty::kBR_NO_REPEAT:
					ioDocNode->SetBackgroundImageRepeat( kTEXT_BACKGROUND_NO_REPEAT);
					break;
				default:
					xbox_assert(false); 
					break;
		...
		if (valueCSS)
		{
			...
			else 
			{
				...
				case CSSProperty::kBB_BORDER_BOX:
					ioDocNode->SetBackgroundClip( kTEXT_BACKGROUND_BORDER_BOX);
					break;
				case CSSProperty::kBB_PADDING_BOX:
					ioDocNode->SetBackgroundClip( kTEXT_BACKGROUND_PADDING_BOX);
					break;
				case CSSProperty::kBB_CONTENT_BOX:
					ioDocNode->SetBackgroundClip( kTEXT_BACKGROUND_CONTENT_BOX);
					break;
				default:
					xbox_assert(false); 
					break;
	...
	else if (inProperty == kDOC_PROP_BACKGROUND_ORIGIN)
	{
		...
		if (valueCSS)
		{
			...
			else 
			{
				...
				{
				case CSSProperty::kBB_BORDER_BOX:
					ioDocNode->SetBackgroundImageOrigin( kTEXT_BACKGROUND_BORDER_BOX);
					break;
				case CSSProperty::kBB_PADDING_BOX:
					ioDocNode->SetBackgroundImageOrigin( kTEXT_BACKGROUND_PADDING_BOX);
					break;
				case CSSProperty::kBB_CONTENT_BOX:
					ioDocNode->SetBackgroundImageOrigin( kTEXT_BACKGROUND_CONTENT_BOX);
					break;
				default:
					xbox_assert(false); 
					break;
		...
		if (valueCSS)
		{
			...
			else 
			{
				...
				case CSSProperty::kBS_COVER:
					ioDocNode->SetBackgroundImageWidth( kTEXT_BACKGROUND_SIZE_COVER);
					break;
				case CSSProperty::kBS_CONTAIN:
					ioDocNode->SetBackgroundImageWidth( kTEXT_BACKGROUND_SIZE_CONTAIN);
					break;
				default:
					{
						if (valueCSS->v.css.fBackgroundSize.fWidth.fAuto)
							ioDocNode->SetBackgroundImageWidth( kTEXT_BACKGROUND_SIZE_AUTO);	
						...
						if (valueCSS->v.css.fBackgroundSize.fHeight.fAuto)
							ioDocNode->SetBackgroundImageHeight( kTEXT_BACKGROUND_SIZE_AUTO);	
...
void VSpanTextParser::_SerializePropsCommon( const VDocNode *inDocNode, VString& outCSSDeclarations, const VDocNode *inDocNodeDefault, bool inIsFirst)
{
	...
	//CSS padding
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_PADDING);
	if (isOverriden)
	{
	...
	//CSS width
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_WIDTH);
	if (isOverriden)
	{
	...
	//CSS min-width
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_MIN_WIDTH);
	if (isOverriden)
	{
	...
	//CSS height
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_HEIGHT);
	if (isOverriden)
	{
	...
	//CSS min-height
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_MIN_HEIGHT);
	if (isOverriden)
	{
	...
	//CSS background-image 
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BACKGROUND_IMAGE);
	if (isOverriden)
	{
	...
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BACKGROUND_POSITION);
	if (isOverriden)
	{
	...
	//CSS background-repeat 
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BACKGROUND_REPEAT);
	if (isOverriden)
	{
		...
		else
		{
			...
			if (!(inDocNodeDefault && repeat == inDocNodeDefault->GetBackgroundImageRepeat()))
			{
				...
				switch( repeat)
				{
				case kTEXT_BACKGROUND_REPEAT:
					outCSSDeclarations.AppendCString( "repeat");
					break;
				case kTEXT_BACKGROUND_REPEAT_X:
					outCSSDeclarations.AppendCString( "repeat-x");
					break;
				case kTEXT_BACKGROUND_REPEAT_Y:
	...
	//CSS background-clip 
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BACKGROUND_CLIP);
	if (isOverriden)
	{
		...
		else
		{
			...
			if (!(inDocNodeDefault && clip == inDocNodeDefault->GetBackgroundClip()))
			{
				...
				switch( clip)
				{
				case kTEXT_BACKGROUND_BORDER_BOX:
					outCSSDeclarations.AppendCString( "border-box");
					break;
				case kTEXT_BACKGROUND_PADDING_BOX:
	...
	//CSS background-origin 
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BACKGROUND_ORIGIN);
	if (isOverriden)
	{
	...
	//CSS background-size 
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BACKGROUND_SIZE);
	if (isOverriden)
	{
		...
		else
		{
			...
			if (!(inDocNodeDefault && width == inDocNodeDefault->GetBackgroundImageWidth() && height == inDocNodeDefault->GetBackgroundImageHeight()))
			{
				...
				switch( width)
				{
				case kTEXT_BACKGROUND_SIZE_COVER:
					outCSSDeclarations.AppendCString( "cover");
					break;
				case kTEXT_BACKGROUND_SIZE_CONTAIN:
					outCSSDeclarations.AppendCString( "contain");
					break;
				default:
					{
						if (width != kTEXT_BACKGROUND_SIZE_AUTO)
						{
							if (width < 0)
							{
								//percentage
						...
						if (height == kTEXT_BACKGROUND_SIZE_AUTO)
						{
							if (width != kTEXT_BACKGROUND_SIZE_AUTO)
								outCSSDeclarations.AppendUniChar(' ');
	...
	//CSS border-style
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BORDER_STYLE);
	if (isOverriden)
	{
			...
			if (doIt)
			{
				...
				for (int i = 0; i < numDim; i++)
				{
					...
					case kTEXT_BORDER_STYLE_DASHED:
						sValue = "dashed";
						break;
					case kTEXT_BORDER_STYLE_SOLID:
						sValue = "solid";
						break;
					case kTEXT_BORDER_STYLE_DOUBLE:
						sValue = "double";
						break;
	...
	//CSS border-width
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BORDER_WIDTH);
	if (isOverriden)
	{
	...
	//CSS border-color
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BORDER_COLOR);
	if (isOverriden)
	{
	...
	//CSS border-radius
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_BORDER_RADIUS);
	if (isOverriden)
	{
	...
	//CSS text-align
	isOverriden = inDocNode->IsOverriden( kDOC_PROP_TEXT_ALIGN);
	if (isOverriden)
	{
