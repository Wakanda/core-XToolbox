...
#define kCSS_NAME_D4_VERSION		"-d4-version" //if "d4-version" is not defined (it is automatically defined for all edited/modified styled text in v15), it is assumed to be v12/v13 4D SPAN compatible format 
...
#define kCSS_NAME_D4_PADDING_FIRST_LINE			"-d4-padding-first-line"
...
@param inParseSpanOnly
	true (default): input text is 4D SPAN tagged text - 4D CSS rules apply (all span character styles are inherited)
...
*/
virtual bool ReplaceStyledText(	VString& ioSpanText, const VString& inSpanTextOrPlainText, sLONG inStart = 0, sLONG inEnd = -1, bool inTextPlainText = false, bool inInheritUniformStyle = false, bool inSilentlyTrapParsingErrors = true);
...
*/
virtual VDocText *ParseDocument( const VString& inDocText, const uLONG inVersion = kDOC_VERSION_SPAN4D_1, const uLONG inDPI = 72, bool inShowSpanRef = false, 
...
void _CloseTagSpan(VString& ioText, bool inAfterPlainText = true);

void _OpenTagDoc(VString& ioText);
void _CloseTagDoc(VString& ioText, bool inAfterPlainText = true);

void _OpenTagParagraph(VString& ioText);
void _CloseTagParagraph(VString& ioText, bool inAfterPlainText = true);
