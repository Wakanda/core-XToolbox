...
#define kCSS_NAME_TEXT_BACKGROUND_COLOR	"-d4-text-background-color" 
...
#define kCSS_NAME_WHITE_SPACE			"white-space" 
...
#define kCSS_NAME_LIST_STYLE_TYPE				"list-style-type"
#define kCSS_NAME_D4_LIST_STYLE_TYPE			"-d4-list-style-type"
#define kCSS_NAME_D4_LIST_STRING_FORMAT_LTR		"-d4-list-string-format-ltr"
#define kCSS_NAME_D4_LIST_STRING_FORMAT_RTL		"-d4-list-string-format-rtl"
#define kCSS_NAME_D4_LIST_FONT_FAMILY			"-d4-list-font-family"
#define kCSS_NAME_LIST_STYLE_IMAGE				"list-style-image"
#define kCSS_NAME_D4_LIST_STYLE_IMAGE_HEIGHT	"-d4-list-style-image-height"
#define kCSS_NAME_D4_LIST_START					"-d4-list-start"
#define kCSS_NAME_D4_NEW_PARA_CLASS				"-d4-new-para-class"
...
*/
virtual void TextToXML(VString& ioText, bool inFixInvalidChars = true, bool inEscapeXMLMarkup = true, 
					   bool inConvertCRtoBRTag = false, bool inConvertForCSS = false, bool inEscapeCR = false, bool inNoEncodeQuotes = false);
...
virtual void ParseSpanText( const VString& inTaggedText, VTreeTextStyle*& outStyles, VString& outPlainText, bool inParseSpanOnly = true, const uLONG inDPI = 72, 
						    SpanRefCombinedTextTypeMask inShowSpanRefs = 0, VDBLanguageContext *inLC = NULL, uLONG inOptions = kHTML_PARSER_OPTIONS_DEFAULT);
...
*/
virtual void GetPlainTextFromSpanText( const VString& inTaggedText, VString& outPlainText, bool inParseSpanOnly = true, SpanRefCombinedTextTypeMask inShowSpanRefs = kSRCTT_Value, VDBLanguageContext *inLC = NULL);
...
virtual VDocText *ParseDocument(	const VString& inDocText, const uLONG inVersion = kDOC_VERSION_SPAN4D, const uLONG inDPI = 72, 
									SpanRefCombinedTextTypeMask inShowSpanRefs = 0, bool inSilentlyTrapParsingErrors = true, VDBLanguageContext *inLC = NULL, uLONG inOptions = kHTML_PARSER_OPTIONS_DEFAULT);
...
virtual void ParseCSSToVDocNode( VDocNode *ioDocNode, const VString& inCSSDeclarations, VSpanHandler *inSpanHandler = NULL);
...
*/
virtual void SerializeDocument( const VDocText *inDoc, VString& outDocText, const VDocClassStyleManager *inDocDefaultStyleMgr = NULL, bool inSerializeDefaultStyle = false);
...
protected:
		/** pair<class name, css declarations>*/
typedef std::pair<VString, VString>     AutoClassStyle;
		/** vector of automatic class styles */
typedef std::vector<AutoClassStyle>		AutoClassStyles;
		/** map of automatic class styles per document node type */ 
typedef	std::map<eDocNodeType, AutoClassStyles> MapOfAutoClassStyle;
...
/** parse paragraph CSS property */	
bool _ParsePropParagraph( VCSSLexParser *inLexParser, VDocNode *ioDocNode, const eDocProperty inProperty, const VString& inValue);
...
void _ParseCSSWhiteSpace( VSpanHandler *inSpanHandler, const VString& inValue);
...
void _SerializeVTextStyleToCSS(  const VDocNode *inDocNode, const VTextStyle &inStyle, VString& outCSSDeclarations, const VTextStyle *inStyleInherit = NULL, bool inIgnoreColors = false, bool inD4TextBackgroundColor = false, bool inIsFirst = true,
							     eDocNodeType inClassStyleNodeType = kDOC_NODE_TYPE_PARAGRAPH, const VDocText *inClassStyleOwnerDoc = NULL);
...
bool _SerializeAttributeID( const VDocNode *inDocNode, VString& outText);

bool _SerializeAttributeClass( const VDocNode *inDocNode, VString& outText, const VString& inAutoClass = CVSTR(""));
...
void _OpenTagBody(VString& ioText, sLONG inIndent = 0);
void _CloseTagBody(VString& ioText, bool inAfterChildren = true, sLONG inIndent = 0);

void _OpenTagHead(VString& ioText, sLONG inIndent = 0);
void _CloseTagHead(VString& ioText, bool inAfterChildren = true, sLONG inIndent = 0);

void _OpenTagStyle(VString& ioText, sLONG inIndent = 0);
void _CloseTagStyle(VString& ioText, bool inAfterChildren = true, sLONG inIndent = 0);

void _OpenTagList(VString& ioText, sLONG inIndent = 0, bool inOrdered = false);
void _CloseTagList(VString& ioText, bool inAfterChildren = true, sLONG inIndent = 0, bool inOrdered = false);

void _GenerateNewClassName( const VDocText *inDoc, eDocNodeType inType, sLONG *ioNextAutoClassID, VString& outClass);

const VString &_AddAutoClassStyle(const VDocText *inDoc, eDocNodeType inType, const VString& inAutoClassStyle, 
								  sLONG *ioNextAutoClassID, MapOfAutoClassStyle& ioAutoClassStyles, const VString &inClass = CVSTR(""));

void _FinalizeAutoClassStyles( const MapOfAutoClassStyle& inAutoClassStyles, VString& outClassStyles, sLONG inIndent = 0);

void _NewLine(VString& ioText)
{
	ioText.AppendUniChar(13);
}

void _Indent(VString& ioText, sLONG inIndent = 0)
{
	inIndent *= 4;
...
void _OpenTagParagraph(VString& ioText, sLONG inIndent = 0, bool isListItem = false);
void _CloseTagParagraph(VString& ioText, bool inAfterChildren = true, sLONG inIndent = 0, bool isListItem = false);
...
void _SerializeParagraphTextAndStyles(	const VDocNode *inDocNode, const VTreeTextStyle *inStyles, const VString& inPlainText, VString &ioText, 
										...
										const VDocClassStyleManager *inDocDefaultStyleMgr = NULL, const VTextStyle *inStyleInherit = NULL, bool inSkipCharStyles = false);
 
void _SerializeSpanRefImageTagAndStyles(	const VDocText *inDoc, VDocSpanTextRef *inSpanRef, VString& ioText, 
											sLONG *ioNextAutoClassID, MapOfAutoClassStyle& ioAutoClassStyles, 
											const VDocClassStyleManager *inDocDefaultStyleMgr = NULL);
...
static	VSpanTextParser*	sInstance;
		...
		bool				fShouldSerializeListStartAuto;
};
