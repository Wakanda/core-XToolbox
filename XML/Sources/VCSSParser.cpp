...
void VCSSParserInlineDeclarations::Start( const VString& inDeclarations, bool inCheckDoubleAttribute)
{
	fDeclarations.clear();
	fDeclarations.reserve(10);
	fIndexCur = 0;
	
	std::map<VString,VString> mapOfAttrValue;
	std::map<VString,bool> mapOfAttrImportant;
			...
			case CSSToken::SEMI_COLON:
				{
					...
					if ((!name.IsEmpty()) && (!value.IsEmpty()))
					{
						if (inCheckDoubleAttribute)
						{
							mapOfAttrValue[name] = value;
							mapOfAttrImportant[name] = bIsImportant;
						}
						else
	...
	if (inCheckDoubleAttribute)
	{
		std::map<VString,VString>::const_iterator itAttr = mapOfAttrValue.begin();
		for (;itAttr != mapOfAttrValue.end(); itAttr++)
			fDeclarations.push_back( CSSRuleSet::Declaration( itAttr->first, itAttr->second, mapOfAttrImportant[itAttr->first]));
	}
	delete lexParser;
}
