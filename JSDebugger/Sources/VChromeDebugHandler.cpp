...
XBOX::VError VChromeDebugHandler::HandleRequest( IHTTPResponse* ioResponse )
{
	...
	trace += ioResponse->GetRequest().GetURL();
...

	{
		...
		{
#if 1 // GH debug WS
			if (!ioResponse->GetEndPoint()->IsSSL())
			{
...
OpaqueDebuggerContext VChromeDebugHandler::AddContext()
{
											...
											int&								ioFileNameLength)
{
	...
	if (sStaticSettings->GetData(inContext,inSourceId,fileName,tmpData))
	{
		...
		{
			VString		tmpStr;
			fileName.GetSubString(idx+1,fileName.GetLength()-idx,tmpStr);
			tmpStr.ToBlock(ioFileName,ioFileNameLength,VTC_UTF_16,true,false);
			ioFileNameLength = tmpStr.GetLength();
			result = true;
		}
	}
	if (!result)
