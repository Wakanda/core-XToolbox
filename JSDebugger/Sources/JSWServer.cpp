...
enum VConnectionHandler::E_WORK_STATUS VJSWConnectionHandler::Handle ( VError& outError )
{
	...
	while ( VTask::GetCurrent ( )-> GetState ( ) != TS_DYING && VTask::GetCurrent ( )-> GetState ( ) != TS_DEAD && !fShouldStop )
	{
		...
		if ( vError == VE_JSW_DEBUGGER_AUTHENTICATION_FAILED )
			fShouldStop = true;
	}
...
void VJSWConnectionHandler::_SetEndPoint ( VTCPEndPoint* inEndPoint )
{
	...
	if ( fEndPoint != 0 )
		; // fEndPoint-> Retain ( ); Need to release the endpoint in the servernet after SetEndPoint 
									...
									long inBeginOffset, long inEndOffset /* in bytes */ )
{
	...
	*/
	...
	fEndPointMutex. Lock ( );
		...
		VString				vstrPacket;
		VError				vError = vbagPacket. GetJSONString ( vstrPacket, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
		if ( vError == VE_OK )
		{
			vstrPacket. AppendCString ( "\r\n" );
			vError = SendPacket ( vstrPacket );
...
int VJSWConnectionHandler::SendContextCreated ( uintptr_t inContext )
{
	fEndPointMutex. Lock ( );
		...
		VString				vstrPacket;
		VError				vError = vbagPacket. GetJSONString ( vstrPacket, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays );
		if ( vError == VE_OK )
		{
			vstrPacket. AppendCString ( "\r\n" );
			vError = SendPacket ( vstrPacket );
...
::DebugMsg ( "Crossfire set break point command" );
	...
	else if ( vstrCommand. EqualToString ( CVSTR ( "authenticate" ) ) )
	{
		...
		else
		{
			...
			VString			vstrHA1;
			vbagArguments-> GetString ( CrossfireKeys::ha1, vstrHA1 );

			vError = HandleAuthenticate ( vstrID. GetLong ( ), vstrUserName, vstrHA1 );
...
VError VJSWConnectionHandler::HandleAuthenticate ( long inCommandID, const VString & inUserName, const VString & inHA1 )
{
	VError		vError = VE_OK;
	fHasAuthenticated = false;
	if ( fDebuggerSettings == 0 )
		...
		fHasAuthenticated = fDebuggerSettings-> UserCanDebug ( inUserName. GetCPointer ( ), inHA1. GetCPointer ( ), fNonce. GetCPointer ( ) );
