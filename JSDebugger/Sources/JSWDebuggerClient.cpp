...
VError VJSWDebuggerValue::GetJSONForEvaluate ( XBOX::VString& outString )
{
	...
	VError			vError = VE_OK;
	if ( bAutoBuild )
		vError = vbagEval. GetJSONString ( outString, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays );
...
VError VJSDebugger::Connect ( VString const & inDNSNameOrIP, short inPort )
{
	VError					vError = VE_OK;
	fHasDoneHandShake = false;
	fServerTools. Clear ( );
...
VError VJSDebugger::Authenticate ( VString const & inUserName, VString const & inHA1Seeded )
{
	...
	VString					vstrPacket;
	vError = vbagPacket. GetJSONString ( vstrPacket, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
	if ( vError == VE_OK )
	{
...
XBOX::VError VJSDebugger::SetBreakPoint ( XBOX::VString const & inRelativeFilePath, XBOX::VIndex inLineNumber )
{
	...
	vbagArguments-> SetLong ( CrossfireKeys::line, inLineNumber );
	vbagPacket. AddElement( CrossfireKeys::arguments, vbagArguments );
	vbagArguments-> Release ( );
...
XBOX::VError VJSDebugger::HandleEvaluateResponse ( XBOX::VValueBag & inMessage )
{
	...
	vError = inMessage. GetJSONString ( vstrEvalJSON, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
	if ( vError == VE_OK )
	{
		if ( bIsNullResult )
		{
...
XBOX::VError VJSDebugger::HandleLookupResponse ( XBOX::VValueBag & inMessage )
{
	...
	vError = inMessage. GetJSONString ( vstrLookupJSON, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
	if ( vError == VE_OK )
	{
		VJSWDebuggerValue*		valResult = new VJSWDebuggerValue ( CVSTR ( "" ) );
...
XBOX::VError VJSDebugger::HandleScopeResponse ( XBOX::VValueBag & inMessage )
{
	...
	vError = inMessage. GetJSONString ( vstrScopeJSON, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
	if ( vError == VE_OK )
	{
		VJSWDebuggerValue*		valResult = new VJSWDebuggerValue ( CVSTR ( "" ) );
...
XBOX::VError VJSDebugger::HandleScopeChainResponse ( XBOX::VValueBag & inMessage )
{
	...
	vError = inMessage. GetJSONString ( vstrScopesJSON, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
	if ( vError == VE_OK )
	{
		VJSWDebuggerValue*		valResult = new VJSWDebuggerValue ( CVSTR ( "" ) );
...
XBOX::VError VJSDebugger::HandleScriptResponse ( XBOX::VValueBag & inMessage )
{
	...
	else
	{
		...
		vError = inMessage. GetJSONString ( vstrSource, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
...
XBOX::VError VJSDebugger::HandleCallStackResponse ( XBOX::VValueBag & inMessage )
{
	...
	if ( vError == VE_OK )
	{
		...
		vError = inMessage. GetJSONString ( vstrCallStackJSON, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
		if ( vError == VE_OK )
		{
			VJSWDebuggerValue*		valResult = new VJSWDebuggerValue ( CVSTR ( "" ) );
...
XBOX::VError VJSDebugger::HandleListContextsResponse ( XBOX::VValueBag & inMessage )
{
	...
	vError = inMessage. GetJSONString ( vstrContextsListJSON, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
	if ( vError == VE_OK )
	{
		VJSWDebuggerValue*			valResult = new VJSWDebuggerValue ( CVSTR ( "" ) );
...
VError VJSDebugger::HandleRemoteContextCreated ( VValueBag & inMessage )
{
	...
	VString					vstrMessage;
	vError = inMessage. GetJSONString ( vstrMessage, JSON_WithQuotesIfNecessary | JSON_FormatDateIso | JSON_UniqueSubElementsAreNotArrays | JSON_PrettyFormatting );
