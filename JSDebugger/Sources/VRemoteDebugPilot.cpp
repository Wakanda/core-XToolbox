...
void VRemoteDebugPilot::TreatStartMsg(RemoteDebugPilotMsg_t& ioMsg)
{
	switch(fState)
	{
	case STOPPED_STATE:
...
void VRemoteDebugPilot::TreatStopMsg(RemoteDebugPilotMsg_t& ioMsg)
{
	XBOX::VError	err;
	switch(fState)
	{
				...
				if (tmpStr && fClientId.GetLength())
				{
					...
					*endStr = 0;
					...
					if ( testAssert(ctxIt != fContextArray.end()) )
					{
						...
						{
							(*ctxIt).second.fPage = new VChromeDbgHdlPage();
