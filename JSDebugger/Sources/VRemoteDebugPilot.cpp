...
void VRemoteDebugPilot::HandleDisconnectingState()
{
	...
	if (err)
	{
		DEBUG_MSG("VRemoteDebugPilot::HandleDisconnectingState fWS->ReadMessage pb\n");
		//AbortContexts(true);
		DisconnectBrowser();
		return;
	}
...
void VRemoteDebugPilot::HandleConnectingState()
{
	...
	if (err)
	{
		DEBUG_MSG("VRemoteDebugPilot::HandleConnectingState fWS->ReadMessage pb\n");
	}
	/*else
	{
		if (!msgLen)
...
void VRemoteDebugPilot::HandleConnectedState()
{
	...
	if (err)
	{
		DEBUG_MSG("VRemoteDebugPilot::HandleConnectedState fWS->ReadMessage pb\n");
		DisconnectBrowser();
		fState = STARTED_STATE;
		return;
	}
...
void VRemoteDebugPilot::VPilotNewContextMessage::DoExecute()
{
	...
	if (fPilot->fState >= STARTED_STATE)
	{
		if ( testAssert(fPilot->fContextArray.size() < K_NB_MAX_CTXS) )
		{
			...
			*(fOutCtx) = (OpaqueDebuggerContext)sRemoteDebugPilotContextId;
