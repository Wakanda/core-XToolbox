...
void VRemoteDebugPilot::HandleConnectingState()
{
	...
	if (err)
	{
		DebugMsg("VRemoteDebugPilot::HandleConnectingState  pb\n");
		fWS->Close();
		fState = STARTED_STATE;
	}
	else
...
//wait_connection:
	...
	DebugMsg("VRemoteDebugPilot::InnerTaskProc quits \n");
	return 0;
}
											...
											XBOX::VectorOfVString&		inSourceData)
{
	...
	if (err == VE_OK)
	{
		...
		VPilotBreakpointReachedMessage*		breakpointReachedMsg = new VPilotBreakpointReachedMessage(this);
